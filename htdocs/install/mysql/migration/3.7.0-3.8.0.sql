...
INSERT INTO llx_c_incoterms (code, label, active) VALUES ('EXW', 'Ex Works, au départ non chargé, non dédouané sortie d''usine (uniquement adapté aux flux domestiques, nationaux)', 1);
INSERT INTO llx_c_incoterms (code, label, active) VALUES ('FCA', 'Free Carrier, marchandises dédouanées et chargées dans le pays de départ, chez le vendeur ou chez le commissionnaire de transport de l''acheteur', 1);
INSERT INTO llx_c_incoterms (code, label, active) VALUES ('FAS', 'Free Alongside Ship, sur le quai du port de départ', 1);
INSERT INTO llx_c_incoterms (code, label, active) VALUES ('FOB', 'Free On Board, chargé sur le bateau, les frais de chargement dans celui-ci étant fonction du liner term indiqué par la compagnie maritime (à la charge du vendeur)', 1);
INSERT INTO llx_c_incoterms (code, label, active) VALUES ('CFR', 'Cost and Freight, chargé dans le bateau, livraison au port de départ, frais payés jusqu''au port d''arrivée, sans assurance pour le transport, non déchargé du navire à destination (les frais de déchargement sont inclus ou non au port d''arrivée)', 1);
INSERT INTO llx_c_incoterms (code, label, active) VALUES ('CIF', 'Cost, Insurance and Freight, chargé sur le bateau, frais jusqu''au port d''arrivée, avec l''assurance marchandise transportée souscrite par le vendeur pour le compte de l''acheteur', 1);
INSERT INTO llx_c_incoterms (code, label, active) VALUES ('CPT', 'Carriage Paid To, livraison au premier transporteur, frais jusqu''au déchargement du mode de transport, sans assurance pour le transport', 1);
INSERT INTO llx_c_incoterms (code, label, active) VALUES ('CIP', 'Carriage and Insurance Paid to, idem CPT, avec assurance marchandise transportée souscrite par le vendeur pour le compte de l''acheteur', 1);
INSERT INTO llx_c_incoterms (code, label, active) VALUES ('DAT', 'Delivered At Terminal, marchandises (déchargées) livrées sur quai, dans un terminal maritime, fluvial, aérien, routier ou ferroviaire désigné (dédouanement import, et post-acheminement payés par l''acheteur)', 1);
INSERT INTO llx_c_incoterms (code, label, active) VALUES ('DAP', 'Delivered At Place, marchandises (non déchargées) mises à disposition de l''acheteur dans le pays d''importation au lieu précisé dans le contrat (déchargement, dédouanement import payé par l''acheteur)', 1);
INSERT INTO llx_c_incoterms (code, label, active) VALUES ('DDP', 'Delivered Duty Paid, marchandises (non déchargées) livrées à destination finale, dédouanement import et taxes à la charge du vendeur ; l''acheteur prend en charge uniquement le déchargement (si exclusion des taxes type TVA, le préciser clairement)', 1);

-- Remove term libelle - replace by label
ALTER TABLE llx_adherent_type DROP INDEX uk_adherent_type_libelle;
ALTER TABLE llx_adherent_type CHANGE COLUMN libelle label VARCHAR(50) NOT NULL;
ALTER TABLE llx_adherent_type ADD UNIQUE INDEX uk_adherent_type_label (label, entity);
ALTER TABLE llx_c_actioncomm CHANGE COLUMN libelle label VARCHAR(48) NOT NULL;
ALTER TABLE llx_c_barcode_type CHANGE COLUMN libelle label VARCHAR(50) NOT NULL;
ALTER TABLE llx_c_chargesociales CHANGE COLUMN libelle label VARCHAR(80) NOT NULL;
ALTER TABLE llx_c_ecotaxe CHANGE COLUMN libelle label VARCHAR(255);
ALTER TABLE llx_c_effectif CHANGE COLUMN libelle label VARCHAR(30);
ALTER TABLE llx_c_forme_juridique CHANGE COLUMN libelle label VARCHAR(255);
ALTER TABLE llx_c_input_method CHANGE COLUMN libelle label VARCHAR(60);
ALTER TABLE llx_c_paiement CHANGE COLUMN libelle label VARCHAR(30);
ALTER TABLE llx_c_payment_term CHANGE COLUMN libelle label VARCHAR(255);
ALTER TABLE llx_c_payment_term CHANGE COLUMN libelle_facture label_facture TEXT;
ALTER TABLE llx_c_shipment_mode CHANGE COLUMN libelle label VARCHAR(50) NOT NULL;
ALTER TABLE llx_c_stcomm CHANGE COLUMN libelle label VARCHAR(30);
ALTER TABLE llx_c_type_contact CHANGE COLUMN libelle label VARCHAR(64) NOT NULL;
ALTER TABLE llx_c_typent CHANGE COLUMN libelle label VARCHAR(30);
ALTER TABLE llx_chargesociales CHANGE COLUMN libelle label VARCHAR(80) NOT NULL;
ALTER TABLE llx_document_model CHANGE COLUMN libelle label VARCHAR(255);
ALTER TABLE llx_facture_fourn CHANGE COLUMN libelle label VARCHAR(255);
ALTER TABLE llx_rights_def CHANGE COLUMN libelle label VARCHAR(255);