...
UPDATE llx_accounting_account AS acc JOIN llx_const AS cst ON acc.account_number = cst.value SET acc.centralized = 1 WHERE cst.name in (__ENCRYPT('ACCOUNTING_ACCOUNT_CUSTOMER')__,__ENCRYPT('ACCOUNTING_ACCOUNT_SUPPLIER')__,__ENCRYPT('SALARIES_ACCOUNTING_ACCOUNT_PAYMENT')__);
