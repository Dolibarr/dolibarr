...
$oneoptionset = (GETPOST('standard', 'alpha') || GETPOST('restore_thirdparties_logos', 'alpha') || GETPOST('clean_linked_elements', 'alpha') || GETPOST('clean_menus', 'alpha')
	...
	|| GETPOST('rebuild_sequences', 'alpha') || GETPOST('recalculateinvoicetotal', 'alpha') || GETPOST('recalculatecommandetotal', 'alpha'));
...
// Repair llx_commande to calculate totals from line items
// WARNING : The process can be long on production environments due to restrictions.
// consider raising php_max_execution time if failing to execute completely.
if ($ok && GETPOST('recalculatecommandetotal') == 'confirmed') {
	$err = 0;
	$db->begin();
	$sql = "
		SELECT
			f.rowid,
			SUM(fd.total_ht) as total_ht
		FROM ".MAIN_DB_PREFIX."commande f 
			LEFT JOIN ".MAIN_DB_PREFIX."commandedet fd
				ON fd.fk_commande = f.rowid
		WHERE f.total_ht = 0 
		GROUP BY fd.fk_commande HAVING SUM(fd.total_ht) != 0";
	$resql = $db->query($sql);
	if ($resql) {
		$num = $db->num_rows($resql);
		print "We found ".$num." commandes qualified that will have their total recalculated because they are at zero and line items not at zero\n";
		dol_syslog("We found ".$num." commandes qualified that will have their total recalculated because they are at zero and line items not at zero");
		...
		if ($num) {
			...
			while ($i < $num) {
				...
				$sql_calculs = "
					SELECT 
						...
						SUM(fd.total_ttc) as 'total_ttc'
					FROM
						".MAIN_DB_PREFIX."commandedet fd
					WHERE 
						fd.fk_commande = $obj->rowid";
				$ressql_calculs = $db->query($sql_calculs);
				while ($obj_calcul = $db->fetch_object($ressql_calculs)) {
					$sql_maj = "
						UPDATE ".MAIN_DB_PREFIX."commande 
						SET 
			...
			print "Pas de commandes à traiter\n";
		}
	} else {
		dol_print_error($db);
		dol_syslog("calculate_total_and_taxes.php: Error");
