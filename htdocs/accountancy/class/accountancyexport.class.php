				...
				if (($line->doc_type == 'customer_invoice' || $line->doc_type == 'supplier_invoice') && $line->fk_doc > 0) {
					if (($line->doc_type == 'customer_invoice' && !isset($invoices_infos[$line->fk_doc])) ||
						...
						if ($line->doc_type == 'customer_invoice') {
							...
							$sql = 'SELECT f.ref, s.name FROM ' . MAIN_DB_PREFIX . 'facture as f';
							...
							if ($resql) {
								if ($obj = $this->db->fetch_object($resql)) {
									// Save invoice infos
									$invoices_infos[$line->fk_doc] = array('ref' => $obj->ref, 'company_name' => $obj->name);
									$invoice_ref = $obj->ref;
							...
							$sql = 'SELECT ff.ref, s.name FROM ' . MAIN_DB_PREFIX . 'facture_fourn as ff';
							...
							if ($resql) {
								if ($obj = $this->db->fetch_object($resql)) {
									// Save invoice infos
									$supplier_invoices_infos[$line->fk_doc] = array('ref' => $obj->ref, 'company_name' => $obj->name);
									$invoice_ref = $obj->ref;
									$company_name = $obj->name;
								}
							}
