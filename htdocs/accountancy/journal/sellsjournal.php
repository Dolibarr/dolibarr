	...
	foreach ($tabfac as $key => $val) {
		...
		if (getDolGlobalString('INVOICE_USE_RETAINED_WARRANTY') && isset($tabwarranty[$key])) {
			foreach ($tabwarranty[$key] as $k => $mt) {
				...
				print '"'.mb_convert_encoding($bookkeepingstatic->accountingLabelForOperation($companystatic->name, $invoicestatic->ref, $langs->trans("RetainedWarranty")), 'ISO-8859-1').'"'.$sep;
		...
		foreach ($tabttc[$key] as $k => $mt) {
			...
			print '"'.length_accounta(html_entity_decode($k)).'"'.$sep;
			print '"'.$langs->trans("Thirdparty").'"'.$sep;
			print '"'.mb_convert_encoding($bookkeepingstatic->accountingLabelForOperation($companystatic->name, $invoicestatic->ref, $langs->trans("Thirdparty")), 'ISO-8859-1').'"'.$sep;
			print '"'.($mt >= 0 ? price($mt) : '').'"'.$sep;
			print '"'.($mt < 0 ? price(-$mt) : '').'"'.$sep;
			print '"'.$journal.'"';
			print "\n";
		...
		foreach ($listoftax as $numtax) {
			...
			foreach ($arrayofvat[$key] as $k => $mt) {
				if ($mt) {
					...
					print '"'.mb_convert_encoding($bookkeepingstatic->accountingLabelForOperation($companystatic->name, $invoicestatic->ref, $langs->trans("VAT").implode($def_tva[$key][$k]).' %'.($numtax ? ' - Localtax '.$numtax : '')), 'ISO-8859-1').'"'.$sep;
					print '"'.($mt < 0 ? price(-$mt) : '').'"'.$sep;
					print '"'.($mt >= 0 ? price($mt) : '').'"'.$sep;
					print '"'.$journal.'"';
					print "\n";
