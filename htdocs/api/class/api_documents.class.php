...
class Documents extends DolibarrApi
{
	 ...
	 * @param	int		$agenda_id				Optional agenda action id to link file
	 * @return  string
	 *
	 ...
	 */
	public function post($filename, $modulepart, $ref = '', $subdir = '', $filecontent = '', $fileencoding = '', $overwriteifexists = 0, $createdirifnotexists = 1, $position = 0, $cover = '', $array_options = [], $generateThumbs = 0, $agenda_id = 0)
	{
		global $conf;
		...
		if (!empty($agenda_id)) {
			require_once DOL_DOCUMENT_ROOT.'/ecm/class/ecmfiles.class.php';
			$rel_file = preg_replace('/^'.preg_quote(DOL_DATA_ROOT, '/').'/', '', $dest_file);
			$rel_file = preg_replace('/^[\\/]/', '', $rel_file);
			$ecmfile = new EcmFiles($this->db);
			$res = $ecmfile->fetch(0, '', $rel_file);
			if ($res > 0) {
				$ecmfile->agenda_id = (int) $agenda_id;
				$ecmfile->update(DolibarrApiAccess::$user);
			}
		}
