...
class Documents extends DolibarrApi
{
	...
	public function getDocumentsListByElement($modulepart, $id = 0, $ref = '', $sortfield = '', $sortorder = '', $limit = 100, $page = 0, $content_type = '', $pagination_data = false)
	{
		...
		/** @var array<array{name:string,path:string,level1name:string,relativename:string,fullname:string,date:string,size:int,perm:int,type:string,ref:string,label:string,filepath:string,filename:string,fullpath_orig:string,position:int,gen_or_uploaded:int,description:string,keywords:string,cover:int,share:int,date_c:string,agenda_id:int,fk_user_c:int,fk_user_m:int,note_private:string,note_public:string,content-type:string}> $filearray */
		'@phan-var-force array<array{name:string,path:string,level1name:string,relativename:string,fullname:string,date:string,size:int,perm:int,type:string,ref:string,label:string,filepath:string,filename:string,fullpath_orig:string,position:int,gen_or_uploaded:int,description:string,keywords:string,cover:int,share:int,date_c:string,agenda_id:int,fk_user_c:int,fk_user_m:int,note_private:string,note_public:string,content-type:string}> $filearray';
			...
			if (($object->id) > 0 && !empty($modulepart)) {
				...
				if ($result < 0) {
					...
					for ($i = 0 ; $i < $count ; $i++) {
						...
						$relativeName = isset($fileInfo['relativename']) ? (string) $fileInfo['relativename'] : '';
						if ($relativeName !== '') {
							$filearray[$i]['content-type'] = dol_mimetype($relativeName);
						}
						$arraycontenttype = explode(",", $content_type);
						if (!empty($content_type) && $relativeName !== '' && !in_array(dol_mimetype($relativeName), $arraycontenttype)) {
							unset($filearray[$i]);
							$countarray -= 1;
						}
					}
