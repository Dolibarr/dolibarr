...
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_vat as ct ON fd.tva_tx = ct.taux AND fd.info_bits = ct.recuperableonly AND ct.fk_pays = '".$idpays."'";
...
foreach ($tabfac as $key => $val)
{
	...
	$lines = array(
		...
		array(
			'var' => $tabttc[$key],
			'label' => $langs->trans('ThirdParty').' ('.$companystatic->getObjectUrl(0, 'supplier', 16).')',
			'nomtcheck' => true,
			'inv' => true
		)
	);
