...
if ($id)
{
	...
	if ($action == 'rejet' && $user->rights->prelevement->bons->credit)
	{
		...
		print '<form name="confirm_rejet" method="post" action="'.$_SERVER["PHP_SELF"].'?id='.$id.'">';
	...
	if ($action == '')
	{
		if ($bon->statut == 2 && $lipre->statut == 2)
		{
			...
			{
	  			print "<a class=\"butAction\" href=\"".$_SERVER["PHP_SELF"]."?action=rejet&amp;id=$lipre->id\">".$langs->trans("StandingOrderReject")."</a>";
			}
			else
			{
