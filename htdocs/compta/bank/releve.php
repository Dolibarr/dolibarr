...
else
{
	...
	$mesprevnext.='<li class="pagination"><a data-role="button" data-icon="arrow-l" data-iconpos="left" href="'.$_SERVER["PHP_SELF"].'?rel=prev&amp;num='.$num.'&amp;account='.$object->id.'"><</a></li>';
	//$mesprevnext.=' &nbsp; ';
	$mesprevnext.='<li class="pagination"><span class="inactive">'.$langs->trans("AccountStatement")." ".$num.'</span></li>';
	//$mesprevnext.=' &nbsp; ';
    $mesprevnext.='<li class="pagination"><a data-role="button" data-icon="arrow-r" data-iconpos="right" href="'.$_SERVER["PHP_SELF"].'?rel=next&amp;num='.$num.'&amp;account='.$object->id.'">></a></li>';
    $mesprevnext.='</ul></div>';
	...
	$transactions = $object->getConciledTransactions($num);
	...
	foreach ($transactions as $transaction) {
		...
		// Date operation
		print '<td class="nowrap" align="center">'.dol_print_date($db->jdate($transaction->dateo),"day").'</td>';

		// Date de valeur
		print '<td align="center" valign="center" class="nowrap">';
		print '<a href="releve.php?action=dvprev&amp;num='.$num.'&amp;account='.$object->id.'&amp;dvid='.$transaction->id.'">';
		print img_previous().'</a> ';
		print dol_print_date($db->jdate($transaction->datev),"day") .' ';
		print '<a href="releve.php?action=dvnext&amp;num='.$num.'&amp;account='.$object->id.'&amp;dvid='.$transaction->id.'">';
		print img_next().'</a>';
		print "</td>\n";

		// Type and num
		print '<td class="nowrap">'.$transaction->getPaymentType(true).'</td>';

		// Description
		print '<td valign="center"><a href="'.DOL_URL_ROOT.'/compta/bank/ligne.php?rowid='.$transaction->id.'&amp;account='.$object->id.'">';
		 ...
		 */
		$newline=1;
		$links = $object->get_url($transaction->id);
		...
		foreach ($transaction->getCategories() as $trcategory) {
			if ($newline) {
				print '<br>';
			}

			print "<br>-&nbsp;<i>$trcategory->label</i>";
		}
		...
		if ($transaction->amount < 0)
		{
			$totald = $totald + abs($transaction->amount);
			print '<td align="right" class="nowrap">'.price($transaction->amount * -1)."</td><td>&nbsp;</td>\n";
		}
		else
		{
			$totalc = $totalc + abs($transaction->amount);
			print '<td>&nbsp;</td><td align="right" class="nowrap">'.price($transaction->amount)."</td>\n";
		}
		...
		{
			print "<td align=\"center\"><a href=\"ligne.php?rowid=$transaction->id&amp;account=".$object->id."\">";
			print img_edit();
			print "</a></td>";
		}
		else
