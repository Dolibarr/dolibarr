...
if ($action == 'create') {
	...
	if ($sourceInvoice->fetch($id, $ref) > 0) {
		...
		print $form->select_types_paiements(GETPOSTISSET('mode_reglement_id') ? GETPOSTINT('mode_reglement_id') : $sourceInvoice->mode_reglement_id, 'mode_reglement_id', '', 0, 1, 0, 0, 1, '', 1);
		...
		if ($sourceInvoice->fk_account > 0) {
			...
			$form->formSelectAccount($_SERVER['PHP_SELF'].'?id='.$sourceInvoice->id, $sourceInvoice->fk_account, 'none');
			print "</td></tr>";
		}
