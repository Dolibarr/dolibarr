...
$sql = "SELECT s.nom as name, s.rowid as socid, f.rowid as facid, f.titre as title, f.total, f.total_vat, f.total_ttc, f.frequency, f.unit_frequency,";
	...
	if ($num > 0)
	{
		...
		while ($i < min($num, $limit))
		{
			...
			if (!empty($arrayfields['f.total_tva']['checked']))
			{
			    ...
			    $totalarray['val']['f.total_tva'] += $objp->total_vat;
			}
			if (!empty($arrayfields['f.total_ttc']['checked']))
			{
