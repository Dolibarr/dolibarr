...
class Facture extends CommonInvoice {
            ...
            if (!$error && empty($this->fac_rec) && count($this->lines) && is_object($this->lines[0])) { // If this->lines is array of InvoiceLines (preferred mode)
                ...
                foreach ($this->lines as $i => $val) {
                    ...
                    if ($result >= 0) {
                        ...
                        $result = $this->addline(
                                $line->desc, $line->subprice, $line->qty, $vatrate, $line->localtax1_tx, $line->localtax2_tx, $line->fk_product, $line->remise_percent, $line->date_start, $line->date_end, $line->fk_code_ventilation, $line->info_bits, $line->fk_remise_except, 'HT', 0, $line->product_type, $line->rang, $line->special_code, $this->element, $line->id, $fk_parent_line, $line->fk_fournprice, $line->pa_ht, $line->label, $line->array_options, $line->situation_percent, $line->fk_prev_id, $line->fk_unit, $line->pu_ht_devise
                        );
                        if ($result < 0) {
                            $this->error = $this->db->lasterror();
    ...
    function fetch($rowid, $ref = '', $ref_ext = '', $ref_int = '', $fetch_situation = false) {
        ...
        if ($rowid)
            ...
            $sql .= ' WHERE f.entity IN (' . getEntity('facture') . ')'; // Dont't use entity if you use rowid
        ...
        if ($result && $this->db->num_rows($result) > 0) {
            while ($objp = $this->db->fetch_object($result)) {
                ...
                if ($invoice->fetch($objp->rowid) > 0) {
                    if ($objp->situation_counter < $this->situation_counter || ($objp->situation_counter == $this->situation_counter && $objp->rowid < $this->id) // This case appear when there are credit notes
                    ) {
