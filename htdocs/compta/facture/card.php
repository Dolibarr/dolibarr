		...
		if ((GETPOST('type') == Facture::TYPE_STANDARD || GETPOST('type') == Facture::TYPE_DEPOSIT || GETPOST('type') == Facture::TYPE_PROFORMA || (GETPOST('type') == Facture::TYPE_SITUATION && !GETPOST('situations'))) && GETPOST('fac_rec') <= 0) {
			...
			if (!$error) {
				...
				if (!empty($origin) && !empty($originid)) {
					...
					if (GETPOST('other_linked_objects', 'array') && !empty(GETPOST('other_linked_objects', 'array'))) {
						$object->linked_objects = array_merge($object->linked_objects, GETPOST('other_linked_objects', 'array'));
					}
								...
								for ($i = 0; $i < $num; $i++) {
									...
									if ($lines[$i]->subprice < 0 && empty($conf->global->INVOICE_KEEP_DISCOUNT_LINES_AS_IN_ORIGIN)) {
										...
										$result = $object->addline(
											...
											$label,
											$array_options,
											isset($lines[$i]->situation_percent) ? $lines[$i]->situation_percent : null,
											isset($lines[$i]->fk_prev_id) ? $lines[$i]->fk_prev_id : null,
											$lines[$i]->fk_unit,
											0,
											'',
											1
