	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $permissiontoadd) {
		...
		if ($close_code) {
			...
			if (isModEnabled('tax') && $close_code == 'withholdingtax') {
				...
				$resteapayer = GETPOST("resteapayer");
...
if ($action == 'create') {
	...
	if ($action == 'paid' && $resteapayer > 0 && (!getDolGlobalString('INVOICE_CAN_SET_PAID_EVEN_IF_PARTIALLY_PAID') || $resteapayer != $object->total_ttc)) {
		...
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?facid='.$object->id.'&resteapayer='.$resteapayer, $langs->trans('ClassifyPaid'), $langs->trans('ConfirmClassifyPaidPartially', $object->ref), 'confirm_paid_partially', $formquestion, "yes", 1, 400, 600);
	}
