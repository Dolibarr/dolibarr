...
if ($action == 'create') {
	...
	if ($object->type != Facture::TYPE_CREDIT_NOTE) {
		...
		if ($object->multicurrency_code != $conf->currency || $object->multicurrency_tx != 1) {
			...
			print '</span>';
			print '</td>';
			print '<td class="right'.($resteapayeraffiche ? ' amountremaintopayback' : (' '.$cssforamountpaymentcomplete)).'">'.(!empty($object->multicurrency_code) ? $object->multicurrency_code : $conf->currency).' '.price($sign * $multicurrency_remaintopay).'</td><td>&nbsp;</td></tr>';
		}
