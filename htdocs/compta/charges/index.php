	...
	if (empty($_GET["mode"]) || $_GET["mode"] != 'sconly')
	{
		...
		if ($result)
		{
		    ...
		    while ($i < $num)
		    {
		        ...
		        print '<td align="left">'.$sal_static->getNameUrl(1)."</td>\n";
...
if ($conf->tax->enabled)
{
	...
	if ($resql)
	{
		...
		while ($i < $num)
		{
			...
			print $socialcontrib->getNameUrl(1,'20');
			print '</td>';
			// Type
			...
			print '<td>'.$payment_sc_static->getNameUrl(1)."</td>\n";
			// Date payment
	...
	if (empty($_GET["mode"]) || $_GET["mode"] != 'sconly')
	{
		...
		if ($result)
		{
		    ...
		    while ($i < $num)
		    {
		        ...
		        print '<td align="left">'.$tva_static->getNameUrl(1)."</td>\n";
