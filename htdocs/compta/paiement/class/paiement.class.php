		...
		if (empty($totalamount) && empty($atleastonepaymentnotnull)) // We accept negative amounts for withdraw reject but not empty arrays
{
	...
	function delete($notrigger = 0) {
		...
		if ($bank_line_id > 0) {
			...
			if ($result == 0)
				$accline->rowid = $bank_line_id; // If not found, we set artificially rowid to allow delete of llx_bank_url
	...
	function addPaymentToBank($user, $mode, $label, $accountid, $emetteur_nom, $emetteur_banque, $notrigger = 0) {
		...
		if (! empty($conf->banque->enabled)) {
			...
			$bank_line_id = $acc->addline($this->datepaye, $this->paiementid, // Payment mode id or code ("CHQ or VIR for example")
$label, $totalamount, // Sign must be positive when we receive money (customer payment), negative when you give money (supplier invoice or credit note)
$this->num_paiement, '', $user, $emetteur_nom, $emetteur_banque);
							...
							if (! in_array($fac->thirdparty->id, $linkaddedforthirdparty)) // Not yet done for this thirdparty
{
				...
				if (! $error && $label == '(WithdrawalPayment)') {
					$result = $acc->add_url_line($bank_line_id, $this->id_prelevement, DOL_URL_ROOT . '/compta/prelevement/card.php?id=', $this->num_paiement, 'withdraw');
				}
	...
	function getNextNumRef($soc, $mode = 'next') {
		...
		else if ($conf->global->PAYMENT_ADDON == 'ant')
			$conf->global->PAYMENT_ADDON = 'mod_payment_ant';
		else if ($conf->global->PAYMENT_ADDON == 'cicada')
			$conf->global->PAYMENT_ADDON = 'mod_payment_cicada';
				...
				if (! empty($value)) // one value found then payment is in invoice currency
{
	...
	function getInvoiceUrl($withpicto = 0) {
		...
		if ($resql) {
			...
			$link = '<a href="' . DOL_URL_ROOT . '/compta/facture.php?facid=' . $obj->facid . '" title="' . dol_escape_htmltag($label, 1) . '" class="classfortooltip">';
			$linkend = '</a>';
	...
	function LibStatut($status, $mode = 0) {
		...
		if ($mode == 2) {
			if ($status == 0)
				return img_picto($langs->trans('ToValidate'), 'statut1') . ' ' . $langs->trans('ToValidate');
			if ($status == 1)
				return img_picto($langs->trans('Validated'), 'statut4') . ' ' . $langs->trans('Validated');
		}
		if ($mode == 3) {
			if ($status == 0)
				return img_picto($langs->trans('ToValidate'), 'statut1');
			if ($status == 1)
		...
		if ($mode == 5) {
			if ($status == 0)
				return $langs->trans('ToValidate') . ' ' . img_picto($langs->trans('ToValidate'), 'statut1');
			if ($status == 1)
