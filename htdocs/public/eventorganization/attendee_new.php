...
$conferenceattendeecounts = array();
if ($type == 'global') {
	...
	if ($resultproject < 0) {
		...
		if (is_array($resultconferences)) {
			foreach ($resultconferences as $availableconference) {
				...
				$sql .= ' WHERE fk_actioncomm = '.((int) $availableconference->id);
				$sql .= ' AND status IN (0, 1)';
				$resql = $db->query($sql);
				if ($resql && ($obj = $db->fetch_object($resql))) {
					$conferenceattendeecounts[$availableconference->id] = (int) $obj->nb;
				}
			}
		}
	}
...
if ((!empty($conference->id) && $conference->status == ConferenceOrBooth::STATUS_CONFIRMED) || (!empty($project->id) && $project->status == Project::STATUS_VALIDATED)) {
	if (empty($maxattendees) ||
		...
		if ($type == 'global' && count($registrableconferences) > 0) {
			print '<tr><td>'.$form->textwithpicto($langs->trans('OptionalConferenceSessions'), $langs->trans('OptionalConferenceSessionsHelp')).'</td><td>';
			foreach ($registrableconferences as $registrableconference) {
				$currentconferenceattendees = $conferenceattendeecounts[$registrableconference->id] ?? 0;
				...
				if (!empty($registrableconference->max_participants)) {
					print ' <span class="opacitymedium">('.$currentconferenceattendees.' / '.((int) $registrableconference->max_participants).')</span>';
				}
				if ($isfull) {
