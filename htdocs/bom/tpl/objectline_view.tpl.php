...
if (!empty($line->fk_bom_child)) {
	echo '<span class="amount">'.price($tmpbom->total_cost * $line->qty / (($tmpbom->qty>1 ? $tmpbom->qty : 1)*$line->efficiency)).'</span>';
} else {
...
if ($resql) {
	...
	while ($obj = $object->db->fetch_object($resql)) {
		...
		if ($sub_bom_line->qty_frozen > 0) {
			print '<td class="linecolqty nowrap right" id="sub_bom_qty_'.$sub_bom_line->id.'">'.price($sub_bom_line->qty / ($tmpbom->qty>1?$tmpbom->qty:1), 0, '', 0, 0).'</td>';
			...
			print '<td class="linecolqty nowrap right" id="sub_bom_qty_'.$sub_bom_line->id.'">'.price($sub_bom_line->qty * $line->qty / ($tmpbom->qty>1?$tmpbom->qty:1), 0, '', 0, 0).'</td>';
			if (getDolGlobalString('PRODUCT_USE_UNITS')) {
				print '<td class="linecoluseunit nowrap left">';
				if ($label !== '') {
					print $langs->trans($label);
		...
		if (!empty($sub_bom->id)) {
			...
			print '<td class="linecolcost nowrap right" id="sub_bom_cost_'.$sub_bom_line->id.'"><span class="amount">'.price(price2num($sub_bom->total_cost * $sub_bom_line->qty * $line->qty / ($tmpbom->qty>1?$tmpbom->qty:1), 'MT')).'</span></td>';
			$total_cost+= $sub_bom->total_cost * $sub_bom_line->qty * $line->qty / ($tmpbom->qty>1?$tmpbom->qty:1);
			...
			print '<td class="linecolcost nowrap right" id="sub_bom_cost_'.$sub_bom_line->id.'"><span class="amount">'.price(price2num($sub_bom_line->total_cost / (($tmpbom->qty>1?$tmpbom->qty:1)*$sub_bom_line->efficiency), 'MT')).'</span></td>';
			$this->total_cost += $line->total_cost;
		} elseif ($sub_bom_product->cost_price > 0) {
			print '<td class="linecolcost nowrap right" id="sub_bom_cost_'.$sub_bom_line->id.'">';
			print '<span class="amount">'.price(price2num($sub_bom_product->cost_price * $sub_bom_line->qty * $line->qty / (($tmpbom->qty>1?$tmpbom->qty:1)*$sub_bom_line->efficiency), 'MT')).'</span></td>';
			$total_cost+= $sub_bom_product->cost_price * $sub_bom_line->qty * $line->qty / ($tmpbom->qty>1?$tmpbom->qty:1);
		} elseif ($sub_bom_product->pmp > 0) {	// PMP if cost price isn't defined
			print '<td class="linecolcost nowrap right" id="sub_bom_cost_'.$sub_bom_line->id.'">';
			print '<span class="amount">'.price(price2num($sub_bom_product->pmp * $sub_bom_line->qty * $line->qty / (($tmpbom->qty>1?$tmpbom->qty:1)*$sub_bom_line->efficiency), 'MT')).'</span></td>';
			$total_cost.= $sub_bom_product->pmp * $sub_bom_line->qty * $line->qty / (($sub_bom->qty > 1 ? $sub_bom_product->qty : 1) * $sub_bom_line->efficiency);
			...
			if ($resql_supplier_price) {
				...
				if (!empty($obj->qty) && !empty($sub_bom_line->qty) && !empty($line->qty)) {
					$line_cost = $obj->min_price/$obj->qty * $sub_bom_line->qty * $line->qty / ($tmpbom->qty>1?$tmpbom->qty:1);
				} else {
