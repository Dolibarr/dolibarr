	...
	if ($object->id > 0)
	{
		...
		if ($object->id > 0)
		{
			...
			if ($action == 'delete')
			{
				...
				print $form->formconfirm($_SERVER['PHP_SELF'].'?id='.$object->id.'&expid='.$expedition_id.'&backtourl='.urlencode($backtourl),$langs->trans("DeleteDeliveryReceipt"),$langs->trans("DeleteDeliveryReceiptConfirm",$object->ref),'confirm_delete','','',1);
			...
			while ($i < $num_prod)
			{
				...
				if ($object->lines[$i]->fk_product > 0)
				{
					...
					$text = '<a href="'.DOL_URL_ROOT.'/product/fiche.php?id='.$object->lines[$i]->fk_product.'">';
			...
			if ($user->societe_id == 0)
			{
				...
				if ($user->rights->expedition->livraison->supprimer)
				{
					...
					{
						print '<a class="butActionDelete" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;expid='.$object->origin_id.'&amp;action=delete&amp;backtourl='.urlencode(DOL_URL_ROOT.'/expedition/fiche.php?id='.$object->origin_id).'">'.$langs->trans("Delete").'</a>';
					}
					else
					{
