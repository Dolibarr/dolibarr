...
if (in_array($mysoc->country_code, array('FR'))) {
	...
	if (!isRegistrationDataSavedAndPushed() || $origin == 'initmodule') {
		...
		if ($mysoc->country_code === 'FR') {
			$htmltext .= '<span class="small">'.$langs->trans("UnalterableLogTool1FR").'<br>';
			$htmltext .= $langs->trans("UnalterableLogTool2FR", $langs->transnoentitiesnoconv("Bank"), $langs->transnoentitiesnoconv("CashControl")).'</span>';
		}
