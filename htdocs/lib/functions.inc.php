...
// Pour compatibilit矬ors de l'upgrade
...
/**
   \brief      Renvoi vrai si l'email a un nom de domaine qui r貯ud via dns
...
/**
	\brief      Envoi des messages dolibarr dans un fichier ou dans syslog
				Pour fichier:   fichier d襩ni par SYSLOG_FILE
				Pour syslog:    facility d襩ni par SYSLOG_FACILITY
	\param      message		    Message a tracer. Ne doit pas etre traduit si level = LOG_ERR
	\param      level           Niveau de l'erreur
	\remarks	Cette fonction n'a un effet que si le module syslog est activ笍
				Warning, les fonctions syslog sont buggu豠sous Windows et g诩rent des
				fautes de protection m謯ire. Pour r貯udre, utiliser le loggage fichier,
				au lieu du loggage syslog (configuration du module).
				Si SYSLOG_FILE_NO_ERROR d襩ni, on ne g豥 pas erreur ecriture log
...
function dolibarr_syslog($message, $level=LOG_INFO)
{
	...
	if (isset($conf->syslog->enabled) && $conf->syslog->enabled)
	{
		...
		{
			//define_syslog_variables(); d詡 d襩nit dans master.inc.php
...
/**
		\brief      R袵p豥 une constante depuis la base de donn褳.
		\see        dolibarr_del_const, dolibarr_set_const
		\param	    db          Handler d'acc豠base
...
/**
   \brief      Insertion d'une constante dans la base de donn褳.
   \see        dolibarr_del_const, dolibarr_get_const
   \param	    db          Handler d'acc豠base
   \param	    name		Nom de la constante
   \param	    value		Valeur de la constante
   \param	    type		Type de constante (chaine par d襡ut)
   \param	    visible	    La constante est elle visible (0 par d襡ut)
...
/**
	\brief		Effacement d'une constante dans la base de donn褳
...
/**
   ...
   \param	    url         Si defini, on sauve parametre du tableau tab dont cl瞽 (url avec sortfield, sortorder, begin et page)
   Si non defini on sauve tous parametres du tableau tab
   \param	    tab         Tableau (cl绾valeur) des param賲es a sauvegarder
   \return     int         <0 si ko, >0 si ok
*/
function dolibarr_set_user_page_param($db, &$user, $url='', $tab)
{
    ...
    // On efface anciens param賲es pour toutes les cl矤ans $tab
    ...
    foreach ($tab as $key => $value)
    {
        // On positionne nouveaux param賲es
...
/**
		\brief      Effectue un d袡lage de date par rapport a une dur裍
		\param	    time                Date timestamp ou au format YYYY-MM-DD
		\param	    duration_value      Valeur de la dur裠a ajouter
		\param	    duration_unit       Unit矤e la dur裠a ajouter (d, m, y)
...
/**
		...
		\return     string      Date format裠ou '' si time null
*/
function dolibarr_print_date($time,$format='')
{
    global $conf;

    // Si format non d襩ni, on prend $conf->format_date_text_short sinon %Y-%m-%d %H:%M:%S
...
/**
		\brief  	Retourne une date fabriqu裠depuis une chaine
		\param		string			Date format裠en chaine
									YYYYMMDD
									YYYYMMDDHHMMSS
...
/**
		\brief  	Retourne une date fabriqu裠depuis infos.
					Remplace la fonction mktime non impl謥nt裠sous Windows si ann裠< 1970
		\param		hour			Heure
		\param		minute			Minute
...
function dolibarr_mktime($hour,$minute,$second,$month,$day,$year,$gm=0)
{
	...
	if ($usealternatemethod || $gm)	// Si time gm, seule adodb peut convertir
	{
		...
		// strtotime is ok for range: Vendredi 13 D袥mbre 1901 20:45:54 GMT au Mardi 19 Janvier 2038 03:14:07 GMT.
...
/**
        \brief      Formatage des num豯s de telephone en fonction du format d'un pays
        \param	    phone			Num豯 de telephone a formater
        \param	    country			Pays selon lequel formatter
        \return     string			Num豯 de t譩phone format狊*/
...
/**
        \brief      Tronque une chaine a une taille donn裠en ajoutant les points de suspension si cela d诡sse
...
/**
        \brief      Compl賥 une chaine a une taille donn裠par des espaces
        \param      string		Chaine a compl賥r
        \param      size		Longueur de la chaine.
        \param      side		0=Compl賩on a droite, 1=Compl賩on a gauche
        \param		char		Chaine de compl賩on
        \return     string		Chaine compl赩e
*/
...
/**
        \brief      Affiche picto propre a une notion/module (fonction g诩rique)
...
/**
        \brief      Affiche picto (fonction g诩rique)
        \param      alt         Texte sur le alt de l'image
        \param      picto       Nom de l'image a afficher (Si pas d'extension, on met '.png')
        \param		options		Attribut suppl謥ntaire a la balise img
		\return     string      Retourne tag img
*/
...
/**
        \brief      Affiche logo t譩phone out
        \param      alt         Texte sur le alt de l'image
        \return     string      Retourne tag img
*/
...
function dol_loginfunction($notused,$pearstatus)
{
	...
	{
		// Cas qui ne devrait pas arriver (pour compatibilit种
	...
	// Code de s袵rit狊	if ($conf->global->MAIN_SECURITY_ENABLECAPTCHA)
	{
...
/**
		\brief      Affiche message erreur system avec toutes les informations pour faciliter le diagnostic et la remont裠des bugs.
                    On doit appeler cette fonction quand une erreur technique bloquante est rencontr裮
                    Toutefois, il faut essayer de ne l'appeler qu'au sein de pages php, les classes devant
                    renvoyer leur erreur par l'interm裩aire de leur propri赩 "error".
        \param      db      Handler de base utilis狊        \param      error	Chaine erreur ou tableau de chaines erreur compl謥ntaires a afficher
*/
...
/**
		\brief  Deplacer les fichiers telecharg豬 apres quelques controles divers
...
function doliMoveFileUpload($src_file, $dest_file)
{
	...
	// Security:
	// On interdit les remont褳 de repertoire ainsi que les pipe dans 
...
function transcoS2L($zonein,$devise)
{
  ...
  // d衵t de la fonction de transcodification de somme en toutes lettres

  /*  $zonein = "123,56";
   *  $devise = "E"; // pr袩ser F si francs , sinon ce sera de l'euro
   *  $r = transcoS2L($zonein,$devise); // appeler la fonction
   *  echo "r貵ltat   vaut $r<br>";
   *  $zonelettresM =  strtoupper($r); // si vous voulez la m謥 zone mais tout en majuscules
   *  echo "r貵ltat en Majuscules  vaut $zonelettresM<br>";
   *  $zonein = "1,01";
  ...
  $arr1_99 = array("z豯","un","deux","trois",
  ...
  // echo "Pour une valeur en entr裠= $zonein<br>"; //pour ceux qui ne croient que ce qu'ils voient !
  // quelques petits controles s'imposent !!
  $valid = "[a-zA-Z\&\蚢\'\(\-\蛟\蝩\)\=\;\:\!\*\$\^\<\>]";
  if (ereg($valid,$zonein))
    {
      $r = "<b>la cha譥 ".$zonein." n'est pas valide</b>";
      return($r);
    }
  $zone = explode(" ",$zonein); // supprimer les blancs s诡rateurs
  $zonein = implode("",$zone); // reconcat譥 la zone input
  $zone = explode(".",$zonein); // supprimer les points s诡rateurs
  $zonein = implode("",$zone); // reconcat譥 la zone input, 蟠c'est fort ! merci PHP
  ...
  if ($virg == 0) { // ya pas de virgule
    if ($i > 7)
      {
	$r = "<b>la cha譥 ".$zonein." est trop longue (maxi = 9 millions)</b>";
	return($r);
      }
    $deb = 7 - $i;
    ...
    { //ya une virgule
      $ti = explode(",",$zonein); // mettre de c赩 ce qu'il y a devant la virgule
      $i = strlen($ti[0]); // en controler la longueur
      $zonechiffres = "0000000,00";
      if ($i > 7)
	{
	  $r = "<b>la cha譥 ".$zonein." est trop longue (maxi = 9 millions,00)</b>";
	  return($r);
	}
      $deb = 7 - $i;
  ...
  if (substr($zoneanaly,2,1)==0){ $MM = substr($zoneanaly,3,1);} // enlever le z豯 des milliers cause indexation
  if ($MM ==0 && $CM > 0)
    {
      $r = $r."mille ";
    }
  ...
  if (substr($zoneanaly,5,1)==0){ $C2 = substr($zoneanaly,6,1);} // enlever le z豯 des centaines cause indexation
  ...
  if ($UN != "00")
    {
      $cts = $UN;
      if (substr($UN,0,1)==0){ $cts = substr($UN,1,1);} // enlever le z豯 des centimes cause indexation
      $r = $r." et ". $arr1_99[$cts].$cent_singulier;
      if ($UN > 1) $r =$r."s"; // accorde au pluriel
    }
  $r1 = ltrim($r); // enleve quelques blancs possibles en d衵t de zone
  $r = ucfirst($r1); // met le 1er caract豥 en Majuscule, c'est + zoli
  return($r); // retourne le r貵ltat
...
/**
		...
		\param      sortfield   nom du champ sur lequel est effectu矬e tri du tableau
...
function print_liste_field_titre($name, $file, $field, $begin="", $options="", $td="", $sortfield="", $sortorder="")
{
    ...
    // Le champ de tri est mis en 赩dence.
...
/**
		\brief  Affichage d'un titre d'une fiche, align矡 gauche
		\param	titre			Le titre a afficher
		\param	mesg			Message supl謥ntaire a afficher a droite
...
/**
		\brief  	Effacement d'un r该rtoire
		\param		file			R该rtoire a effacer
*/
...
/**
		\brief  	Effacement d'un r该rtoire $dir et de son arborescence
		\param		file			R该rtoire a effacer
		\param		count			Compteur pour comptage nb elements supprim豍
		\return		int				Nombre de fichier+rep豴oires supprim豍
*/
...
/**
		...
		\return	 malware	Nom du virus si infect石inon retourne "null"
*/
...
/**
		...
		\param	num             nombre d'譩ment total
*/
...
/**
*		\brief      Fonction qui retourne un taux de tva format矰our visualisation
*		\remarks    Fonction utilis裠dans les pdf et les pages html
*		\param	    rate			Taux a formater (19.6 19,6 19.6% 19,6%...)
*		\return		string			Chaine avec montant format瞨19,6 ou 19,6%)
*/
...
/**
*		\brief      Fonction qui retourne un montant mon賡ire format矰our visualisation
*		\remarks    Fonction utilis裠dans les pdf et les pages html
*		\param	    amount			Montant a formater
*		\param	    html			Formatage html ou pas (0 par defaut)
*		\param	    outlangs		Objet langs pour formatage text
*		\param		trunc			1=Tronque affichage si trop de d袩males,0=Force le non troncage
*		\param		nbdecimal		Nbre decimals minimum.
*		\return		string			Chaine avec montant format狊*		\seealso	price2num		Fonction inverse de price
*/
function price($amount, $html=0, $outlangs='', $trunc=1, $nbdecimal=2)
{
	...
	$datas = split('\.',$amount);
	$decpart = $datas[1];
	$decpart = eregi_replace('0+$','',$decpart);	// Supprime les 0 de fin de partie d袩male
	//print "decpart=".$decpart."<br>";
	$end='';

	// On augmente au besoin si il y a plus de 2 d袩males
	...
	if ($trunc && $nbdecimal > $conf->global->MAIN_MAX_DECIMALS_SHOWN) 
	{
		...
		{
			// Si un affichage est tronqu窠on montre des ...
			$end='...';
		}
	}
...
/**
	\brief     		Fonction qui retourne un num豩que conforme PHP et SQL, depuis un montant au
					format utilisateur.
	...
	\param	    	rounding	'MU'=Round to Max unit price (MAIN_MAX_DECIMALS_UNIT)
								...
								''=No rounding
	\return			string		Montant au format num豩que PHP et SQL (Exemple: '99.99999')
...
/**
   ...
   \remarks    	Si vendeur non assujeti a TVA, TVA par d襡ut=0. Fin de r覬e.
				Si le (pays vendeur = pays acheteur) alors TVA par d襡ut=TVA du produit vendu. Fin de r覬e.
				Si (vendeur et acheteur dans Communaut知urop褮ne) et (bien vendu = moyen de transports neuf comme auto, bateau, avion) alors TVA par d襡ut=0 (La TVA doit 賲e pay矰ar acheteur au centre d'impots de son pays et non au vendeur). Fin de r覬e.
				Si (vendeur et acheteur dans Communaut知urop褮ne) et (acheteur = particulier ou entreprise sans num TVA intra) alors TVA par d襡ut=TVA du produit vendu. Fin de r覬e.
				Si (vendeur et acheteur dans Communaut知urop褮ne) et (acheteur = entreprise avec num TVA) intra alors TVA par d襡ut=0. Fin de r覬e.
				Sinon TVA propos裠par d襡ut=0. Fin de r覬e.
   \param      	societe_vendeuse    	Objet soci赩 vendeuse
   \param      	societe_acheteuse   	Objet soci赩 acheteuse
   \param      	taux_produit        	Taux par defaut du produit vendu
   \return     	float               	Taux de tva a appliquer, -1 si ne peut etre d賥rmin狊 */
function get_default_tva($societe_vendeuse, $societe_acheteuse, $taux_produit)
{
	...
	// Si le (pays vendeur = pays acheteur) alors la TVA par d襡ut=TVA du produit vendu. Fin de r覬e.
	...
	// Le test ci-dessus ne devrait pas etre necessaire. Me signaler l'exemple du cas juridique concercn石i le test suivant n'est pas suffisant.
	if ($societe_vendeuse->pays_id == $societe_acheteuse->pays_id)
	{
		if (strlen($taux_produit) == 0) return -1;	// Si taux produit = '', on ne peut d賥rminer taux tva
	    return $taux_produit;
	}

	// Si (vendeur et acheteur dans Communaut知urop褮ne) et (bien vendu = moyen de transports neuf comme auto, bateau, avion) alors TVA par d襡ut=0 (La TVA doit 賲e pay矰ar l'acheteur au centre d'impots de son pays et non au vendeur). Fin de r覬e.
	// Non g賩
	...
	// Sinon la TVA propos裠par d襡ut=0. Fin de r覬e.
	// Rem: Cela signifie qu'au moins un des 2 est hors Communaut知urop褮ne et que le pays diff豥
    return 0;
}
...
function makesalt($type=CRYPT_SALT_LENGTH)
{
  ...
  switch($type)
    ...
    case 8:		// 8 + 4 (Pour compatibilite, ne devrait pas etre utilis种
      $saltlen=8; $saltprefix='$1$'; $saltsuffix='$'; break;
    case 2:		// 2
...
/**
   \brief      Renvoi un chemin de classement r该rtoire en fonction d'un id
   \remarks    Examples: 1->"0/0/1/", 15->"0/1/5/"
   \param      $num        	Id a d袯mposer
...
/**
   \brief      Cr蠴ion de r该rtoire recursive
   \param      $dir        R该rtoire a cr褲
   \return     int         < 0 si erreur, >= 0 si succ豍
*/
function create_exdir($dir)
{
    ...
    for ($i = 0 ; $i < sizeof($cdir) ; $i++)
    {
		...
		// Attention, le is_dir() peut 袨ouer bien que le rep existe.
        ...
        if ($ccdir)
        {
		  ...
		  if (! @mkdir($ccdir, 0755))
		    {
		      // Si le is_dir a renvoy矵ne fausse info, alors on passe ici.
		  ...
		  else
		    {
		      ...
		      $nberr=0;	// On remet a z豯 car si on arrive ici, cela veut dire que les 袨ecs pr褩dents peuvent etre ignor豍
		      $nbcreated++;
		    }
		}
		else
...
/**
   \brief   Retourne le num豯 de la semaine par rapport a une date
   \param   time   	Date au format 'timestamp'
   \return  int		Num豯 de semaine
*/
function numero_semaine($time)
{
    ...
    /*
     * Norme ISO-8601:
     * - La semaine 1 de toute ann裠est celle qui contient le 4 janvier ou que la semaine 1 de toute ann裠est celle qui contient le 1er jeudi de janvier.
     * - La majorit矤es ann褳 ont 52 semaines mais les ann褳 qui commence un jeudi et les ann褳 bissextiles commen蠮t un mercredi en poss裥 53.
     * - Le 1er jour de la semaine est le Lundi
     */

    // D襩nition du Jeudi de la semaine
    ...
    // D襩nition du premier Jeudi de l'ann裍
    ...
    // D襩nition du num豯 de semaine: nb de jours entre "premier Jeudi de l'ann裢 et "Jeudi de la semaine";
    $numeroSemaine =     (
                    (
    ...
    if ($numeroSemaine==53)
    {
        // Les ann褳 qui commence un Jeudi et les ann褳 bissextiles commen蠮t un Mercredi en poss裥 53
...
/**
   ...
   \return  string	            Unite
   \todo    gerer les autres unit豠de mesure comme la livre, le gallon, le litre, ...
*/
function measuring_units_string($unit,$measuring_style='')
{
  /* Note Rodo aux dev :)
   * Ne pas ins豥r dans la base de donn褳 ces valeurs
   * cela surchagerait inutilement d'une requete suppl謥ntaire
...
/**
   \brief   Convertir du binaire en h跡d袩mal
   \param   string      bin
   \return  string	    x
*/
...
/**
   \brief   Convertir de l'h跡d袩mal en binaire
   \param   string      hexa
   \return  string	    bin
*/
...
/*
*	\brief		Cette fonction est appel裠pour coder ou non une chaine en html.
...
*					writeHTMLCell -> a besoin d'etre encod知n HTML
*					MultiCell -> ne doit pas etre encod知n HTML
*/
...
/**
   \brief   Encode\decode le mot de passe de la base de donn褳 dans le fichier de conf
   \param   level    niveau d'encodage : 0 non encod窠1 encod狊*/
...
/**
   \brief   Encode une chaine de caract豥
   \param   chain    chaine de caract豥s a encoder
   \return  string_coded  chaine de caract豥s encod裍
*/
...
/**
   \brief   Decode une chaine de caract豥
   \param   chain    chaine de caract豥s a decoder
   \return  string_coded  chaine de caract豥s decod裍
*/
...
/**
   ...
   \param	    timestampStart      Timestamp de d衵t
   \param	    timestampEnd        Timestamp de fin
...
/**
   \brief     Fonction retournant le nombre de jour entre deux dates sans les jours f豩豠(jours ouvr豩
   \param	    timestampStart      Timestamp de d衵t
   \param	    timestampEnd        Timestamp de fin
...
/**
   \brief     Fonction retournant le nombre de lignes dans un texte format狊   \param	    texte      Texte
...
/**
   \brief     R袵p豥 la valeur d'un champ, effectue un traitement Ajax et affiche le r貵ltat
   \param	  htmlname            nom et id du champ
   \param     keysearch           nom et id compl謥ntaire du champ de collecte
   \param	  url                 chemin du fichier de r误nse : /chemin/fichier.php
   \param     option              champ suppl謥ntaire de recherche dans les param賲es
...
/*
 *    \brief      Effectue les substitutions des mots cl豠par les donn褳 en fonction du tableau
 *    \param      chaine      			Chaine dans laquelle faire les substitutions
 *    \param      substitutionarray		Tableau cl石ubstitution => valeur a mettre
 *    \return     string      			Chaine avec les substitutions effectu褳
 */
 ...
 */
 function php2js($var)
 {
  ...
  // autres cas: objets, on ne les g豥 pas
  $result = FALSE;
  return $result;
}

/*
 *    \brief      Formate l'affichage de date de d衵t et de fin
 *    \param      date_start    date de d衵t
...
/*
 *    \brief     Cr蠴ion de 2 vignettes a partir d'un fichier image (une small et un mini)
 *    \brief     Les extension prise en compte sont jpg et png
 *    \param     file           Chemin du fichier image a redimensionner
 *    \param     maxWidth       Largeur maximum que dois faire la miniature (160 par d襡ut)
 *    \param     maxHeight      Hauteur maximum que dois faire l'image (120 par d襡ut)
 *    \param     extName        Extension pour diff豥ncier le nom de la vignette
 *    \param     quality        Qualit矤e compression (0=worst, 100=best)
...
function vignette($file, $maxWidth = 160, $maxHeight = 120, $extName='_small', $quality=50)
{
	...
	// V豩fication des param賲es
	if (! $file)
	{
		// Si le fichier n'a pas 赩 indiqu狊		return 'Bad parameter file';
	}
	elseif (! file_exists($file))
	{
		// Si le fichier pass知n param賲e n'existe pas
	...
	elseif(!is_numeric($maxWidth) || empty($maxWidth) || $maxWidth < 0){
		// Si la largeur max est incorrecte (n'est pas num豩que, est vide, ou est inf豩eure a 0)
		return 'Valeur de la largeur incorrecte.';
	}
	elseif(!is_numeric($maxHeight) || empty($maxHeight) || $maxHeight < 0){
		// Si la hauteur max est incorrecte (n'est pas num豩que, est vide, ou est inf豩eure a 0)
	...
	$infoImg = getimagesize($fichier); // R袵p象tion des infos de l'image
	$imgWidth = $infoImg[0]; // Largeur de l'image
	$imgHeight = $infoImg[1]; // Hauteur de l'image

	// Si l'image est plus petite que la largeur et la hauteur max, on ne cr裠pas de vignette
	if ($infoImg[0] < $maxWidth && $infoImg[1] < $maxHeight)
	{
		// On cree toujours les vignettes
		dolibarr_syslog("File size is smaller than thumb size",LOG_DEBUG);
		//return 'Le fichier '.$file.' ne n袥ssite pas de cr蠴ion de vignette';
	}
	...
	// On cr裠le r该rtoire contenant les vignettes
	...
	// Initialisation des dimensions de la vignette si elles sont sup豩eures a l'original
	...
	if($whFact < $imgWhFact){
		// Si largeur d賥rminante
		$thumbWidth  = $maxWidth;
		$thumbHeight = $thumbWidth / $imgWhFact;
	} else {
		// Si hauteur d賥rminante
	...
	{
		case 1:	// Gif
			$trans_colour = imagecolorallocate($imgThumb, 255, 255, 255); // On proc裥 autrement pour le format GIF
		...
		case 2:	// Jpg
			...
			break;
		case 3:	// Png
			imagealphablending($imgThumb,false); // Pour compatibilit石ur certain syst謥
	...
	//imagecopyresized($imgThumb, $img, 0, 0, 0, 0, $thumbWidth, $thumbHeight, $imgWidth, $imgHeight); // Ins豥 l'image de base redimensionn裍
	imagecopyresampled($imgThumb, $img, 0, 0, 0, 0, $thumbWidth, $thumbHeight, $imgWidth, $imgHeight); // Ins豥 l'image de base redimensionn裍
 ...
 *    \param      max_rows            Nombre max de lignes a afficher (0 = illimit秠 	
 */
 ...
 *    \param      max_rows            Nombre max de lignes a afficher (0 = illimit种
 *    \param      max_cols            Nombre max de colonnes a afficher (0 = illimit秠 	
 */
...
/**
   \brief   G诩rateur de codes barres
   \param   $code			  Valeur num豩que a coder
   \param   $encoding		Mode de codage
   \param   $generator  G诩rateur utilis瞨1=php-barcode, 2=pi_barcode)
...
/**
   \brief   Retourne un tableau des mois ou le mois s諥ctionn狊   \param   Mois s諥ctionn狊   \return  month
*/
