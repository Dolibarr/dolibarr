...
*		\return		string			Chaine avec montant format� (19,6 ou 19,6%)
*/
...
/**
	\brief     		Fonction qui retourne un num�rique conforme PHP et SQL, depuis un montant au
					format utilisateur.
	\remarks   		Fonction � appeler sur montants saisis avant un insert en base
	\param	    	amount		Montant a formater
...
/**
   ...
   \return     	float               	Taux de tva � appliquer, -1 si ne peut etre d�termin�
 */
...
/*
 *    \brief     Cr�ation de 2 vignettes � partir d'un fichier image (une small et un mini)
 *    \brief     Les extension prise en compte sont jpg et png
 *    \param     file           Chemin du fichier image � redimensionner
 ...
 *    \param     extName        Extension pour diff�rencier le nom de la vignette
 *    \param     quality        Qualit� de compression (0=worst, 100=best)
...
function vignette($file, $maxWidth = 160, $maxHeight = 120, $extName='_small', $quality=50)
{
	...
	if (! $file)
	{
		// Si le fichier n'a pas �t� indiqu�
	...
	elseif(!is_numeric($maxWidth) || empty($maxWidth) || $maxWidth < 0){
		// Si la largeur max est incorrecte (n'est pas num�rique, est vide, ou est inf�rieure � 0)
		return 'Valeur de la largeur incorrecte.';
	}
	elseif(!is_numeric($maxHeight) || empty($maxHeight) || $maxHeight < 0){
		// Si la hauteur max est incorrecte (n'est pas num�rique, est vide, ou est inf�rieure � 0)
	...
	// Si l'image est plus petite que la largeur et la hauteur max, on ne cr�e pas de vignette
	...
	// Initialisation des dimensions de la vignette si elles sont sup�rieures � l'original
	...
	{
		case 1:	// Gif
			$trans_colour = imagecolorallocate($imgThumb, 255, 255, 255); // On proc�de autrement pour le format GIF
		...
		case 2:	// Jpg
			...
			break;
		case 3:	// Png
			imagealphablending($imgThumb,false); // Pour compatibilit� sur certain syst�me
	...
	imagecopyresampled($imgThumb, $img, 0, 0, 0, 0, $thumbWidth, $thumbHeight, $imgWidth, $imgHeight); // Ins�re l'image de base redimensionn�e
 ...
 *    \param      file_to_include     Fichier CSV � afficher
 *    \param      max_rows            Nombre max de lignes � afficher (0 = illimit�)  	
 */
 ...
 *    \param      file_to_include     Fichier Excel � afficher
 *    \param      max_rows            Nombre max de lignes � afficher (0 = illimit�)
 *    \param      max_cols            Nombre max de colonnes � afficher (0 = illimit�)  	
 */
...
/**
   \brief   G�n�rateur de codes barres
   \param   $code			  Valeur num�rique � coder
   \param   $encoding		Mode de codage
   \param   $generator  G�n�rateur utilis� (1=php-barcode, 2=pi_barcode)
