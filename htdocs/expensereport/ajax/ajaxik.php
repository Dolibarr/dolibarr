...
if (empty($fk_expense) || $fk_expense < 0) {
	...
	if ($expense->fetch($fk_expense) <= 0) {
		$rep->errorMessage =  $langs->transnoentitiesnoconv('ErrorRecordNotFound', $fk_expense);
		$rep->response_status = 'error';
	} else {
		...
		if ($userauthor->fetch($expense->fk_user_author) <= 0) {
			...
			if ($result) {
				...
				if ($result < 0) {
					$rep->error = $result;
					$rep->errorMessage = $langs->trans('errorComputeTtcOnMileageExpense') . ' : '.$expense->errorsToString();
					$rep->response_status = 'error';
				} else {
					$rep->data = $result;
