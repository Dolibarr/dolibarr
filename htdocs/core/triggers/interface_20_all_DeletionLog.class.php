...
class InterfaceDeletionLog extends DolibarrTriggers
{
	...
	public function runTrigger($action, $object, User $user, Translate $langs, Conf $conf)
	{
		...
		$newid = $deletionlog->add($object->element, $object->id, $user, $entity);
		if ($newid < 0) {
		...
		// Keep the table bounded without a scheduled job: one insert in a hundred,
		// picked on the new row id, also drops the rows past the retention delay.
		if ($newid > 0 && ($newid % 100) === 0) {
			$retentiondays = getDolGlobalInt('MAIN_DELETION_LOG_RETENTION_DAYS', DeletionLog::DEFAULT_RETENTION_DAYS);
			$deletionlog->purge($retentiondays, 0);
		}
