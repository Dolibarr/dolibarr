	...
	if ($issignatureok) {
		...
		if ($mode == "propale" || $mode == 'proposal') {
			...
			if (!$error) {
				...
				$user = new User($db);
				$object->setSignedStatus($user, CommonSignedObject::$SIGNED_STATUSES['STATUS_SIGNED_RECEIVER_ONLINE'], 0, 'CONTRACT_MODIFY');
			}
			...
			if (!$error) {
				...
				$user = new User($db);
				$object->setSignedStatus($user, CommonSignedObject::$SIGNED_STATUSES['STATUS_SIGNED_RECEIVER_ONLINE'], 0, 'FICHINTER_MODIFY');
			}
