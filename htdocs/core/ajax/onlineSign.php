				...
				if (!$error) {
					...
					if (preg_match('/\.pdf/i', $last_main_doc_file)) {
						...
						if ($last_modelpdf == 'sepamandate') {
							...
							// Fallback for setups using a non-default bank PDF model (eg. "ban"): take the
							// last generated main document as source and append "_signed-<date>" before
							// the extension. Without this, $sourcefile stays empty, the dol_is_file()
							// check below fails silently, no signed PDF is ever built, and the public
							// download link keeps pointing at the unsigned original.
