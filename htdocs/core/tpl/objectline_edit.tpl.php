...
if (getDolGlobalString('MAIN_VIEW_LINE_NUMBER')) {
	print '<td class="linecolnum center">';
	print ($i + 1) . '</td>';
	$coldisplay++;
}
...
print '<td class="linecoldesc minwidth250onall">';
print '<div id="line_' . $line->id . '"></div>';

print '<input type="hidden" name="lineid" value="' . $line->id . '">';
print '<input type="hidden" id="product_type" name="type" value="' . $line->product_type . '">';
print '<input type="hidden" id="special_code" name="special_code" value="' . $line->special_code . '">';
print '<input type="hidden" id="fk_parent_line" name="fk_parent_line" value="' . $line->fk_parent_line . '">';
...
if ($object->element == 'supplier_proposal' || $object->element == 'order_supplier' || $object->element == 'invoice_supplier' || $object->element == 'invoice_supplier_rec') {	// We must have same test in printObjectLines
	print '<td class=" right linecolrefsupplier">';
	print '<input id="fourn_ref" name="fourn_ref" class="flat minwidth50 maxwidth100 maxwidth125onsmartphone" value="' . GETPOSTISSET('fourn_ref') ? GETPOST('fourn_ref') : ($line->ref_supplier ? $line->ref_supplier : $line->ref_fourn) . '">';
	print '</td>';
...
print '<td class="nowraponall right linecoldiscount">';
...
if (!empty($usemargins)) {
	if ($user->hasRight('margins', 'creer')) {
		$coldisplay++;
		print '<td class="margininfos right">';
		print '<!-- For predef product -->';
		if (isModEnabled("product") || isModEnabled("service")) {
			print '<select id="fournprice_predef" name="fournprice_predef" class="flat minwidth75imp right" style="display: none;"></select>';
		}
		print "<!-- For free product -->\n";
		print '<input class="flat maxwidth75 right" type="text" id="buying_price" name="buying_price" class="hideobject" value="' . (GETPOSTISSET('buying_price') ? GETPOST('buying_price') : price($line->pa_ht, 0, '', 0)) . '">';
		print '</td>';
	}
...
print '<!-- colspan for this td because it replace total_ht+3 td for buttons+... -->';
print '<td class="center valignmiddle" colspan="' . $colspan . '">';
$coldisplay += $colspan;
print '<input type="submit" class="reposition button buttongen marginbottomonly button-save" id="savelinebutton marginbottomonly" name="save" value="' . $langs->trans("Save") . '">';
print '<br>';
print '<input type="submit" class="reposition button buttongen marginbottomonly button-cancel" id="cancellinebutton" name="cancel" value="' . $langs->trans("Cancel") . '">';

print '</td>';
print '</tr>';

if (isModEnabled("service") && $line->product_type == 1 && $dateSelector) {
	print '<tr id="service_duration_area" class="treditedlinefordate">';
	if (getDolGlobalString('MAIN_VIEW_LINE_NUMBER')) {
		print '<td class="linecolnum center"></td>';
	}
	print '<td colspan="' . $coldisplay - (!getDolGlobalString('MAIN_VIEW_LINE_NUMBER') ? 0 : 1) . '">' . $langs->trans('ServiceLimitedDuration') . ' ' . $langs->trans('From') . ' ';
	...
	if (event.which != 9 && (event.which < 37 || event.which> 40) && jQuery("#price_ttc").val() != '') {
		...
		if (event.which != 9 && (event.which < 37 || event.which> 40) && jQuery("#price_ttc").val() != '') {
			...
			if (isModEnabled('margin')) {
				...
				jQuery("#tva_tx").click(function() {
				/* somtimes field is a text, sometimes a combo */
				jQuery("input[name='np_marginRate']:first").val('');
				jQuery("input[name='np_markRate']:first").val('');
				});
				jQuery("#tva_tx").keyup(function() {
				/* somtimes field is a text, sometimes a combo */
				jQuery("input[name='np_marginRate']:first").val('');
				jQuery("input[name='np_markRate']:first").val('');
				});
					...
					$(data).each(function() {
						...
						<?php if ($line->fk_fournprice > 0) { ?>
							if (this.id == <?php echo $line->fk_fournprice; ?>) {
								options += ' selected'; $("#buying_price").val(this.price); trouve=true; } <?php } ?> options +='>' + this.label + '</option>' ; }); options +='<option value=null' + (trouve ? '' : ' selected' ) + '><?php echo $langs->trans("InputPrice"); ?></option>' ; $("#fournprice").html(options); if (trouve) { $("#buying_price").hide(); $("#fournprice").show(); } else { $("#buying_price").show(); } $("#fournprice").change(function() { var selval=$(this).find('option:selected').attr("price"); if (selval) $("#buying_price").val(selval).hide(); else $('#buying_price').show(); }); } else { $("#fournprice").hide(); $('#buying_price').show(); } }, 'json' ); <?php
																																																																																																																																																																									}
																																																																																																																																																																										?> }); </script>
