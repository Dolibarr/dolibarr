...
?>
	...
	if (! empty($conf->product->enabled) || ! empty($conf->service->enabled))
	{
		...
		{
			$ajaxoptions=array(
					'update' => array('duration_unit' => 'duration_unit', 'seltype' => 'type', 'qty'=>'qty','remise_percent' => 'discount'),	// html id tags that will be edited with which ajax json response key
...
jQuery(document).ready(function() {
	...
	$("#seltype").change(function() {
		if ($('#seltype').val() == '<?php echo Product::TYPE_SERVICE ?>') {
			...
			if (duration_unit == 'd' || duration_unit == 'h') {
