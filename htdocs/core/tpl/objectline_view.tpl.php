...
if (getDolGlobalString('MAIN_VIEW_LINE_NUMBER')) {
	print '<td class="linecolnum center"><span class="opacitymedium">';
	print($i + 1);
	print '</span></td>';
	$coldisplay++;
}
...
if (isModEnabled("multicurrency") && $this->multicurrency_code != $conf->currency) {
	print '<td class="linecoluht_currency nowraponall right">';
	print price($sign * $line->multicurrency_subprice) . '</td>';
	$coldisplay++;
}

if (!empty($inputalsopricewithtax) && !getDolGlobalInt('MAIN_NO_INPUT_PRICE_WITH_TAX')) {
	print '<td class="linecoluttc nowraponall right">';
...
if ($usemargins && isModEnabled('margin') && empty($user->socid)) {
	if ($user->hasRight('margins', 'creer')) {
		print '<td class="linecolmargin1 nowrap margininfos right">';
		print price($line->pa_ht) . '</td>';
		$coldisplay++;
	}
	if (getDolGlobalString('DISPLAY_MARGIN_RATES') && $user->hasRight('margins', 'liretous')) {
		print '<td class="linecolmargin2 nowrap margininfos right">';
		print (($line->pa_ht == 0) ? 'n/a' : price(price2num($line->marge_tx, 'MT')) . '%') . '</td>';
		$coldisplay++;
	}
	if (getDolGlobalString('DISPLAY_MARK_RATES') && $user->hasRight('margins', 'liretous')) {
		print '<td class="linecolmargin2 nowrap margininfos right">';
		print price(price2num($line->marque_tx, 'MT')) . '%' . '</td>';
		$coldisplay++;
	}
}
...
if ($line->special_code == 3) {
	print '<td class="linecoloption nowrap right">';
	print $langs->trans('Option') . '</td>';
	$coldisplay++;
} else {
...
if ($this->statut == 0 && !empty($object_rights->creer) && $action != 'selectlines') {
	...
	if (($line->info_bits & 2) == 2 || !empty($disableedit)) {
		...
		print '<a class="editfielda reposition" href="' . $_SERVER["PHP_SELF"] . '?id=' . $this->id . '&action=editline&token=' . newToken() . '&lineid=' . $line->id . '">';
		print img_edit() . '</a>';
	}
	print '</td>';
...
if ($action == 'selectlines') {
	print '<td class="linecolcheck center"><input type="checkbox" class="linecheckbox" name="line_checkbox[' . ($i + 1) . ']" value="' . $line->id . '"></td>';
}
