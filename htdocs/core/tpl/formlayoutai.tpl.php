	...
	$out .= '<script>
						$(document).ready(function() {
							...
							$(document).on("click", function (event) {
								...
								if (!aidropdown.is(event.target) && !aidropdownbutton.is(event.target) && $(event.target).closest(aidropdown).length === 0 
								&& $(event.target).closest(aidropdownbutton).length === 0 && aidropdown.is(":visible")) {
