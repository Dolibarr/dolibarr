...
foreach ($object->fields as $key => $val)
{
	...
	elseif ($val['label'] == 'MitarbeiterIn') $value = $user->id;
	else $value = GETPOST($key, 'alphanohtml');
	if ($val['noteditable']) print $object->showOutputField($val, $key, $value, '', '', '', $val['css']);
	else print $object->showInputField($val, $key, $value, '', '', '', $val['css']);
	print '</td>';
	print '</tr>';
}
?>
