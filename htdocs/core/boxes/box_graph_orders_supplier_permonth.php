...
class box_graph_orders_supplier_permonth extends ModeleBoxes
{
	...
	function loadBox($max=5)
	{
		...
		if ($user->rights->fournisseur->commande->lire) {
			...
			if ($shownb) {
                $px1 = new DolChartJs();
                $graph_datas = $stats->getNbByMonthWithPrevYear($endyear,$startyear,(GETPOST('action','aZ09')==$refreshaction?-1:(3600*24)), ($width<80?2:0), 1, $px1);
            ...
            if ($showtot) {
                $px2 = new DolChartJs();
                $graph_datas = $stats->getAmountByMonthWithPrevYear($endyear,$startyear,(GETPOST('action','aZ09')==$refreshaction?-1:(3600*24)), ($width<80?2:0), 1, $px2);
                ...
                $px2->element('idboxgraphboxamount'.$this->boxcode)
                    ...
                    ->setOptions(array(
                        ...
                        'title' => array(
                            'display' => true,
                            'text' => $langs->transnoentitiesnoconv("AmountOfOrdersByMonthHT"),
                        )
                    )
                );
            }
