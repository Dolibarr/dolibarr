			...
			if ($sumMembers) {
				...
				foreach ($sumMembers as $key => $data) {
					...
					$this->info_box_contents[$line][] = array(
						...
						'text' => '<a href="'.DOL_MAIN_URL_ROOT.'/adherent/list.php?search_categ='.$adhtag->id.'&sortfield=d.datefin,t.subscription&sortorder=desc,desc&backtopage='.urlencode($_SERVER['PHP_SELF']).'">'.dol_trunc(($adhtag->ref ? $adhtag->ref : $adhtag->label), dol_size(32)).'</a>',
						'asis' => 1,
					);
					$this->info_box_contents[$line][] = array(
						'td' => 'class="right"',
