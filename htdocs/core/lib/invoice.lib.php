...
function getNumberInvoicesPieChart($mode)
{
	...
	if (($mode == 'customers' && isModEnabled('invoice') && $user->hasRight('facture', 'lire'))
		...
		$sql .= ", sum(".$db->ifsql("f.date_lim_reglement < '".date_format($datenowsub15, 'Y-m-d')."'", $amount_mode ? "f.total_ht" : 1, 0).") as late15";
		$sql .= ", sum(".$db->ifsql("f.date_lim_reglement < '".date_format($now, 'Y-m-d')."'", $amount_mode ? "f.total_ht" : 1, 0).") as latenow";
		$sql .= ", sum(".$db->ifsql("f.date_lim_reglement >= '".date_format($now, 'Y-m-d')."' OR f.date_lim_reglement IS NULL", $amount_mode ? "f.total_ht" : 1, 0).") as notlatenow";
		$sql .= ", sum(".$db->ifsql("f.date_lim_reglement > '".date_format($datenowadd15, 'Y-m-d')."'", $amount_mode ? "f.total_ht" : 1, 0).") as notlate15";
