...
function contact_prepare_head(Contact $object)
{
	...
	$head[$tab][0] = dolBuildUrl('/contact/card.php', ['id' => $object->id]);
	...
	if ((isModEnabled('ldap') && getDolGlobalString('LDAP_CONTACT_ACTIVE'))
		...
		$head[$tab][0] = dolBuildUrl('/contact/ldap.php', ['id' => $object->id]);
		$head[$tab][1] = $langs->trans("LDAPCard");
		$head[$tab][2] = 'ldap';
		$tab++;
	}

	$head[$tab][0] = dolBuildUrl('/contact/perso.php', ['id' => $object->id]);
	...
	if (isModEnabled('project') && $user->hasRight('project', 'lire')) {
		...
		$head[$tab][0] = dolBuildUrl('/contact/project.php', ['id' => $object->id]);
	...
	if (isModEnabled('order') || isModEnabled("propal") || isModEnabled('invoice') || isModEnabled('intervention') || isModEnabled("supplier_proposal") || isModEnabled("supplier_order") || isModEnabled("supplier_invoice")) {
		$head[$tab][0] = dolBuildUrl('/contact/consumption.php', ['id' => $object->id]);
	...
	if (!getDolGlobalString('MAIN_DISABLE_NOTES_TAB')) {
		...
		$head[$tab][0] = dolBuildUrl('/contact/note.php', ['id' => $object->id]);
	...
	$head[$tab][0] = dolBuildUrl('/contact/document.php', ['id' => $object->id]);
	...
	// Agenda / Events
	$head[$tab][0] = dolBuildUrl('/contact/agenda.php', ['id' => $object->id]);
