...
function contact_prepare_head(Contact $object)
{
	...
	// Related items
	if (!empty($conf->commande->enabled) || !empty($conf->propal->enabled) || isModEnabled('facture') || isModEnabled('ficheinter') || (!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled) || !empty($conf->supplier_invoice->enabled)) {
