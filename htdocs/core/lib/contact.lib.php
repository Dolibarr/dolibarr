...
function contact_prepare_head(Contact $object)
{
	...
	$head[$tab][0] = dol_buildurl('/contact/card.php', ['id' => $object->id]);
	...
	if ((isModEnabled('ldap') && getDolGlobalString('LDAP_CONTACT_ACTIVE'))
		...
		$head[$tab][0] = dol_buildurl('/contact/ldap.php', ['id' => $object->id]);
		$head[$tab][1] = $langs->trans("LDAPCard");
		$head[$tab][2] = 'ldap';
		$tab++;
	}

	$head[$tab][0] = dol_buildurl('/contact/perso.php', ['id' => $object->id]);
	...
	if (isModEnabled('project') && $user->hasRight('project', 'lire')) {
		...
		$head[$tab][0] = dol_buildurl('/contact/project.php', ['id' => $object->id]);
	...
	if (isModEnabled('order') || isModEnabled("propal") || isModEnabled('invoice') || isModEnabled('intervention') || isModEnabled("supplier_proposal") || isModEnabled("supplier_order") || isModEnabled("supplier_invoice")) {
		$head[$tab][0] = dol_buildurl('/contact/consumption.php', ['id' => $object->id]);
	...
	if (!getDolGlobalString('MAIN_DISABLE_NOTES_TAB')) {
		...
		$head[$tab][0] = dol_buildurl('/contact/note.php', ['id' => $object->id]);
	...
	$head[$tab][0] = dol_buildurl('/contact/document.php', ['id' => $object->id]);
	...
	// Agenda / Events
	$head[$tab][0] = dol_buildurl('/contact/agenda.php', ['id' => $object->id]);
	...
	// Log
	/*
	$head[$tab][0] = dol_buildurl('/contact/info.php', ['id' => $object->id]);
