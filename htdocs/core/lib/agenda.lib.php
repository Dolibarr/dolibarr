...
function show_array_actions_to_do($max = 5)
{
	...
	if ($resql) {
		...
		print '<th colspan="2" class="right"><a class="commonlink" href="'.dolBuildUrl('/comm/action/list.php', ['mode' => 'show_list', 'status' => 'todo']).'">'.$langs->trans("FullList").'</a></th>';
		print '</tr>';
...
function show_array_last_actions_done($max = 5)
{
	...
	if ($resql) {
		...
		print '<th colspan="2" class="right"><a class="commonlink" href="'.dolBuildUrl('/comm/action/list.php', ['mode'=> 'show_list', 'status' => 'done']).'">'.$langs->trans("FullList").'</a></th>';
		print '</tr>';
...
function agenda_prepare_head()
{
	...
	$head[$h][0] = dolBuildUrl("/admin/agenda_other.php");
	$head[$h][1] = $langs->trans("Miscellaneous");
	$head[$h][2] = 'other';
	$h++;

	$head[$h][0] = dolBuildUrl("/admin/agenda.php");
	$head[$h][1] = $langs->trans("AutoActions");
	$head[$h][2] = 'autoactions';
	$h++;

	$head[$h][0] = dolBuildUrl("/admin/agenda_reminder.php");
	$head[$h][1] = $langs->trans("Reminders");
	$head[$h][2] = 'reminders';
	$h++;

	$head[$h][0] = dolBuildUrl("/admin/agenda_xcal.php");
	$head[$h][1] = $langs->trans("ExportCal");
	$head[$h][2] = 'xcal';
	$h++;

	$head[$h][0] = dolBuildUrl("/admin/agenda_extsites.php");
	...
	$head[$h][0] = dolBuildUrl("/admin/agenda_extrafields.php");
...
function actions_prepare_head($object)
{
	...
	$head[$h][0] = dolBuildUrl('/comm/action/card.php', ['id'=> $object->id]);
	...
	if (isModEnabled('resource')) {
		...
		$head[$h][0] = dolBuildUrl('/resource/element_resource.php', ['element' => 'action', 'element_id'=> $object->id]);
	...
	$head[$h][0] = dolBuildUrl('/comm/action/document.php', ['id' => $object->id]);
	...
	$head[$h][0] = dolBuildUrl('/comm/action/info.php', ['id' => $object->id]);
...
function calendars_prepare_head($param)
{
	...
	$head[$h][0] = dolBuildUrl('/comm/action/list.php', $query);
	$head[$h][1] = $langs->trans("ViewList");
	$head[$h][2] = 'cardlist';
	$h++;

	$query['mode'] = 'show_month';
	$head[$h][0] = dolBuildUrl('/comm/action/index.php', $query);
	...
	if (getDolGlobalString('AGENDA_SHOW_PERTYPE')) {
		$head[$h][0] = dolBuildUrl('/comm/action/pertype.php', $query);
	...
	$head[$h][0] = dolBuildUrl('/comm/action/peruser.php', $query);
