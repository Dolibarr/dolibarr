...
function member_prepare_head(Adherent $object)
{
	...
	$head[$h][0] = dolBuildUrl('/adherents/card.php', ['rowid' => $object->id]);
	...
	if ((isModEnabled('ldap') && getDolGlobalString('LDAP_MEMBER_ACTIVE'))
		...
		$head[$h][0] = dolBuildUrl('/adherents/ldap.php', ['id' => $object->id]);
	...
	if ($user->hasRight('adherent', 'cotisation', 'lire')) {
		...
		$head[$h][0] = dolBuildUrl('/adherents/subscription.php', ['rowid' => $object->id]);
	...
	if (getDolGlobalString('PARTNERSHIP_IS_MANAGED_FOR') == 'member') {
		if ($user->hasRight('partnership', 'read')) {
			...
			$head[$h][0] = dolBuildUrl('/partnership/partnership_list.php', ['rowid' => $object->id]);
	...
	$head[$h][0] = dolBuildUrl('/adherents/note.php', ['id' => $object->id]);
	...
	$head[$h][0] = dolBuildUrl('/adherents/document.php', ['id' => $object->id]);
	...
	// Show agenda tab
	$head[$h][0] = dolBuildUrl('/adherents/agenda.php', ['id' => $object->id]);
...
function member_type_prepare_head(AdherentType $object)
{
	...
	$head[$h][0] = dolBuildUrl('/adherents/type.php', ['rowid' => $object->id]);
	...
	if (getDolGlobalInt('MAIN_MULTILANGS')) {
		$head[$h][0] = dolBuildUrl('/adherents/type_translation.php', ['rowid' => $object->id]);
	...
	if ((isModEnabled('ldap') && getDolGlobalString('LDAP_MEMBER_TYPE_ACTIVE'))
		...
		$head[$h][0] = dolBuildUrl('/adherents/type_ldap.php', ['rowid' => $object->id]);
		$head[$h][1] = $langs->trans("LDAPCard");
		$head[$h][2] = 'ldap';
		$h++;
	}
...
function member_admin_prepare_head()
{
	...
	$head[$h][0] = dolBuildUrl('/adherents/admin/member.php');
	$head[$h][1] = $langs->trans("Miscellaneous");
	$head[$h][2] = 'general';
	$h++;

	$head[$h][0] = dolBuildUrl('/adherents/admin/member_emails.php');
	...
	$head[$h][0] = dolBuildUrl('/adherents/admin/member_extrafields.php');
	...
	$head[$h][0] = dolBuildUrl('/adherents/admin/member_type_extrafields.php');
	...
	$head[$h][0] = dolBuildUrl('/adherents/admin/website.php');
...
function member_stats_prepare_head($object)
{
	...
	$head[$h][0] = dolBuildUrl('/adherents/stats/index.php');
	$head[$h][1] = $langs->trans("Subscriptions");
	$head[$h][2] = 'statssubscription';
	$h++;

	$head[$h][0] = dolBuildUrl('/adherents/stats/geo.php', ['mode' => 'memberbycountry']);
	$head[$h][1] = $langs->trans("Country");
	$head[$h][2] = 'statscountry';
	$h++;

	$head[$h][0] = dolBuildUrl('/adherents/stats/geo.php', ['mode' => 'memberbyregion']);
	$head[$h][1] = $langs->trans("Region");
	$head[$h][2] = 'statsregion';
	$h++;

	$head[$h][0] = dolBuildUrl('/adherents/stats/geo.php', ['mode' => 'memberbystate']);
	$head[$h][1] = $langs->trans("State");
	$head[$h][2] = 'statsstate';
	$h++;

	$head[$h][0] = dolBuildUrl('/adherents/stats/geo.php', ['mode' => 'memberbytown']);
	$head[$h][1] = $langs->trans('Town');
	$head[$h][2] = 'statstown';
	$h++;

	$head[$h][0] = dolBuildUrl('/adherents/stats/byproperties.php');
...
function subscription_prepare_head(Subscription $object)
{
	...
	$head[$h][0] = dolBuildUrl('/adherents/subscription/card.php', ['rowid' => $object->id]);
	$head[$h][1] = $langs->trans("Subscription");
	$head[$h][2] = 'general';
	$h++;

	$head[$h][0] = dolBuildUrl('/adherents/subscription/info.php', ['rowid' => $object->id]);
