...
function supplierorder_prepare_head(CommandeFournisseur $object)
{
	...
	if (!getDolGlobalString('MAIN_DISABLE_NOTES_TAB')) {
		...
		$head[$h][0] = DOL_URL_ROOT.'/supplier_order/note.php?id='.$object->id;
	...
	$head[$h][0] = DOL_URL_ROOT.'/supplier_order/document.php?id='.$object->id;
	...
	if (isModEnabled('agenda') && ($user->hasRight('agenda', 'myactions', 'read') || $user->hasRight('agenda', 'allactions', 'read'))) {
		...
		if (!is_null($dataretrieved)) {
			...
			$sql .= " AND elementtype = 'order_supplier'";
			$resql = $db->query($sql);
			if ($resql) {
...
function supplier_order_admin_prepare_head()
{
	...
	$head[$h][0] = dolBuildUrl(DOL_URL_ROOT."/supplier_order/admin/supplier_order.php");
	$head[$h][1] = $langs->trans("SupplierOrder");
	$head[$h][2] = 'order';
	$h++;
