 ...
 *
 *	@param	BookKeeping	$object		Book Keeping
 *	@return	array<array{0:string,1:string,2:string}>	Array of tabs to show
 */
function accountingTransactionPrepareHead(BookKeeping $object)
{
	...
	if (!getDolGlobalString('MAIN_DISABLE_NOTES_TAB')) {
		...
		$head[$h][0] = DOL_URL_ROOT.'/bookkeeping/note.php?id='.$object->id;
	...
	$head[$h][0] = DOL_URL_ROOT.'/bookkeeping/document.php?id='.$object->id;
	...
	$head[$h][0] = DOL_URL_ROOT.'/bookkeeping/agenda.php?id='.$object->id;
	...
	// $this->tabs = array('entity:+tabname:Title:@mymodule:/mymodule/mypage.php?id=__ID__'); to add new tab
	// $this->tabs = array('entity:-tabname); to remove a tab
	complete_head_from_modules($conf, $langs, $object, $head, $h, 'accounting_account_transaction');

	complete_head_from_modules($conf, $langs, $object, $head, $h, 'accounting_account_transaction', 'remove');
