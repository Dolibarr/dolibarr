...
function user_prepare_head(User $object)
{
	...
	$head[$h][0] = dolBuildUrl('/user/card.php', ['id' => $object->id]);
	...
	if ((isModEnabled('ldap') && getDolGlobalString('LDAP_SYNCHRO_ACTIVE'))
		...
		$langs->load("ldap");
		$head[$h][0] = dolBuildUrl('/user/ldap.php', ['id' => $object->id]);
	...
	if ($canreadperms) {
		$head[$h][0] = dolBuildUrl('/user/perms.php', ['id' => $object->id]);
	...
	$head[$h][0] = dolBuildUrl('/user/param_ihm.php', ['id' => $object->id]);
	...
	if (isModEnabled('agenda')) {
		...
		$head[$h][0] = dolBuildUrl('/user/agenda_extsites.php', ['id' => $object->id]);
	...
	if (isModEnabled('clicktodial')) {
		$head[$h][0] = dolBuildUrl('/user/clicktodial.php', ['id' => $object->id]);
	...
	if ($user->socid == 0 && isModEnabled('notification')) {
		...
		$langs->load("mails");
		$head[$h][0] = dolBuildUrl('/user/notify/card.php', ['id' => $object->id]);
	...
	if ((isModEnabled('salaries') && $user->hasRight('salaries', 'read'))
		...
		// Bank
		$head[$h][0] = dolBuildUrl('/user/bank.php', ['id' => $object->id]);
	...
	if (empty($user->socid)) {
		...
		$head[$h][0] = dolBuildUrl('/user/note.php', ['id' => $object->id]);
		...
		$head[$h][0] = dolBuildUrl('/user/document.php', ['userid' => $object->id]);
		...
		$head[$h][0] = dolBuildUrl('/user/agenda.php', ['id' => $object->id]);
...
function group_prepare_head($object)
{
	...
	$head[$h][0] = dolBuildUrl('/user/group/card.php', ['id' => $object->id]);
	...
	if ((isModEnabled('ldap') && getDolGlobalString('LDAP_SYNCHRO_ACTIVE'))
		&& (!getDolGlobalString('MAIN_DISABLE_LDAP_TAB') || !empty($user->admin))) {
		$langs->load("ldap");
		$head[$h][0] = dolBuildUrl('/user/group/ldap.php', ['id' => $object->id]);
		$head[$h][1] = $langs->trans("LDAPCard");
		$head[$h][2] = 'ldap';
		$h++;
	}
	...
	if ($canreadperms) {
		$head[$h][0] = dolBuildUrl('/user/group/perms.php', ['id' => $object->id]);
...
function user_admin_prepare_head()
{
	...
	$head[$h][0] = dolBuildUrl('/admin/user.php');
	$head[$h][1] = $langs->trans("Parameters");
	$head[$h][2] = 'card';
	$h++;

	$head[$h][0] = dolBuildUrl('/admin/usergroup.php');
	$head[$h][1] = $langs->trans("Group");
	$head[$h][2] = 'usergroupcard';
	$h++;

	$head[$h][0] = dolBuildUrl('/user/admin/user_extrafields.php');
	...
	$head[$h][0] = dolBuildUrl('/user/admin/group_extrafields.php');
			...
			if (is_resource($handle)) {
				while (($subdir = readdir($handle)) !== false) {
					if (is_dir($dirtheme."/".$subdir) && substr($subdir, 0, 1) != '.'
						...
						if (!file_exists($file)) {
							$url = dolBuildUrl('/public/theme/common/nophoto.png');
						}
