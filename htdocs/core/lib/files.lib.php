...
function dol_check_secure_access_document($modulepart, $original_file, $entity, $fuser = null, $refname = '', $mode = 'read')
{
	...
	} elseif ($modulepart == 'invoice' && !empty($conf->invoice->multidir_output[$entity])) {
	...
	} elseif ($modulepart == 'intervention' && !empty($conf->ficheinter->dir_output)) {
...
/**
 * Normalize a modulepart value for backward compatibility.
 *
 * Maps legacy and localized module part identifiers to the canonical
 * modulepart names used across the codebase. This function only
 * normalizes the string and does not perform permission checks or
 * path resolution.
 *
...
function normalizeModulepart($modulepart) {
	...
	$modulepart = strtolower($modulepart);
