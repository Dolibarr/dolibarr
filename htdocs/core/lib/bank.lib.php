...
/**
 * Calculating the reference of a structured communication
 *
 * @param	string	$ref		Invoice reference
 * @param	string	$type		Invoice type
 * @return	string
 */
function getStructuredCommunication($ref = '', $type = 0)
{
	if (!empty($ref)) {
		// Extract invoice number digits only
		...
		switch ($type) {
			case '0':
				...
				break;
			case '1':
				$type = '30'; // invoice type replacement
				break;
			case '2':
				$type = '40'; // invoice type credit note
				break;
			case '3':
				$type = '21'; // invoice type deposit
				break;
			case '5':
				$type = '20'; // invoice type situation, force to standard
				break;
			default:
				$type = '00';
		}
		...
		$ref = $ref . str_pad($controlKey, 2, '0', STR_PAD_LEFT);

		// Format reference as XXX/XXXX/XXXXX
		$part1 = '+++'.substr($ref, 0, 3);
		$part2 = substr($ref, 3, 4);
		$part3 = substr($ref, 7, 5).'+++'; // Includes last 3 digits + 2 check digits

		$ref = $part1 . '/' . $part2 . '/' . $part3;
		return $ref;
	} else {
		return "ErrorWrongParameters";
	}
}
