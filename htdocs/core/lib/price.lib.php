...
function calcul_price_total($qty, $pu, $remise_percent_ligne, $txtva, $uselocaltax1_rate, $uselocaltax2_rate, $remise_percent_global, $price_base_type, $info_bits, $type, $seller = null, $localtaxes_array = [], $progress = 100, $multicurrency_tx = 1, $pu_devise = 0, $multicurrency_code = '') // @phpstan-ignore-line
{
	...
	if ($price_base_type == 'HT') {
		...
		// Calculer le montant total ttc implique de faire pu_ttc * qte et l'arrondir
		// Faire pu * qte * (1+pct_taxe) créé une erreur d'accumulation de la plus petite décimale
		// en ajoutant des quantités.
		$result[6] = (float) price2num($tot_sans_remise, 'MT');
		...
		$result[8] = (float) price2num($tmpResult8 + $localtaxes[0], 'MT'); // Selon TVA NPR ou non
		$result8bis = (float) price2num($tmpREsult8bis + $localtaxes[0], 'MT'); // Si TVA consideree normal (non NPR)
		$result[7] = (float) price2num((float) $result8bis - ((float) $result[6] + $localtaxes[0]), 'MT');

		$result[0] = (float) price2num($tot_avec_remise, 'MT');
		// Le terme $tot_avec_remise * (1 + ((($info_bits & 1) ? 0 : $txtva) / 100) doit être tronqué au
		// nombre de décimales des totaux
		$tmpResult2 = (float) numTruncate(price2num($tot_avec_remise * (1 + ((($info_bits & 1) ? 0 : $txtva) / 100)), ''), $dolMaxDecimalUnits);
		$tmpResult2bis = (float) numTruncate(price2num($tot_avec_remise * (1 + ($txtva / 100)), ''), $dolMaxDecimalUnits);

		$result[2] = (float) price2num($tmpResult2 + $localtaxes[1], 'MT'); // Selon TVA NPR ou non
		$result2bis = (float) price2num($tmpResult2bis + $localtaxes[1], 'MT'); // Si TVA consideree normal (non NPR)
