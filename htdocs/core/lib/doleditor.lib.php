    		...
    		if (is_resource($handle))
    		{
    			while (($subdir = readdir($handle))!==false)
    			{
    						...
    						&& substr($subdir, 0, 3) <> 'CVS' && ! preg_match('/common|phones/i',$subdir))
    				{
    					...
    					{
    						print '<input '.($edit?'':'disabled').' type="radio" '.$bc[$var?1:0].' style="border: 0px;" checked name="fckeditor_skin" value="'.$subdir.'"> <b>'.$subdir.'</b>';
    					}
    					else
    					{
    						print '<input '.($edit?'':'disabled').' type="radio" '.$bc[$var?1:0].' style="border: 0px;" name="fckeditor_skin" value="'.$subdir.'"> '.$subdir;
    					}
						print '</div>';
