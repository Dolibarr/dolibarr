...
jQuery(document).ready(function () {
	...
	jQuery(".classfortooltiponclick").click(function () {
		...
		if ($(this).attr('dolid')) {
			let obj = $("#idfortooltiponclick_"+$(this).attr('dolid')); /* obj is a div component */
			obj.dialog("open");
			return false;
		}
	});
...
<?php if (!defined('JS_JQUERY_DISABLE_DROPDOWN')) : ?>
...
jQuery(document).ready(function () {
	...
	$(document).on('click', '.dropdown dt a', function () {
		...
		let heigthofcontent = 21 * $(this).parent().parent().find('dd div ul li').length;
		if (heigthofcontent > 300) heigthofcontent = 300; // limited by max-height on css .dropdown dd ul
		let posbottom = $(this).parent().parent().find('dd').offset().top + heigthofcontent + 8;
		let scrollBottom = $(window).scrollTop() + $(window).height();
		let diffoutsidebottom = (posbottom - scrollBottom);
		...
		if (diffoutsidebottom > 0) {
			let pix = "-"+(diffoutsidebottom+8)+"px";
...
jQuery(document).ready(function() {
	...
	let page_y = getParameterByName('page_y', 0); /* search in GET parameter */
...
jQuery(document).ready(function() {
	...
	jQuery('.clipboardCPValue, .clipboardCPButton, .clipboardCPValueToPrint').click(function() {
		...
		if (window.getSelection) {
			let jqobj = $(this).parent().children(".clipboardCPValue");
			console.log(jqobj.html());

			let selection = window.getSelection();	/* get the object used for selection */
...
jQuery(document).ready(function() {
	jQuery(".cssforclicktodial").click(function(event) {
...
jQuery(document).ready(function() {
	$(document).on("click", '.butActionConfirm', function(event) {
		...
		let confirmUrl  			= $(this).attr('data-confirm-url');
		let confirmTitle 			= $(this).attr('data-confirm-title');
		let confirmContent 			= $(this).attr('data-confirm-content');
		let confirmActionBtnLabel 	= $(this).attr('data-confirm-action-btn-label');
		let confirmCancelBtnLabel 	= $(this).attr('data-confirm-cancel-btn-label');
		let confirmModal	= $(this).attr('data-confirm-modal');
		if (confirmModal == undefined) { confirmModal = false; }

		let confirmId = 'confirm-dialog-box';
		if($(this).attr('id') != undefined) { confirmId = confirmId + "-" + $(this).attr('id'); }
