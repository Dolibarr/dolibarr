 ...
 */
class pdf_delivery_note extends ModelePDFCommandes
{
        /**
                ...
                if (getMultidirOutput($object)) {
                        ...
                        if (file_exists($dir)) {
                                ...
                                if (count($arrayidcontact) > 0) {
                                        ...
                                        $shippping_contact = '<b>Delivery Address:</b><br/>'.$outputlangs->convToOutputCharset($object->contact->getFullName($outputlangs));
                                        $shippping_contact .= !(empty($object->contact->address)) ? '<br/>'.$object->contact->address : '';
                                        $shippping_contact .= !(empty($object->contact->town)) ? '<br/>'.$object->contact->town : '';
                                        $shippping_contact .= !(empty($object->contact->zipcode)) ? '<br/>'.$object->contact->zipcode : '';
                                }
                                $notetoshow = $shippping_contact;*/
        ...
        protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $titlekey = "PdfOrderTitle")
        {
                ...
                $title_doc = 'Delivery Note';
                $pdf->MultiCell($w, 3, $title_doc, '', 'R');
                ...
                */
                ...
                if ($showaddress) {
                        ...
                        if (count($arrayidcontact) > 0) {
                                $object->fetch_contact($arrayidcontact[0]);
                                $shippping_contact = $outputlangs->convToOutputCharset($object->contact->getFullName($outputlangs));
                                $shippping_contact .= !(empty($object->contact->address)) ? '
'.$object->contact->address : '';
                                $shippping_contact .= !(empty($object->contact->town)) ? '
'.$object->contact->town : '';
                                $shippping_contact .= !(empty($object->contact->zip)) ? '
'.$object->contact->zip : '';
                        }
                        $carac_client = $shippping_contact;
