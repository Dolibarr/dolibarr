...
/**
 *       \file       htdocs/core/modules/societe/mod_codeclient_gazel.php
 *       \ingroup    societe
 *       \brief      File of class to manage third party code with gazel rule
 */
...
/**
 *	Class to manage third party code with gazel rule
 */
class mod_codeclient_gazel extends ModeleThirdPartyCode
{
	...
	public function info($langs)
	{
		...
		$texte .= '<input type="hidden" name="param1" value="COMPANY_GAZEL_MASK_CUSTOMER">';
		$texte .= '<input type="hidden" name="param2" value="COMPANY_GAZEL_MASK_SUPPLIER">';
		$texte .= '<input type="hidden" name="param3" value="COMPANY_GAZEL_DATE_START">';
		...
		$texte .= '<td class="right">'.$form->textwithpicto('<input type="text" class="flat minwidth175" name="value1" value="'.getDolGlobalString('COMPANY_GAZEL_MASK_CUSTOMER').'"'.$disabled.'>', $tooltip, 1, 1).'</td>';
		...
		if (!empty($datedb)) {
			if (!is_numeric($datedb) && GETPOSTISSET('value3')) {
				...
				$res = dolibarr_set_const($this->db, 'COMPANY_GAZEL_DATE_START', $dateinput, 'chaine', 0, '', $conf->entity);
			} else {
				$dateinput = $datedb;
			}
		}
		...
		$texte .= '<td class="right">'.$form->selectDate($dateinput, 'value3', 0, 0, 1, '', 1, 1, $disabled).'</td>';
		$texte .= '</tr>';
	...
	public function getExample($langs, $objsoc = '', $type = -1)
	{
		...
		$cssforerror = (getDolGlobalString('SOCIETE_CODECLIENT_ADDON') == 'mod_codeclient_gazel' ? 'error' : 'opacitymedium');
	...
	public function getNextValue($objsoc = '', $type = -1)
	{
		...
		$mask = '';
		if ($type == 0) {
			$mask = getDolGlobalString('COMPANY_GAZEL_MASK_CUSTOMER');
		}
		if ($type == 1) {
			$mask = getDolGlobalString('COMPANY_GAZEL_MASK_SUPPLIER');
		}
		if (!$mask) {
		...
		$numFinal = get_next_value($this->db, $mask, 'societe', $field, $where, '', $now);
	...
	public function verif($db, &$code, $soc, $type)
	{
		...
		//no check for customer numbers before COMPANY_GAZEL_DATE_START
		if ($soc->date_creation < getDolGlobalString('COMPANY_GAZEL_DATE_START')) {
			return -5;
		}
		...
		if (empty($code) && $this->code_null && !getDolGlobalString('MAIN_COMPANY_CODE_ALWAYS_REQUIRED')) {
			...
			$mask = '';
			if ($type == 0) {
				$mask = !getDolGlobalString('COMPANY_GAZEL_MASK_CUSTOMER') ? '' : $conf->global->COMPANY_GAZEL_MASK_CUSTOMER;
			}
			if ($type == 1) {
				$mask = !getDolGlobalString('COMPANY_GAZEL_MASK_SUPPLIER') ? '' : $conf->global->COMPANY_GAZEL_MASK_SUPPLIER;
			}
			if (!$mask) {
				$this->error = 'NotConfigured';
				return -5;
		...
		dol_syslog("mod_codeclient_gazel::verif type=".$type." result=".$result);
		return $result;
	}
