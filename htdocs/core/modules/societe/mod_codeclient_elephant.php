...
class mod_codeclient_elephant extends ModeleThirdPartyCode
{
	...
	public function info($langs)
	{
		...
		$texte .= '<td class="right">'.$form->textwithpicto('<input type="text" class="flat minwidth175" name="value1" value="'.(property_exists($conf->global, 'COMPANY_ELEPHANT_MASK_CUSTOMER')?$conf->global->COMPANY_ELEPHANT_MASK_CUSTOMER:'').'"'.$disabled.'>', $tooltip, 1, 1).'</td>';
		...
		$texte .= '<td class="right">'.$form->textwithpicto('<input type="text" class="flat minwidth175" name="value2" value="'.(property_exists($conf->global, 'COMPANY_ELEPHANT_MASK_SUPPLIER')?$conf->global->COMPANY_ELEPHANT_MASK_SUPPLIER:'').'"'.$disabled.'>', $tooltip, 1, 1).'</td>';
		$texte .= '</tr>';
