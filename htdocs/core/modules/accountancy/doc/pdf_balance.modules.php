 ...
 */
class pdf_balance extends ModelePdfAccountancy
{
	/**
	...
	/**
	 * @var string $balanceType Balance type, default is empty for general ledger, or 'sub' for subledgers
	 */
	public $balanceType;
	...
	public function __construct(DoliDB $db)
	{
		...
		$this->name = "balance";
		$this->description = $langs->trans("PDFAccountancyBalanceDescription");
	...
	public function write_file(BookKeeping $object, Translate $outputlangs, string $srctemplatepath = '', bool $directDownload = true)
	{
		...
		$outputlangs->loadLangs(array("main", "bills", "orders", "companies", "other", "accountancy"));
		...
		for ($i = 0; $i < $nblines; $i++) {
			...
			// If account group has changed, display the group summary line
			if ($accountGroup != $accountingAccount->pcg_type) {
				$this->addTotalLine(
					$pdf,
					$curY,
					$nexY,
			...
			$groupDebit += $object->lines[$i]->debit;
			$groupCredit += $object->lines[$i]->credit;
	...
	protected function addTotalLine(TCPDF $pdf, &$curY, &$nexY, $default_font_size, string $label, $tab_top_newpage, $debit, $credit, bool $uppercase = true)
	{
		...
		$this->printStdColumnContent($pdf, $curY, 'account_label', $label);
