...
/**
 *	\file       htdocs/core/modules/cheque/doc/pdf_blochet.modules.php
 *	\ingroup    banque
 ...
 */
class pdf_blochet extends ModeleChequeReceipts
{
	/**
	 * @var int tab_top
	 */
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$number  = dol_sanitizeFileName($object->ref);
		$dir     = $baseDir.'/'.$number;
		...
		$pdf->AddPage();
		$pagenb++;
		$this->Header($pdf, $pagenb, $pages, $outputlangs, $object);
	 ...
	 *	@return	void
	 */
	public function Header(&$pdf, $page, $pages, $outputlangs, $object)
	{
		// phpcs:enable
		global $langs;
		...
		$pdf->SetXY(32, 15);
		$pdf->SetFont('', '', $default_font_size);
		$pdf->MultiCell(60, 2, $outputlangs->convToOutputCharset($object->ref.(!empty($object->ref_ext) ? " - ".$object->ref_ext : '')), 0, 'L');
		...
		$pdf->SetXY(32, 20);
		$pdf->SetFont('', '', $default_font_size);
		$pdf->MultiCell(60, 2, dol_print_date($object->date_bordereau, "day", false, $outputlangs));
		...
		$pdf->SetFont('', '', $default_font_size);
		$pdf->SetXY(32, 26);
		$pdf->MultiCell(80, 2, $outputlangs->convToOutputCharset($object->account->owner_name), 0, 'L');

		$pdf->SetFont('', '', $default_font_size);
		$pdf->SetXY(10, 32);
		$pdf->MultiCell(22, 2, $outputlangs->transnoentities("BankAccount"), 0, 'L');
		pdf_bank($pdf, $outputlangs, 32, 32, $object->account, 1);
		...
		$pdf->SetFont('', 'B', $default_font_size);
		$pdf->SetXY(57, $posy + 1);
		$pdf->MultiCell(40, 2, (string) $object->nbcheque, 0, 'L');
		...
		$pdf->MultiCell(31, 2, price($object->amount), 0, 'C', false);
	...
	public function Body(&$pdf, $pagenb, $pages, $outputlangs, $object)
	{
		...
		for ($j = 0; $j < $num; $j++) {
			...
			$dynamic_line_height[] = $pdf->getStringHeight(60, $outputlangs->convToOutputCharset($object->lines[$j]->bank_chq));
			$dynamic_line_height[] = $pdf->getStringHeight(80, $outputlangs->convToOutputCharset($object->lines[$j]->emetteur_chq));
			...
			$pdf->MultiCell(30, $this->line_height, $object->lines[$j]->num_chq ? $object->lines[$j]->num_chq : '', 0, 'L', false);

			$pdf->SetXY(40, $this->tab_top + 10 + $yp);
			$pdf->MultiCell(60, $this->line_height, $outputlangs->convToOutputCharset($object->lines[$j]->bank_chq, '44'), 0, 'L', false);

			$pdf->SetXY(100, $this->tab_top + 10 + $yp);
			$pdf->MultiCell(80, $this->line_height, $outputlangs->convToOutputCharset($object->lines[$j]->emetteur_chq, '50'), 0, 'L', false);

			$pdf->SetXY(180, $this->tab_top + 10 + $yp);
			$pdf->MultiCell(20, $this->line_height, price($object->lines[$j]->amount_chq), 0, 'R', false);
