...
class modStock extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		$this->export_sql_start[$r] = 'SELECT DISTINCT ';
		$this->export_sql_end[$r] = ' FROM '.MAIN_DB_PREFIX.'product_stock as ps';	// Primary table is stock, but product must exist
		...
		$this->export_sql_end[$r] .= ' LEFT JOIN '.MAIN_DB_PREFIX.'entrepot as e ON e.rowid = ps.fk_entrepot';
		$this->export_sql_end[$r] .= ' WHERE e.entity IN ('.getEntity('stock').')';
		...
		if ($conf->productbatch->enabled)
		{
			...
			$this->export_sql_start[$r] = 'SELECT DISTINCT ';
			$this->export_sql_end[$r] = ' FROM '.MAIN_DB_PREFIX.'product_stock as ps';	// Primary table is stock, but product and batch must exist
			$this->export_sql_end[$r] .= ' INNER JOIN '.MAIN_DB_PREFIX.'product as p ON p.rowid = ps.fk_product';
			$this->export_sql_end[$r] .= ' INNER JOIN '.MAIN_DB_PREFIX.'product_batch as pb ON pb.fk_product_stock = ps.rowid';
		...
		$this->export_sql_start[$r] = 'SELECT DISTINCT ';
		$this->export_sql_end[$r]  = ' FROM '.MAIN_DB_PREFIX.'stock_mouvement as sm';	// Primary table is stock movements
		$this->export_sql_end[$r] .= ' LEFT JOIN '.MAIN_DB_PREFIX.'product as p ON p.rowid = sm.fk_product';
		$this->export_sql_end[$r] .= ' LEFT JOIN '.MAIN_DB_PREFIX.'entrepot as e ON e.rowid = sm.fk_entrepot';
		$this->export_sql_end[$r] .= ' WHERE e.entity IN ('.getEntity('stock').')';
