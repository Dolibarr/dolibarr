...
class modStock extends DolibarrModules
{
	...
	function __construct($db)
	{
		...
		$this->requiredby = array("modProductDluo");
		...
		$this->export_fields_array[$r]['ps.reel']='Stock';
		if (! empty($conf->productdluo->enabled)) $this->export_fields_array[$r]=array_merge($this->export_fields_array[$r],array('dl.dlc'=>'DLC','dl.dluo'=>'lDLUO','dl.lot'=>'lot_number','dl.qty'=>'Stock'));
		...
		if (! empty($conf->productdluo->enabled)) $this->export_TypeFields_array[$r]=array_merge($this->export_TypeFields_array[$r],array('p.todluo'=>'Boolean','dl.dlc'=>'Date','dl.dluo'=>'Date','dl.lot'=>'Text','dl.qty'=>'Number'));
		...
		if (! empty($conf->productdluo->enabled)) $this->export_entities_array[$r]=array_merge($this->export_entities_array[$r],array('p.todluo'=>'product','dl.dlc'=>'dluo','dl.dluo'=>'dluo','dl.lot'=>'dluo','dl.qty'=>'dluo'));
		...
		$this->export_sql_end[$r]  =' FROM '.MAIN_DB_PREFIX.'product as p JOIN '.MAIN_DB_PREFIX.'product_stock as ps on p.rowid = ps.fk_product JOIN '.MAIN_DB_PREFIX.'entrepot as e ON ps.fk_entrepot = e.rowid';
		if (! empty($conf->productdluo->enabled)) $this->export_sql_end[$r].= ' left join '.MAIN_DB_PREFIX.'product_dluo as dl on dl.fk_product_stock=ps.rowid';
