	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->propal->multidir_output[$conf->entity]) {
			...
			if ($object->specimen) {
				...
				$dir = $conf->propal->multidir_output[$object->entity ?? 1]."/".$objectref;
			...
			if (file_exists($dir)) {
				...
				*/
				...
				if (getDolGlobalString('MAIN_INFO_PROPAL_TERMSOFSALE') && getDolGlobalInt('MAIN_PDF_ADD_TERMSOFSALE_PROPAL')) {
					...
					if (!empty($conf->propal->multidir_output[$object->entity ?? 1])) {
						$termsofsale = $conf->propal->multidir_output[$object->entity ?? 1].'/'.$termsofsalefilename;
					}
									...
									if (!empty($linefile->id) && !empty($linefile->file_name)) {
										if (getDolGlobalInt('PRODUCT_USE_OLD_PATH_FOR_PHOTO')) {
											if (isModEnabled("product")) {
												$filetomerge_dir = $conf->product->multidir_output[$entity_product_file ?? 1].'/'.get_exdir($product->id, 2, 0, 0, $product, 'product').$product->id."/photos";
											} elseif (isModEnabled("service")) {
												$filetomerge_dir = $conf->service->multidir_output[$entity_product_file ?? 1].'/'.get_exdir($product->id, 2, 0, 0, $product, 'product').$product->id."/photos";
											}
										} else {
											if (isModEnabled("product")) {
												$filetomerge_dir = $conf->product->multidir_output[$entity_product_file ?? 1].'/'.get_exdir(0, 0, 0, 0, $product, 'product');
											} elseif (isModEnabled("service")) {
												$filetomerge_dir = $conf->service->multidir_output[$entity_product_file ?? 1].'/'.get_exdir(0, 0, 0, 0, $product, 'product');
											}
										}
