	...
	public function __construct($db)
	{
		...
		*/
		...
		if (!empty($conf->global->PRODUIT_MULTIPRICES)) {
			...
			$this->export_sql_end[$r] .= ' AND pr.date_price = (SELECT MAX(pr2.date_price) FROM '.MAIN_DB_PREFIX.'product_price as pr2 WHERE pr2.fk_product = pr.fk_product)'; // export only latest prices not full history
