...
class modContrat extends DolibarrModules
{
	...
	function __construct($db)
	{
		...
		$this->langfiles = array("companies","contracts","products","interventions");
		// Config pages
		...
		$r++;
		$this->rights[$r][0] = 166;
		$this->rights[$r][1] = 'Exporter les contrats clients, services et interventions';
		$this->rights[$r][2] = 'r';
		$this->rights[$r][3] = 0;
		...
		'co.fin_validite'=>"EndValidity",'co.date_cloture'=>"Cloture",'co.fk_statut'=>'ContractStatus','co.note'=>"NotePrivate",'co.note_public'=>"NotePublic",
		...
		'co.fin_validite'=>"Contract",'co.date_cloture'=>"Contract",'co.fk_statut'=>'Contract','co.note'=>"Contract",'co.note_public'=>"Contract",
		...
		'co.fin_validite'=>"Date",'co.date_cloture'=>"Date",'co.fk_statut'=>'Statut','co.note'=>"Text",'co.note_public'=>"Text",
		...
		$r++;
		$this->export_code[$r]=$this->rights_class.'_'.$r;
		$this->export_label[$r]='ContractAndInterventions';	// Translation key (used only if key ExportDataset_xxx_z not found)
		$this->export_icon[$r]='contract';
		//$this->export_permission[$r]=array(array("contrat","ficheinter","export"));
		$this->export_permission[$r]=array(array("contrat","export"));
		
		$this->export_fields_array[$r]=array('s.rowid'=>"IdCompany",'s.nom'=>'CompanyName','s.address'=>'Address','s.zip'=>'Zip','s.town'=>'Town','c.code'=>'CountryCode',
		's.phone'=>'Phone','s.siren'=>'ProfId1','s.siret'=>'ProfId2','s.ape'=>'ProfId3','s.idprof4'=>'ProfId4','s.code_compta'=>'CustomerAccountancyCode',
		's.code_compta_fournisseur'=>'SupplierAccountancyCode','s.tva_intra'=>'VATIntra',
		'co.rowid'=>"contractId",'co.ref'=>"RefContract",'co.datec'=>"DateContract",'co.date_contrat'=>"ContractStartDate",'co.mise_en_service'=>"DateStartReal",
		'co.fin_validite'=>"DateEndReal",'co.date_cloture'=>"ContractEndDate",'co.fk_statut'=>'ContractStatus','co.note'=>"NotePrivate",'co.note_public'=>"NotePublic",
		'fi.rowid'=>"InterId",'fi.ref'=>"InterRef",'fi.datec'=>"InterDateCreation",'fi.duree'=>"InterDuration",'fi.fk_statut'=>'InterStatus','fi.description'=>"InterNote",'fi.datee'=>"DateEnd",'fi.dateo'=>"DateOpen",'fi.fulldayevent'=>"FullDayEvent",'fi.total_ht'=>"InterTotalHT"
		);
		...
		'co.rowid'=>"Contract",'co.ref'=>"Contract",'co.datec'=>"Contract",'co.date_contrat'=>"Contract",'co.mise_en_service'=>"Contract",
		'co.fin_validite'=>"Contract",'co.date_cloture'=>"Contract",'co.fk_statut'=>'Contract','co.note'=>"Contract",'co.note_public'=>"Contract",
		'fi.rowid'=>"intervention",'fi.ref'=>"intervention",'fi.datec'=>"intervention",'fi.duree'=>"intervention",'fi.fk_statut'=>"intervention",'fi.description'=>"intervention",'fi.datee'=>"intervention",'fi.dateo'=>"intervention",'f.fulldayevent'=>"intervention",'fi.total_ht'=>"intervention"
		...
		'co.ref'=>"Text",'co.datec'=>"Date",'co.date_contrat'=>"Date",'co.mise_en_service'=>"Date",
		'co.fin_validite'=>"Date",'co.date_cloture'=>"Date",'co.fk_statut'=>'Statut','co.note'=>"Text",'co.note_public'=>"Text",
		'fi.ref'=>"Text",'fi.datec'=>"Date",'fi.duree'=>"Duree",'fi.fk_statut'=>'Statut','fi.description'=>"Text",'fi.datee'=>"Date",'fi.dateo'=>"Date",'fi.fulldayevent'=>"Boolean",'fi.total_ht'=>"intervention"
		);
		...
		$this->export_sql_end[$r] .=' LEFT JOIN '.MAIN_DB_PREFIX.'c_pays as c on s.fk_pays = c.rowid,';
		$this->export_sql_end[$r] .=' '.MAIN_DB_PREFIX.'contrat as co';
		$this->export_sql_end[$r] .=' LEFT JOIN '.MAIN_DB_PREFIX.'fichinter as fi on (co.rowid = fi.fk_contrat)';
		$this->export_sql_end[$r] .=' WHERE co.fk_soc = s.rowid';
		$this->export_sql_end[$r] .=' AND co.entity = '.$conf->entity;
