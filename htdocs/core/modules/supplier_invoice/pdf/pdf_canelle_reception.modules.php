...
/**
 *	\file       htdocs/core/modules/supplier_invoice/pdf/pdf_canelle_reception.modules.php
 *	\ingroup    fournisseur
 *	\brief      Class file to generate the supplier invoices with the canelle_reception model
 */
...
/**
 *	Class to generate the supplier invoices with the canelle_reception model
 */
class pdf_canelle_reception extends ModelePDFSuppliersInvoices
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->posxpicture=$this->posxreception - (empty($conf->global->MAIN_DOCUMENTS_WITH_PICTURE_WIDTH)?20:$conf->global->MAIN_DOCUMENTS_WITH_PICTURE_WIDTH);	// width of images
		if ($this->page_largeur < 210) // To work with US executive format
		{
			$this->posxpicture-=20;
			$this->posxreception-=20;
			$this->posxtva-=20;
			$this->posxup-=20;
			$this->posxqty-=20;
			$this->posxunit-=20;
		...
		if ($conf->fournisseur->dir_output.'/facture')
		{
			...
			if (file_exists($dir))
			{
				...
				if (empty($this->atleastonediscount) && empty($conf->global->PRODUCT_USE_UNITS))
				{
					...
					$this->posxreception+=($this->postotalht - $this->posxdiscount);
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					pdf_writelinedesc($pdf,$object,$i,$outputlangs,$this->posxreception-$curX,3,$curX,$curY,$hideref,$hidedesc,1);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						pdf_writelinedesc($pdf,$object,$i,$outputlangs,$this->posxreception-$curX,4,$curX,$curY,$hideref,$hidedesc,1);
						$posyafter=$pdf->GetY();
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		$pdf->line($this->posxreception-1, $tab_top, $this->posxreception-1, $tab_top + $tab_height);
		
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxreception-3, $tab_top+1);
			$pdf->MultiCell($this->posxtva-$this->posxreception+3,2, $outputlangs->transnoentities("Reception"),'','C');
		}
