 ...
 */
class pdf_enlevement extends ModelePDFSuppliersOrders
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->description = $langs->trans('SuppliersProductWithdrawal');
	...
	public function write_file($object, $outputlangs = '', $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->fournisseur->commande->dir_output) {
			...
			if (file_exists($dir)) {
				...
				// Affiche la zone de signature
				$posy = $this->_tableau_sign($pdf, $object, $bottomlasttab, $outputlangs);
	 ...
	 *   @return	integer
	 */
	protected function _tableau_sign(&$pdf, $object, $posy, $outputlangs)
	{
		// phpcs:enable
		global $conf, $mysoc;
		$default_font_size = pdf_getPDFFontSize($outputlangs);
		...
		// Show responsible personne
		$pdf->SetFont('', 'B', $default_font_size);
		$pdf->SetXY($this->marge_gauche, $posy);
		$titre = $outputlangs->transnoentities("ProductWithdrawalBy").':';
		$pdf->MultiCell(80, 5, $titre, 0, 'L');
		$posy = $pdf->GetY() + 2;
		...
		$pdf->SetFont('', 'B', $default_font_size);
		$pdf->SetXY($this->marge_gauche, $posy);
		$titre = $outputlangs->transnoentities("Signature").':';
		$pdf->MultiCell(80, 5, $titre, 0, 'L');
		$posy = $pdf->GetY() + 2;
		$this->printRect($pdf, $this->marge_gauche, $posy, ($this->page_largeur - $this->marge_droite - $this->marge_gauche) / 3, 20, $hidetop, $hidebottom);
		$posy = $pdf->GetY() + 2;		
		return $posy;
	}
