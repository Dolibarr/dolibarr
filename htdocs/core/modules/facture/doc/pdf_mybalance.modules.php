 ...
 */
class pdf_mybalance extends ModelePDFFactures
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "mybalance";
		$this->description = "Πρότυπο τιμολογίου με εμφάνιση υπολοίπου πελάτη (αντίγραφο του sponge)";
		...
		$this->isIntercompanyTransfer = false;
	}
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
        ...
        // ---> START: ΕΛΕΓΧΟΣ ΓΙΑ ΑΝΑΓΝΩΡΙΣΗ ΕΝΔΟΔΙΑΚΙΝΗΣΗΣ ---
        $object->fetch_thirdparty();
        $object->thirdparty->fetch_optionals();
        $extrafield_code = 'brxeendodiak';

        if (isset($object->thirdparty->array_options['options_'.$extrafield_code]) && $object->thirdparty->array_options['options_'.$extrafield_code] == 1) {
            $this->isIntercompanyTransfer = true;
        }
        // ---> END: ΕΛΕΓΧΟΣ ΓΙΑ ΑΝΑΓΝΩΡΙΣΗ ΕΝΔΟΔΙΑΚΙΝΗΣΗΣ ---
		...
		if ($conf->facture->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
                ...
                // ---> START: ΔΥΝΑΜΙΚΗ ΕΜΦΑΝΙΣΗ ΠΛΑΙΣΙΩΝ ΣΥΝΟΛΩΝ ---
                if (!$this->isIntercompanyTransfer) {
                    // ΑΝ ΔΕΝ ΕΙΝΑΙ ΕΝΔΟΔΙΑΚΙΝΗΣΗ, ΤΥΠΩΣΕ ΤΑ ΠΑΝΤΑ ΚΑΝΟΝΙΚΑ
                    ...
                    // ΑΝ ΕΙΝΑΙ ΕΝΔΟΔΙΑΚΙΝΗΣΗ, ΤΥΠΩΣΕ ΜΟΝΟ ΤΟ ΣΥΝΟΛΟ ΠΟΣΟΤΗΤΑΣ
                    $total_qty = 0;
                    foreach ($object->lines as $line) {
                        if ($line->product_type < 9) { // Εξαιρούμε τους τίτλους ενοτήτων
                           $total_qty += $line->qty;
                        }
                    }
                    ...
                    // Η συνάρτηση transnoentities θα ψάξει για μετάφραση. Αν δεν βρει, θα τυπώσει το κείμενο ως έχει.
                    $pdf->MultiCell(0, 5, $outputlangs->transnoentities("TotalQuantity") . ': ' . $total_qty, 0, 'L');
                }
                // ---> END: ΔΥΝΑΜΙΚΗ ΕΜΦΑΝΙΣΗ ΠΛΑΙΣΙΩΝ ΣΥΝΟΛΩΝ ---
	...
	protected function drawInfoTable(&$pdf, $object, $posy, $outputlangs, $outputlangsbis)
	{
        ...
        // --- ΑΡΧΗ ΕΝΟΤΗΤΑΣ ΥΠΟΛΟΙΠΟΥ ΠΕΛΑΤΗ ---
        $outstanding_balance = 0;
        
        // Υπολογισμός του υπολοίπου από τη βάση δεδομένων
        if (!empty($object->thirdparty->id)) {
            $socid = (int) $object->thirdparty->id;
        
            $sql_balance = "SELECT SUM(f.total_ttc) as total";
            $sql_balance .= " FROM ".MAIN_DB_PREFIX."facture as f";
            $sql_balance .= " WHERE f.fk_soc = ".$socid;
            $sql_balance .= " AND f.fk_statut IN (1, 2)";
        
            $res = $this->db->query($sql_balance);
            $invoices_balance = 0;
            if ($res) {
                $obj = $this->db->fetch_object($res);
                if ($obj) {
                    $invoices_balance = (double) $obj->total;
                }
            }
        
            $sql_paid = "SELECT SUM(pf.amount) as total";
            $sql_paid .= " FROM ".MAIN_DB_PREFIX."paiement_facture as pf";
            $sql_paid .= " JOIN ".MAIN_DB_PREFIX."facture as f ON pf.fk_facture = f.rowid";
            $sql_paid .= " WHERE f.fk_soc = ".$socid;
        
            $res_paid = $this->db->query($sql_paid);
            $total_paid = 0;
            if ($res_paid) {
                $obj_paid = $this->db->fetch_object($res_paid);
                if ($obj_paid) {
                    $total_paid = (double) $obj_paid->total;
                }
            }
            
            $outstanding_balance = $invoices_balance - $total_paid;
        }
        
        // Εκτύπωση του υπολοίπου στο PDF, αν δεν είναι μηδέν (ή αν υπάρχει ειδική ρύθμιση)
        if (abs($outstanding_balance) > 0.005 || getDolGlobalInt('MYBALANCE_SHOW_ZERO_BALANCE')) {
            
            // Επιλογή της σωστής ετικέτας (Χρεωστικό ή Πιστωτικό)
            if ($outstanding_balance < -0.005) {
                $balance_text_label = "Πιστωτικό υπόλοιπο:";
            } else {
                $balance_text_label = "Ανεξόφλητο υπόλοιπο:";
            }
            
            // Μορφοποίηση της τιμής προς εμφάνιση
            $balance_value_to_show = price(abs($outstanding_balance), 0, $outputlangs);
            
            // Κώδικας εκτύπωσης στο PDF με σωστό στυλ και απόσταση
            $pdf->SetFont('', '', $default_font_size - 2);
            $posy += 2;
            
            $pdf->SetXY($this->marge_gauche, $posy);
            $pdf->MultiCell($posxval - $this->marge_gauche, 4, $balance_text_label, 0, 'L');
            
            $pdf->SetXY($posxval, $posy);
            $pdf->MultiCell(100, 4, $balance_value_to_show, 0, 'L');
        
            $posy = $pdf->GetY();
        }
        // --- ΤΕΛΟΣ ΕΝΟΤΗΤΑΣ ΥΠΟΛΟΙΠΟΥ ΠΕΛΑΤΗ ---
		return $posy;
	}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		// --- START: FINAL DYNAMIC TITLE CODE (v7 - Based on Third Party Extrafield) ---
        $title = ''; // Initialize title variable
        
        // First, fetch the extrafields for the third party associated with this invoice
        $object->thirdparty->fetch_optionals();
        
        // The attribute code from your screenshot is 'brxeendodiak'
        $extrafield_code = 'brxeendodiak';
        
        // Check if the extrafield exists and its value is 1 (checked)
        if (isset($object->thirdparty->array_options['options_'.$extrafield_code]) && $object->thirdparty->array_options['options_'.$extrafield_code] == 1) {
            // The checkbox is checked! Now, let's determine the correct title based on the invoice type.
            
            // Check if it's a Credit Note (type 2)
            if (isset($object->type) && $object->type == 2) {
                $title = 'Πιστωτικό Ενδοδιακίνησης';
            } 
            // For all other types (standard invoice, etc.), use the main title
            else {
                $title = 'Δελτίο Ενδοδιακίνησης';
            }
        
        } else {
            // If the checkbox is NOT checked, use the standard Dolibarr logic
            if ($this->situationinvoice) {
                ...
                switch (isset($object->type) ? $object->type : 0) {
                    case 1:
                    ...
                    case 4:
                        ...
                        break;
                    default: // This covers type 0 (Standard)
                        $title = $outputlangs->transnoentities("PdfInvoiceTitle"); // "Τιμολόγιο"
                        break;
                }
            }
        }
        
        // Final processing before printing to ensure correct character encoding
        $title = $outputlangs->convToOutputCharset($title);
        // --- END: FINAL DYNAMIC TITLE CODE (v7 - Based on Third Party Extrafield) ---
	...
	public function defineColumnField($object, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$this->cols['vat'] = array(
			'rank' => $rank,
			'status' => !$this->isIntercompanyTransfer && !getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN'), // Η ΑΛΛΑΓΗ ΜΑΣ
			'width' => 14, // in mm
			'title' => array(
				'textkey' => 'VAT'
			),
		...
		$this->cols['subprice'] = array(
			'rank' => $rank,
			'width' => 19, // in mm
			'status' => !$this->isIntercompanyTransfer, // Η ΑΛΛΑΓΗ ΜΑΣ
			'title' => array(
				'textkey' => 'PriceUHT'
			),
			'border-left' => true, // add left line separator
		...
		$this->cols['discount'] = array(
			'rank' => $rank,
			'width' => 16, // in mm
			'status' => !$this->isIntercompanyTransfer && $this->atleastonediscount, // Η ΑΛΛΑΓΗ ΜΑΣ
			'title' => array(
		...
		$this->cols['totalexcltax'] = array(
			'rank' => $rank,
			'width' => 26, // in mm
			'status' => !$this->isIntercompanyTransfer && !getDolGlobalBool('PDF_INVOICE_HIDE_PRICE_EXCL_TAX'), // Η ΑΛΛΑΓΗ ΜΑΣ
			'title' => array(
		...
		$this->cols['totalincltax'] = array(
			'rank' => $rank,
			'width' => 26, // in mm
			'status' => !$this->isIntercompanyTransfer && getDolGlobalBool('PDF_INVOICE_SHOW_PRICE_INCL_TAX'), // Η ΑΛΛΑΓΗ ΜΑΣ
			'title' => array(
