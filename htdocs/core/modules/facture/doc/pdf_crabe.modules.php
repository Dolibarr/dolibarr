...
class pdf_crabe extends ModelePDFFactures
{
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		$pdf->MultiCell($largcol2, $tab2_hl, price($sign * ($object->total_ht + (! empty($object->remise)?$object->remise:0)), 0, $outputlangs,1,'MT'), 0, 'R', 1);
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			else
			{
					...
					foreach( $this->localtax1 as $localtax_type => $localtax_rate )
					{
						...
						foreach( $localtax_rate as $tvakey => $tvaval )
						{
							if ($tvakey!=0)    // On affiche pas taux 0
							{
								...
								$pdf->MultiCell($largcol2, $tab2_hl, price($tvaval, 0, $outputlangs,1,'MT'), 0, 'R', 1);
							}
						}
					}
	      		//}
				...
				if (price2num($object->revenuestamp) != 0)
				{
					...
					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($largcol2, $tab2_hl, price($sign * $object->revenuestamp, 0, $outputlangs,1,'MT'), $useborder, 'R', 1);
				}
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, price($sign * $object->total_ttc, 0, $outputlangs,1,'MT'), $useborder, 'R', 1);
			}
		}
		...
		if ($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0)
		{
			...
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($largcol2, $tab2_hl, price($deja_regle + $depositsamount, 0, $outputlangs,1,'MT'), 0, 'R', 0);
			...
			if ($creditnoteamount)
			{
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, price($creditnoteamount, 0, $outputlangs,1,'MT'), 0, 'R', 0);
			}
			...
			if ($object->close_code == Facture::CLOSECODE_DISCOUNTVAT)
			{
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, price($object->total_ttc - $deja_regle - $creditnoteamount - $depositsamount, 0, $outputlangs,1,'MT'), $useborder, 'R', 1);
			...
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($largcol2, $tab2_hl, price($resteapayer, 0, $outputlangs,1,'MT'), $useborder, 'R', 1);
