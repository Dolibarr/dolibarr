...
class pdf_crabe extends ModelePDFFactures
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->dir_output) {
			...
			if ($object->specimen) {
				$dir = empty($conf->facture->multidir_output[$object->entity ?? 1]) ? $conf->facture->dir_output : $conf->facture->multidir_output[$object->entity ?? 1];
				$file = $dir."/SPECIMEN.pdf";
			} else {
				$objectref = dol_sanitizeFileName($object->ref);
				$dir = (empty($conf->facture->multidir_output[$object->entity ?? 1]) ? $conf->facture->dir_output : $conf->facture->multidir_output[$object->entity ?? 1])."/".$objectref;
			...
			if (file_exists($dir)) {
				...
				if (!empty($mysoc->termsofsale) && getDolGlobalInt('MAIN_PDF_ADD_TERMSOFSALE_INVOICE')) {
					...
					if (!empty($conf->mycompany->multidir_output[$object->entity ?? 1])) {
						$termsofsale = $conf->mycompany->multidir_output[$object->entity ?? 1].'/'.$mysoc->termsofsale;
					}
