 ...
 */
class pdf_sponge_subtotal extends ModelePDFFactures
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "sponge_subtotal";
		$this->description = $langs->trans('PDFSpongeSubtotalDescription');
		...
		if ($conf->facture->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if ($object->lines[$i]->special_code == 104777 && $object->lines[$i]->label) {
						$a = preg_split('/\s+/', $object->lines[$i]->label);
						array_shift($a);
						$object->lines[$i]->description = '<b>' . implode(' ', $a) . '</b>';
					}
