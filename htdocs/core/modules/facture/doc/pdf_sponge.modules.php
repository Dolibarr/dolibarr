...
class pdf_sponge extends ModelePDFFactures
{
	...
	protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis)
	{
		...
		if ($total_discount_on_lines > 0 && !$object->isSituationInvoice()) {
			...
			if (!getDolGlobalBool('MAIN_HIDE_AMOUNT_DISCOUNT') && $object->type != 2) {
				$pdf->SetFillColor(255, 255, 255);
			...
			if (!getDolGlobalBool('MAIN_HIDE_AMOUNT_BEFORE_DISCOUNT') && $object->type != 2) {
				$pdf->SetFillColor(255, 255, 255);
		...
		$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities(!getDolGlobalBool('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT') ? "TotalHT" : "Total").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transnoentities(!getDolGlobalBool('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT') ? "TotalHT" : "Total") : ''), 0, 'L', true);
		...
		if (!getDolGlobalBool('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT')) {
			...
			if (getDolGlobalBool('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL') && $tvaisnull) {
				...
				if (!getDolGlobalBool('PDF_INVOICE_SHOW_VAT_ANALYSIS')) {	// by default, we show detail of vat here
					// VAT
					foreach ($this->tva_array as $tvakey => $tvaval) {
	...
	public function defineColumnField($object, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if (!getDolGlobalBool('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT') && !getDolGlobalBool('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN') && !getDolGlobalBool('PDF_PURCHASE_INVOICE_HIDE_VAT')) {
