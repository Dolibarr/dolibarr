...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_protagoras.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from protagoras model
 */
...
/**
 *	Class to generate the customer invoice PDF with template protagoras
 */
class pdf_protagoras extends ModelePDFFactures
{
    /**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "protagoras";
		$this->description = $langs->trans('PDFprotagorasDescription');
			...
			if (file_exists($dir))
			{
				...
				if ($object->situation_cycle_ref && empty($conf->global->MAIN_PDF_HIDE_SITUATION))
				{
					...
					$this->posxtva -= $progress_width;
                                        $this->posxtotaltax -= $progress_width;
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
					{
						...
						$pdf->SetXY($this->posxtva - 5, $curY);
                                                $pdf->MultiCell($this->posxtotaltax - $this->posxtva + 4, 3, $vat_rate, 0, 'R');
					}

                                        // Total tax amount per line
					$totaltva = $object->lines[$i]->total_tva;
					$totallocaltax1 = $object->lines[$i]->total_localtax1;
					$totallocaltax2 = $object->lines[$i]->total_localtax2;

					$totallinetaxinit = $totaltva + $totallocaltax1 + $totallocaltax2;
					$total_line_tax = round($totallinetaxinit, 2);

					$pdf->SetXY($this->posxtotaltax, $curY);
                                        $pdf->MultiCell($this->posxdiscount-$this->posxtotaltax+0.1, 3, $total_line_tax, 0, 'R', 0);
					...
					if (!empty($conf->global->MAIN_PDF_DASH_BETWEEN_LINES) && $i < ($nblines - 1))
					{
						$pdf->setPage($pageposafter);
                                                $pdf->SetLineStyle(array('F'=>'1,1','color'=>array(198, 198, 198)));
						$pdf->line($this->marge_gauche, $nexY + 1, $this->page_largeur - $this->marge_droite, $nexY + 1);
                                                $pdf->SetLineStyle(array('F'=>0));
					}
	...
	global $conf, $mysoc;
		...
		$pdf->SetXY($col1x, $tab2_top + 0);
                $pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities("Total Excl. Tax"), 0, 'L', 1);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop))
		{
                        $titre = "Currency: ".$outputlangs->transnoentitiesnoconv("Currency".$currency);
                        $pdf->SetXY($this->page_largeur - $this->marge_droite - ($pdf->GetStringWidth($titre) + 3), $tab_top - 4.5);
		...
		if (empty($hidetop))
		{
                        $pdf->line($this->marge_gauche, $tab_top + 6, $this->page_largeur - $this->marge_droite, $tab_top + 6); // line takes a position y in 2nd parameter and 4th parameter
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
		{
			...
			if (empty($hidetop))
			{
				$pdf->SetXY($this->posxtva - 3, $tab_top + 1);
                                $pdf->MultiCell($this->posxtotaltax-$this->posxtva+3, 2, $outputlangs->transnoentities("Taxes"), '', 'C');
			}
		}

                $pdf->line($this->posxtotaltax-1, $tab_top, $this->posxtotaltax-1, $tab_top + $tab_height);
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxtotaltax - 1, $tab_top + 1);
			$pdf->MultiCell($this->posxdiscount-$this->posxtotaltax+1, 2, $outputlangs->transnoentities("Taxes Amount"), '', 'C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(34, 2, $outputlangs->transnoentities("Total Excl. Tax"), '', 'C');
