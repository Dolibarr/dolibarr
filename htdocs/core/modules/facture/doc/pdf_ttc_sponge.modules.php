 ...
 */
class pdf_ttc_sponge extends ModelePDFFactures
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				$this->heightforinfotot = (int) (50 + (4 * $nbpayments));
				$this->heightforfreetext = (int) (isset($conf->global->MAIN_PDF_FREETEXT_HEIGHT) ? $conf->global->MAIN_PDF_FREETEXT_HEIGHT : 5);
				$this->heightforfooter   = (int) ($this->marge_basse + (!getDolGlobalString('MAIN_GENERATE_DOCUMENTS_SHOW_FOOT_DETAILS') ? 12 : 22));
				...
				$pagehead = $this->_pagehead($pdf, $object, 1, $outputlangs, $olb);
				...
				if (!empty($extranote)) {
					$notetoshow = dol_concatdesc((string) $notetoshow, (string) $extranote);
				}
					...
					if ($pageposafternote > $pageposbeforenote) {
						...
						while ($pagenb < $pageposafternote) {
							...
							if (!getDolGlobalInt('MAIN_PDF_DONOTREPEAT_HEAD')) {
								$this->_pagehead($pdf, $object, 0, $outputlangs, $olb);
							}
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					// Unit price TTC (replace VAT and price ex. tax)
					if ($this->getColumnStatus('subprice')) {
						$up_incl_tax = $this->pdfGetLineUpInclTax($object, $i, $outputlangs, $hidedetails);
					...
					while ($pagenb < $pageposafter) {
						...
						if ($pagenb == $pageposbeforeprintlines) {
							$this->_tableau($pdf, $this->tab_top, $this->page_hauteur - $this->tab_top - $this->heightforfooter - $heightforqrinvoice, 0, $outputlangs, $hidetop, 1, $object->multicurrency_code, $olb);
						} else {
							$this->_tableau($pdf, $this->tab_top_newpage, $this->page_hauteur - $this->tab_top_newpage - $this->heightforfooter - $heightforqrinvoice, 0, $outputlangs, 1, 1, $object->multicurrency_code, $olb);
						}
				...
				$heightforqrinvoice = $this->getHeightForQRInvoice($pagenb, $object, $langs);
				if ($pagenb == $pageposbeforeprintlines) {
					$this->_tableau($pdf, $this->tab_top, $this->page_hauteur - $this->tab_top - $this->heightforinfotot - $this->heightforfreetext - $this->heightforfooter - $heightforqrinvoice, 0, $outputlangs, $hidetop, 0, $object->multicurrency_code, $olb);
					$bottomlasttab = $this->page_hauteur - $this->heightforinfotot - $this->heightforfreetext - $this->heightforfooter - $heightforqrinvoice + 1;
				} else {
					$this->_tableau($pdf, $this->tab_top_newpage, $this->page_hauteur - $this->tab_top_newpage - $this->heightforinfotot - $this->heightforfreetext - $this->heightforfooter - $heightforqrinvoice, 0, $outputlangs, 1, 0, $object->multicurrency_code, $olb);
					$bottomlasttab = $this->page_hauteur - $this->heightforinfotot - $this->heightforfreetext - $this->heightforfooter - $heightforqrinvoice + 1;
				}

				// Display infos area
				$posy = $this->drawInfoTable($pdf, $object, (int) $bottomlasttab, $outputlangs, $outputlangsbis);

				// Display total zone
				$posy = $this->drawTotalTable($pdf, $object, $deja_regle, (int) $bottomlasttab, $outputlangs, $outputlangsbis);
	...
	/**
	 *  Function to get unit price including tax
	 *
	 ...
	 */
	public function pdfGetLineUpInclTax($object, $i, $outputlangs, $hidedetails = 0)
	{
		if (empty($hidedetails) || $hidedetails > 1) {
			...
			$up_excl_tax = $object->lines[$i]->subprice;
			$vat_rate = $object->lines[$i]->tva_tx;
			$up_incl_tax = $up_excl_tax * (1 + ($vat_rate / 100));

			$unit_price_ttc = price($sign * $up_incl_tax, 0, $outputlangs);
			...
			if (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'VIR') {
				if ($object->fk_account > 0 || $object->fk_bank > 0 || getDolGlobalInt('FACTURE_RIB_NUMBER')) {
					...
					if (getDolGlobalString('INVOICE_ADD_EPC_QR_CODE') == 'bottom') {
						...
						$pdf->MultiCell(30, 3, $langs->trans("INVOICE_ADD_EPC_QR_CODEPay"), 0, 'L', false);
						$posy = $pdf->GetY() + 2;
					}
				}
			}
	...
	protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis = null)
	{
		...
		$tab2_top = $posy;
		$tab2_hl = 4;
		if (($outputlangsbis instanceof Translate)) {
			$pdf->SetFont('', '', $default_font_size - 2);
		} else {
			$pdf->SetFont('', '', $default_font_size - 1);
		}
		...
		$hookmanager->executeHooks('beforePercentCalculation', $parameters, $this);

		// Get Total TTC directly
		$total_ttc = (isModEnabled("multicurrency") && $object->multicurrency_tx != 1) ? $object->multicurrency_total_ttc : $object->total_ttc;

		// AFFICHER DIRECTEMENT LE TOTAL TTC (sans détail HT ni TVA)
		$pdf->SetTextColor(0, 0, 60);
		$pdf->SetFillColor(224, 224, 224);
		$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
		$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Total").(($outputlangsbis instanceof Translate) ? ' / '.$outputlangsbis->transnoentities("Total") : ''), $useborder, 'L', true);
		...
		if (($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0) && !getDolGlobalString('INVOICE_NO_PAYMENT_DETAILS')) {
			...
			$index++;
			$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Paid").(($outputlangsbis instanceof Translate) ? ' / '.$outputlangsbis->transnoentities("Paid") : ''), 0, 'L', false);
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			...
			if ($creditnoteamount) {
				...
				$labeltouse .= (($outputlangsbis instanceof Translate) ? (' / '.(($outputlangsbis->transnoentities("CreditNotesOrExcessReceived") != "CreditNotesOrExcessReceived") ? $outputlangsbis->transnoentities("CreditNotesOrExcessReceived") : $outputlangsbis->transnoentities("CreditNotes"))) : '');
				$index++;
				$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
			...
			if ($object->close_code == Facture::CLOSECODE_DISCOUNTVAT) {
				...
				$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("EscompteOfferedShort").(($outputlangsbis instanceof Translate) ? ' / '.$outputlangsbis->transnoentities("EscompteOfferedShort") : ''), $useborder, 'L', true);
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			...
			$pdf->SetFillColor(224, 224, 224);
			$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("RemainderToPay").(($outputlangsbis instanceof Translate) ? ' / '.$outputlangsbis->transnoentities("RemainderToPay") : ''), $useborder, 'L', true);
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
		...
		$reshook = $hookmanager->executeHooks('afterPDFTotalTable', $parameters, $this);
		if ($reshook < 0) {
			$this->error = $hookmanager->error;
			$this->errors = $hookmanager->errors;
		}
	...
	// phpcs:disable PEAR.NamingConventions.ValidFunctionName.ScopeNotCamelCaps
	/**
 * Return list of available pdf models for invoices.
 *
 * @param DoliDB $db                Database handler
 ...
 */
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '', $outputlangsbis = null)
	{
		...
		if (empty($hidetop)) {
			...
			if (getDolGlobalString('PDF_USE_ALSO_LANGUAGE_CODE') && ($outputlangsbis instanceof Translate)) {
 ...
 */
	...
	public function defineColumnField($object, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		'status' => true,
		'title' => array(
			'textkey' => 'PU TTC' // CHANGÉ DE PriceUHT À PriceTTC
		),
		'border-left' => true,
		);

		// Adapter dynamiquement la largeur pour les prix TTC
		$tmpwidth = 0;
		$nblines = count($object->lines);
		for ($i = 0; $i < $nblines; $i++) {
			$up_excl_tax = $object->lines[$i]->subprice;
			$vat_rate = $object->lines[$i]->tva_tx;
			$up_incl_tax = $up_excl_tax * (1 + ($vat_rate / 100));
			$tmpwidth2 = dol_strlen(dol_string_nohtmltag(price($up_incl_tax, 0, $outputlangs)));
