...
if ($action == 'builddoc' && $permissiontoadd) {
	if (is_numeric(GETPOST('model', 'alpha'))) {
		...
		if (isset($conf->global->MAIN_MULTILANGS) && $conf->global->MAIN_MULTILANGS && empty($newlang) && GETPOST('lang_id', 'aZ09')) {
			$newlang = GETPOST('lang_id', 'aZ09');
		}
		if (isset($conf->global->MAIN_MULTILANGS) && $conf->global->MAIN_MULTILANGS && empty($newlang) && isset($object->thirdparty->default_lang)) {
			$newlang = $object->thirdparty->default_lang; // for proposal, order, invoice, ...
		}
		if (isset($conf->global->MAIN_MULTILANGS) && $conf->global->MAIN_MULTILANGS && empty($newlang) && isset($object->default_lang)) {
