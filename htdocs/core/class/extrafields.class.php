			...
			if (!getDolGlobalString('MAIN_EXTRAFIELDS_ENABLE_NEW_SELECT2')) {
				...
				if (is_array($param['options'])) {
					...
					if (!$filter_categorie) {
						...
						$sql .= ' ORDER BY '.implode(', ', $fields_label);  // not use $this->db->order here, because $fields_label can contain SQL functions like concat_ws which needs parameters separated by , - $this->db->order would break this
