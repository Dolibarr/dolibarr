...
class ExtraFields
{
	...
	public function showInputField($key, $value, $moreparam = '', $keysuffix = '', $keyprefix = '', $morecss = '', $objectid = 0, $extrafieldsobjectkey = '', $mode = 0)
	{
		...
		if (!empty($extrafieldsobjectkey)) {
			...
			$perms = dol_eval($this->attributes[$extrafieldsobjectkey]['perms'][$key], 1, 1, '0');
			$langfile = $this->attributes[$extrafieldsobjectkey]['langfile'][$key];
			$list = dol_eval($this->attributes[$extrafieldsobjectkey]['list'][$key], 1, 1, '0');
		...
		if (is_array($extralabels)) {
			...
			foreach ($extralabels as $key => $value) {
				...
				if (isset($this->attributes[$object->table_element]['enabled'][$key])) {	// 'enabled' is often a condition on module enabled or not
					$enabled = dol_eval($this->attributes[$object->table_element]['enabled'][$key], 1, 1, '0');
				}

				$visibility = 1;
				if (isset($this->attributes[$object->table_element]['list'][$key])) {		// 'list' is option for visibility
					$visibility = intval(dol_eval($this->attributes[$object->table_element]['list'][$key], 1, 1, '0'));
				}

				$perms = 1;
				if (isset($this->attributes[$object->table_element]['perms'][$key])) {
					$perms = dol_eval($this->attributes[$object->table_element]['perms'][$key], 1, 1, '0');
				}
				if (empty($enabled)
					|| (
