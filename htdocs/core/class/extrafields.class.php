...
class ExtraFields
{
	...
	public function showInputField($key, $value, $moreparam = '', $keysuffix = '', $keyprefix = '', $morecss = '', $object = 0, $extrafieldsobjectkey = '', $mode = 0)
	{
		...
		if (in_array($type, array('date'))) {
			...
			$objectdesc = $tmparray[0].":".$tmparray[1];// Example: 'ObjectName:classPath'					To not propagate any filter (selectForForms do ajax call and propagating SQL filter is blocked by some WAF). Also we should use the one into the definition in the ->fields of $elem if found.
