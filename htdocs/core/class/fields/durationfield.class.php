...
/**
 *    \file        htdocs/core/class/fields/durationfield.class.php
 *    \ingroup    core
 *    \brief      File of class to duration field
 */
...
class DurationField extends CommonField
{
	...
	{
		// Todo make filter with min / max ? or same as a number ?
		return $this->printInputField($fieldInfos, $key, $value, $keyPrefix, $keySuffix, $moreCss, $moreAttrib);
	}
	...
	public function printInputField($fieldInfos, $key, $value, $keyPrefix = '', $keySuffix = '', $moreCss = '', $moreAttrib = '')
	{
		...
		return self::$form->select_duration($htmlName, (int) $value, 0, 'text', 0, 1);
	}
	...
	{
		return !$this->isEmptyValue($fieldInfos, $value) ? convertSecondToTime((int) $value, 'allhourmin') : '';
	}
	...
	public function verifyFieldValue($fieldInfos, $key, $value)
	{
		...
		if ($result && !$this->isEmptyValue($fieldInfos, $value)) {
			if (!self::$validator->isDuration($value)) {
				return false;
			}
