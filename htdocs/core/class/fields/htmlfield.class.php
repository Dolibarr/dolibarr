...
/**
 *    \file        htdocs/core/class/fields/htmlfield.class.php
 *    \ingroup    core
 *    \brief      File of class to html field
 */

require_once DOL_DOCUMENT_ROOT . '/core/class/fields/commonfield.class.php';


// TODO same as text field ?

/**
 *    Class to html field
 */
class HtmlField extends CommonField
{
	/**
	...
	public function printOutputField($fieldInfos, $key, $value, $keyPrefix = '', $keySuffix = '', $moreCss = '', $moreAttrib = '')
	{
		if (!$this->isEmptyValue($fieldInfos, $value)) {
			if (!empty($fieldInfos->options)) {
				$value = str_replace(',', "\n", (string) $value);
			}

			return dol_htmlentitiesbr((string) $value);
		}
