...
/**
 *    \file        htdocs/core/class/fields/phonefield.class.php
 *    \ingroup    core
 *    \brief      File of class to phone field
 */
 ...
 */
class PhoneField extends CommonField
{
	/**
	...
	{
		return !$this->isEmptyValue($fieldInfos, $value) ? dol_print_phone((string) $value, '', 0, 0, '', '&nbsp;', 'phone') : '';
	}
	...
	public function verifyFieldValue($fieldInfos, $key, $value)
	{
		...
		if ($result && !$this->isEmptyValue($fieldInfos, $value)) {
			if (!self::$validator->isPhone($value)) {
				return false;
			}
