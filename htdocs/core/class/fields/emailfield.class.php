...
/**
 *    \file        htdocs/core/class/fields/emailfield.class.php
 *    \ingroup    core
 *    \brief      File of class to email field
 */
 ...
 */
class EmailField extends CommonField
{
	/**
	...
	{
		return !$this->isEmptyValue($fieldInfos, $value) ? dol_print_email((string) $value, 0, 0, 0, 64, 1, 1) : '';
	}
	...
	public function verifyFieldValue($fieldInfos, $key, $value)
	{
		...
		if ($result && !$this->isEmptyValue($fieldInfos, $value)) {
			if (!self::$validator->isEmail($value)) {
				return false;
			}
