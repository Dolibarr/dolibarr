...
/**
 *    \file        htdocs/core/class/fields/commongeofield.class.php
 *    \ingroup    core
 *    \brief      File of class to common geo field
 */
 ...
 */
class CommonGeoField extends CommonField
{
	/**
	...
	public function printInputField($fieldInfos, $key, $value, $keyPrefix = '', $keySuffix = '', $moreCss = '', $moreAttrib = '')
	{
		...
		return self::$form->inputGeoPoint($htmlName, (string) $value, $fieldInfos->type);
	}
	...
	{
		return !$this->isEmptyValue($fieldInfos, $value) ? self::$form->outputGeoPoint((string) $value, $fieldInfos->type) : '';
	}
	...
	{
		// Todo make the validator test for geo point
		return parent::verifyFieldValue($fieldInfos, $key, $value);
	}
