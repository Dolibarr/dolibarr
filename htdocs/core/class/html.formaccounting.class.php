...
class FormAccounting extends Form
{
	 ...
	 *   0 = native HTML select with full list loaded, with select2 (original behavior)
	 *   1 = free text input, no autocomplete, no list
	...
	public function select_auxaccount($selectid, $htmlname = 'account_num_aux', $showempty = 0, $morecss = 'minwidth100 maxwidth300 maxwidthonsmartphone', $usecache = '', $labelhtmlname = '', $selected_input_value = '')
	{
		...
		// MODE 1: free text input — no list, no autocomplete
		if ($auxaccount_mode == 1) {
			$out .= '<input type="text"'
				. ' class="'.dol_escape_htmltag($morecss).'"'
				. ' name="'.$htmlname.'"'
				. ' id="'.$htmlname.'"'
				. ' value="'.dol_escape_htmltag((string) $selectid).'" />';
			return $out;
		}

		// MODES 2/3/4: ajax — on-demand loading via endpoint
		// minLength: mode 2 → 1 char, mode 3 → 2 chars, mode 4 → 3 chars
		...
		// MODE 0 (or ajax disabled): native HTML select with full list + select2 — original behavior
		$aux_account = array();
