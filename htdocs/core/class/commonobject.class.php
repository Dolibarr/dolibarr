...
abstract class CommonObject
{
    ...
    function printOriginLine($line,$var)
    {
        ...
        if (($line->info_bits & 2) == 2)  // TODO Not sure this is used for source object
        {
            ...
            $this->tpl['label'].= $discount->getNameUrl(0,'discount');
        }
        else if (! empty($line->fk_product))
        {
            ...
            $this->tpl['label'].= $productstatic->getNameUrl(1);
        ...
        if (! empty($line->desc))
        {
            if ($line->desc == '(CREDIT_NOTE)')  // TODO Not sure this is used for source object
            {
                ...
                $this->tpl['description'] = $langs->transnoentities("DiscountFromCreditNote",$discount->getNameUrl(0));
            }
            elseif ($line->desc == '(DEPOSIT)')  // TODO Not sure this is used for source object
            {
                $discount=new DiscountAbsolute($this->db);
                $discount->fetch($line->fk_remise_except);
                $this->tpl['description'] = $langs->transnoentities("DiscountFromDeposit",$discount->getNameUrl(0));
            }
            else
            {
