...
class FormCompany extends Form
{
	...
	public function selectProspectCustomerType($selected, $htmlname = 'client', $htmlidname = 'customerprospect', $typeinput = 'form', $morecss = '', $allowempty = '')
	{
		...
		if (!$conf->customizeprospectcustomerfield->enabled) {
			...
			return $this->selectProspectCustomerTypeCustom($selected, $htmlname, $htmlidname, $typeinput, $morecss, $allowempty);
		}
	}

	/**
	 * Return a customizable HTML select for thirdparty type
	 * 
	 * @param int 		$selected 		Selected value
	 * @param string 	$htmlname 		HTML select name
	 * @param string 	$htmlidname 	HTML select id
	 ...
	 * @return string HTML string
	 */
	public function selectProspectCustomerTypeCustom($selected, $htmlname = 'client', $htmlidname = 'customerprospect', $morecss = '', $allowempty = '')
	{
		global $conf, $langs;
		...
		while ($obj = $this->db->fetch_object($req)) {
			if ($obj->active == 1) {
				$out .= '<option value="' . $obj->code . '"'.($selected == $obj->code ? ' selected' : '').'>'.$langs->trans($obj->label).'</option>';
			}
		}
