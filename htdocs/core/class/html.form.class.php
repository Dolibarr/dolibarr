...
class Form
{
	...
	public function editfieldkey($text, $htmlname, $preselected, $object, $perm, $typeofdata = 'string', $moreparam = '', $fieldrequired = 0, $notabletag = 0, $paramid = 'id', $help = '')
	{
		...
		if (!empty(getDolGlobalString("MAIN_USE_JQUERY_JEDITABLE")) && !preg_match('/^select;/', $typeofdata)) {
			if (!empty($perm)) {
	...
	public function editfieldval($text, $htmlname, $value, $object, $perm, $typeofdata = 'string', $editvalue = '', $extObject = null, $custommsg = null, $moreparam = '', $notabletag = 0, $formatfunc = '', $paramid = 'id')
	{
		...
		if (!empty(getDolGlobalString("MAIN_USE_JQUERY_JEDITABLE")) && !preg_match('/^select;|datehourpicker/', $typeofdata)) { // TODO add jquery timepicker and support select
	...
	public function widgetForTranslation($fieldname, $object, $perm, $typeofdata = 'string', $check = '', $morecss = '')
	{
		...
		if (!empty(getDolGlobalString("PDF_USE_ALSO_LANGUAGE_CODE"))) {
	...
	public function select_company($selected = '', $htmlname = 'socid', $filter = '', $showempty = '', $showtype = 0, $forcecombo = 0, $events = array(), $limit = 0, $morecss = 'minwidth100', $moreparam = '', $selected_input_value = '', $hidelabel = 1, $ajaxoptions = array(), $multiple = false, $excludeids = array())
	{
		...
		if (!empty($conf->use_javascript_ajax) && !empty(getDolGlobalString("COMPANY_USE_SEARCH_TO_SELECT")) && !$forcecombo) {
			...
			$out .= '<input type="text" class="'.$morecss.'" name="search_'.$htmlname.'" id="search_'.$htmlname.'" value="'.$selected_input_value.'"'.($placeholder ? ' placeholder="'.dol_escape_htmltag($placeholder).'"' : '').' '.(!empty(getDolGlobalString("THIRDPARTY_SEARCH_AUTOFOCUS")) ? 'autofocus' : '').' />';
			if ($hidelabel == 3) {
				$out .= img_picto($langs->trans("Search"), 'search');
			}
		} else {
	...
	public function select_thirdparty_list($selected = '', $htmlname = 'socid', $filter = '', $showempty = '', $showtype = 0, $forcecombo = 0, $events = array(), $filterkey = '', $outputmode = 0, $limit = 0, $morecss = 'minwidth100', $moreparam = '', $multiple = false, $excludeids = array())
	{
		...
		if (!empty(getDolGlobalString("COMPANY_SHOW_ADDRESS_SELECTLIST"))) {
		...
		if (!empty(getDolGlobalString("COMPANY_HIDE_INACTIVE_IN_COMBOBOX"))) {
			$sql .= " AND s.status <> 0";
		}
		if (!empty($excludeids)) {
		...
		if ($filterkey && $filterkey != '') {
			$sql .= " AND (";
			$prefix = empty(getDolGlobalString("COMPANY_DONOTSEARCH_ANYWHERE")) ? '%' : ''; // Can use index if COMPANY_DONOTSEARCH_ANYWHERE is on
			// For natural search
		...
		if ($resql) {
			...
			if (!empty(getDolGlobalString("COMPANY_USE_SEARCH_TO_SELECT"))) {
		...
		if ($resql) {
			...
			if ($num) {
				...
				while ($i < $num) {
					...
					if (!empty(getDolGlobalString("MAIN_SHOW_FACNUMBER_IN_DISCOUNT_LIST")) && !empty($obj->fk_facture_source)) {
	...
	public function selectcontacts($socid, $selected = '', $htmlname = 'contactid', $showempty = 0, $exclude = '', $limitto = '', $showfunction = 0, $moreclass = '', $options_only = false, $showsoc = 0, $forcecombo = 0, $events = array(), $moreparam = '', $htmlid = '', $multiple = false, $disableifempty = 0)
	{
		...
		if ($showsoc > 0 || !empty(getDolGlobalString("CONTACT_SHOW_EMAIL_PHONE_TOWN_SELECTLIST"))) {
		...
		if (!empty(getDolGlobalString("CONTACT_HIDE_INACTIVE_IN_COMBOBOX"))) {
			$sql .= " AND sp.statut <> 0";
		}
		...
		if ($resql) {
			...
			if ($num) {
				...
				while ($i < $num) {
					...
					$extendedInfos = '';
					if (!empty(getDolGlobalString("CONTACT_SHOW_EMAIL_PHONE_TOWN_SELECTLIST"))) {
						$extendedInfos = array();
	...
	public function select_dolusers($selected = '', $htmlname = 'userid', $show_empty = 0, $exclude = null, $disabled = 0, $include = '', $enableonly = '', $force_entity = '0', $maxlength = 0, $showstatus = 0, $morefilter = '', $show_every = 0, $enableonlytext = '', $morecss = '', $noactive = 0, $outputmode = 0, $multiple = false)
	{
		...
		if ((is_numeric($selected) && ($selected < -2 || empty($selected))) && empty(getDolGlobalString("SOCIETE_DISABLE_DEFAULT_SALESREPRESENTATIVE"))) {
			$selected = $user->id;
		}
		...
		if (!empty($conf->multicompany->enabled) && $conf->entity == 1 && $user->admin && !$user->entity) {
			...
			if (!empty(getDolGlobalString("MULTICOMPANY_TRANSVERSE_MODE"))) {
		...
		if (!empty(getDolGlobalString("USER_HIDE_INACTIVE_IN_COMBOBOX")) || $noactive) {
			$sql .= " AND u.statut <> 0";
		}
		if (!empty($morefilter)) {
			$sql .= " ".$morefilter;
		...
		if (empty(getDolGlobalString("MAIN_FIRSTNAME_NAME_POSITION"))) {	// MAIN_FIRSTNAME_NAME_POSITION is 0 means firstname+lastname
		...
		if ($resql) {
			...
			if ($num) {
				...
				while ($i < $num) {
					...
					$moreinfo = '';
					if (!empty(getDolGlobalString("MAIN_SHOW_LOGIN"))) {
					...
					if (!empty($conf->multicompany->enabled) && empty(getDolGlobalString("MULTICOMPANY_TRANSVERSE_MODE")) && $conf->entity == 1 && $user->admin && !$user->entity) {
						if (!$obj->entity) {
	...
	public function select_produits($selected = '', $htmlname = 'productid', $filtertype = '', $limit = 0, $price_level = 0, $status = 1, $finished = 2, $selected_input_value = '', $hidelabel = 0, $ajaxoptions = array(), $socid = 0, $showempty = '1', $forcecombo = 0, $morecss = '', $hidepriceinlabel = 0, $warehouseStatus = '', $selected_combinations = null, $nooutput = 0)
	{
		...
		if (!empty($conf->use_javascript_ajax) && !empty(getDolGlobalString("PRODUIT_USE_SEARCH_TO_SELECT"))) {
			$placeholder = '';
			...
			//Price by customer
			if (!empty(getDolGlobalString("PRODUIT_CUSTOMER_PRICES")) && !empty($socid)) {
				$urloption .= '&socid='.$socid;
			}
			$out .= ajax_autocompleter($selected, $htmlname, DOL_URL_ROOT.'/product/ajax/products.php', $urloption, getDolGlobalString("PRODUIT_USE_SEARCH_TO_SELECT"), 1, $ajaxoptions);
			...
			$out .= '<input type="text" class="minwidth100" name="search_'.$htmlname.'" id="search_'.$htmlname.'" value="'.$selected_input_value.'"'.$placeholder.' '.(!empty(getDolGlobalString("PRODUCT_SEARCH_AUTOFOCUS")) ? 'autofocus' : '').' />';
			if ($hidelabel == 3) {
				$out .= img_picto($langs->trans("Search"), 'search');
			}
		} else {
	...
	public function select_produits_list($selected = '', $htmlname = 'productid', $filtertype = '', $limit = 20, $price_level = 0, $filterkey = '', $status = 1, $finished = 2, $outputmode = 0, $socid = 0, $showempty = '1', $forcecombo = 0, $morecss = '', $hidepriceinlabel = 0, $warehouseStatus = '')
	{
		...
		if (!empty(getDolGlobalString("PRODUCT_SORT_BY_CATEGORY"))) {
			//Product category
		...
		// Price by quantity
		if (!empty(getDolGlobalString("PRODUIT_CUSTOMER_PRICES_BY_QTY")) || !empty(getDolGlobalString("PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES"))) {
			$sql .= ", (SELECT pp.rowid FROM ".MAIN_DB_PREFIX."product_price as pp WHERE pp.fk_product = p.rowid";
			if ($price_level >= 1 && !empty(getDolGlobalString("PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES"))) {
		...
		if (!empty(getDolGlobalString("MAIN_SEARCH_PRODUCT_BY_FOURN_REF"))) {
		...
		if (!empty(getDolGlobalString("MAIN_MULTILANGS"))) {
			...
			if (!empty(getDolGlobalString("PRODUIT_TEXTS_IN_THIRDPARTY_LANGUAGE")) && !empty($socid)) {
				require_once DOL_DOCUMENT_ROOT.'/societe/class/societe.class.php';
				$soc = new Societe($db);
		...
		if (!empty(getDolGlobalString("PRODUIT_ATTRIBUTES_HIDECHILD"))) {
		...
		if ($filterkey != '') {
			$sql .= ' AND (';
			$prefix = empty(getDolGlobalString("PRODUCT_DONOTSEARCH_ANYWHERE")) ? '%' : ''; // Can use index if PRODUCT_DONOTSEARCH_ANYWHERE is on
			// For natural search
			$scrit = explode(' ', $filterkey);
			$i = 0;
			if (count($scrit) > 1) {
			...
			foreach ($scrit as $crit) {
				...
				if (!empty(getDolGlobalString("PRODUCT_AJAX_SEARCH_ON_DESCRIPTION"))) {
		...
		if (!empty(getDolGlobalString("PRODUCT_SORT_BY_CATEGORY"))) {
			...
			//ASC OR DESC order
			(getDolGlobalString("PRODUCT_SORT_BY_CATEGORY") == 1) ? $sql .= "ASC" : $sql .= "DESC";
		} else {
		...
		if ($result) {
			...
			if (!$forcecombo) {
				include_once DOL_DOCUMENT_ROOT.'/core/lib/ajax.lib.php';
				$out .= ajax_combobox($htmlname, $events, getDolGlobalString("PRODUIT_USE_SEARCH_TO_SELECT"));
			}
			...
			// Do not use textifempty = ' ' or '&nbsp;' here, or search on key will search on ' key'.
			//if (! empty($conf->use_javascript_ajax) || $forcecombo) $textifempty='';
			if (!empty(getDolGlobalString("PRODUIT_USE_SEARCH_TO_SELECT"))) {
				if ($showempty && !is_numeric($showempty)) {
					$textifempty = $langs->trans($showempty);
				} else {
					$textifempty .= $langs->trans("All");
			...
			while ($num && $i < $num) {
				...
				if ((!empty(getDolGlobalString("PRODUIT_CUSTOMER_PRICES_BY_QTY")) || !empty(getDolGlobalString("PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES"))) && !empty($objp->price_by_qty) && $objp->price_by_qty == 1) { // Price by quantity will return many prices for the same product
	...
	protected function constructProductListOption(&$objp, &$opt, &$optJson, $price_level, $selected, $hidepriceinlabel = 0, $filterkey = '', $novirtualstock = 0)
	{
		...
		if ($outorigin && !empty(getDolGlobalString("PRODUCT_SHOW_ORIGIN_IN_COMBO"))) {
		...
		if (!empty(getDolGlobalString("PRODUCT_SHOW_DIMENSIONS_IN_COMBO"))) {
		...
		if (!empty($conf->stock->enabled) && isset($objp->stock) && ($objp->fk_product_type == Product::TYPE_PRODUCT || !empty(getDolGlobalString("STOCK_SUPPORTS_SERVICES")))) {
		...
		if (!empty(getDolGlobalString("PRODUIT_TEXTS_IN_THIRDPARTY_LANGUAGE"))) {
		...
		if (empty($hidepriceinlabel) && $price_level >= 1 && (!empty(getDolGlobalString("PRODUIT_MULTIPRICES")) || !empty(getDolGlobalString("PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES")))) {
		...
		// Price by quantity
		if (empty($hidepriceinlabel) && !empty($objp->quantity) && $objp->quantity >= 1 && (!empty(getDolGlobalString("PRODUIT_CUSTOMER_PRICES_BY_QTY")) || !empty(getDolGlobalString("PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES")))) {
			$found = 1;
		...
		// Price by customer
		if (empty($hidepriceinlabel) && !empty(getDolGlobalString("PRODUIT_CUSTOMER_PRICES"))) {
	...
	public function select_produits_fournisseurs_list($socid, $selected = '', $htmlname = 'productid', $filtertype = '', $filtre = '', $filterkey = '', $statut = -1, $outputmode = 0, $limit = 100, $alsoproductwithnosupplierprice = 0, $morecss = '', $showstockinlist = 0, $placeholder = '')
	{
		...
		if (!empty(getDolGlobalString("PRODUIT_FOURN_TEXTS"))) {
		...
		if ($result) {
			...
			if (!$num) {
				...
				while ($i < $num) {
					...
					if ($objp->quantity == 1) {
						$opt .= price($objp->fprice * (!empty(getDolGlobalString("DISPLAY_DISCOUNTED_SUPPLIER_PRICE")) ? (1 - $objp->remise_percent / 100) : 1), 1, $langs, 0, 0, -1, $conf->currency)."/";
					}
					...
					if ($objp->quantity > 1) {
						$opt .= " - ";
						$opt .= price($objp->unitprice * (!empty(getDolGlobalString("DISPLAY_DISCOUNTED_SUPPLIER_PRICE")) ? (1 - $objp->remise_percent / 100) : 1), 1, $langs, 0, 0, -1, $conf->currency)."/".$langs->trans("Unit");
					}
					if ($objp->duration) {
						$opt .= " - ".$objp->duration;
					}
	...
	public function select_conditions_paiements($selected = 0, $htmlname = 'condid', $filtertype = -1, $addempty = 0, $noinfoadmin = 0, $morecss = '')
	{
		...
		if (empty($selected) && !empty(getDolGlobalString("MAIN_DEFAULT_PAYMENT_TERM_ID"))) {
	...
	public function select_types_paiements($selected = '', $htmlname = 'paiementtype', $filtertype = '', $format = 0, $empty = 1, $noadmininfo = 0, $maxlength = 0, $active = 1, $morecss = '', $nooutput = 0)
	{
		...
		// Set default value if not already set by caller
		if (empty($selected) && !empty(getDolGlobalString("MAIN_DEFAULT_PAYMENT_TYPE_ID"))) {
	...
	public function form_contacts($page, $societe, $selected = '', $htmlname = 'contactid')
	{
		...
		if ($htmlname != "none") {
			...
			$num = $this->num;
			if ($num == 0) {
				$addcontact = (!empty(getDolGlobalString("SOCIETE_ADDRESSES_MANAGEMENT")) ? $langs->trans("AddContact") : $langs->trans("AddContactAddress"));
	...
	public function load_tva($htmlname = 'tauxtva', $selectedrate = '', $societe_vendeuse = '', $societe_acheteuse = '', $idprod = 0, $info_bits = 0, $type = '', $options_only = false, $mode = 0)
	{
		...
		if ($num > 0) {
			...
			if ($defaulttx < 0 || dol_strlen($defaulttx) == 0) {
				if (empty(getDolGlobalString("MAIN_VAT_DEFAULT_IF_AUTODETECT_FAILS"))) {
					$defaulttx = $this->cache_vatrates[$num - 1]['txtva'];
				} else {
					$defaulttx = (getDolGlobalString("MAIN_VAT_DEFAULT_IF_AUTODETECT_FAILS") == 'none' ? '' : getDolGlobalString("MAIN_VAT_DEFAULT_IF_AUTODETECT_FAILS"));
				}
			}
			...
			if (is_object($societe_vendeuse) && $societe_vendeuse->id == $mysoc->id && $societe_vendeuse->tva_assuj == "0") {
				...
				if (empty(getDolGlobalString("EXPENSEREPORT_OVERRIDE_VAT"))) {
			...
			foreach ($this->cache_vatrates as $rate) {
				...
				$return .= '>';
				//if (! empty(getDolGlobalString("MAIN_VAT_SHOW_POSITIVE_RATES")))
				if ($mysoc->country_code == 'IN' || !empty(getDolGlobalString("MAIN_VAT_LABEL_IS_POSITIVE_RATES"))) {
	...
	public function selectDate($set_time = '', $prefix = 're', $h = 0, $m = 0, $empty = 0, $form_name = "", $d = 1, $addnowlink = 0, $disabled = 0, $fullday = '', $addplusone = '', $adddateof = '', $openinghours = '', $stepminutes = 1, $labeladddateof = '', $placeholder = '', $gm = 'auto')
	{
		...
		if (preg_match('/^([0-9]+)\-([0-9]+)\-([0-9]+)\s?([0-9]+)?:?([0-9]+)?/', $set_time, $reg)) {	// deprecated usage
			...
			$smonth = '';
			$sday = '';
			$shour = !isset(getDolGlobalString("MAIN_DEFAULT_DATE_HOUR")) ? ($h == -1 ? '23' : '') : getDolGlobalString("MAIN_DEFAULT_DATE_HOUR");
			$smin = !isset(getDolGlobalString("MAIN_DEFAULT_DATE_MIN")) ? ($h == -1 ? '59' : '') : getDolGlobalString("MAIN_DEFAULT_DATE_MIN");
			$ssec = !isset(getDolGlobalString("MAIN_DEFAULT_DATE_SEC")) ? ($h == -1 ? '59' : '') : getDolGlobalString("MAIN_DEFAULT_DATE_SEC");
		}
		if ($h == 3) {
			$shour = '';
		}
		...
		$usecalendar = 'combo';
		if (!empty($conf->use_javascript_ajax) && (empty(getDolGlobalString("MAIN_POPUP_CALENDAR")) || getDolGlobalString("MAIN_POPUP_CALENDAR") != "none")) {
			$usecalendar = ((empty(getDolGlobalString("MAIN_POPUP_CALENDAR")) || getDolGlobalString("MAIN_POPUP_CALENDAR") == 'eldy') ? 'jquery' : getDolGlobalString("MAIN_POPUP_CALENDAR"));
		}
			...
			if ($usecalendar != 'combo') {
				...
				if ($usecalendar == "eldy") {
					...
					if (!$disabled) {
						...
						if (empty(getDolGlobalString("MAIN_POPUP_CALENDAR_ON_FOCUS"))) {
							$retstring .= "
	...
	public function selectTickets($selected = '', $htmlname = 'ticketid', $filtertype = '', $limit = 0, $status = 1, $selected_input_value = '', $hidelabel = 0, $ajaxoptions = array(), $socid = 0, $showempty = '1', $forcecombo = 0, $morecss = '', $selected_combinations = null, $nooutput = 0)
	{
		...
		if (!empty($conf->use_javascript_ajax) && !empty(getDolGlobalString("TICKET_USE_SEARCH_TO_SELECT"))) {
			$placeholder = '';
			...
			$out .= ajax_autocompleter($selected, $htmlname, DOL_URL_ROOT.'/ticket/ajax/tickets.php', $urloption, getDolGlobalString("PRODUIT_USE_SEARCH_TO_SELECT"), 1, $ajaxoptions);
	...
	public function selectTicketsList($selected = '', $htmlname = 'ticketid', $filtertype = '', $limit = 20, $filterkey = '', $status = 1, $outputmode = 0, $showempty = '1', $forcecombo = 0, $morecss = '')
	{
		...
		if ($filterkey != '') {
			$sql .= ' AND (';
			$prefix = empty(getDolGlobalString("TICKET_DONOTSEARCH_ANYWHERE")) ? '%' : ''; // Can use index if PRODUCT_DONOTSEARCH_ANYWHERE is on
			// For natural search
			$scrit = explode(' ', $filterkey);
			$i = 0;
		...
		if ($result) {
			...
			// Do not use textifempty = ' ' or '&nbsp;' here, or search on key will search on ' key'.
			//if (! empty($conf->use_javascript_ajax) || $forcecombo) $textifempty='';
			if (!empty(getDolGlobalString("TICKET_USE_SEARCH_TO_SELECT"))) {
	...
	public static function selectArrayAjax($htmlname, $url, $id = '', $moreparam = '', $moreparamtourl = '', $disabled = 0, $minimumInputLength = 1, $morecss = '', $callurlonselect = 0, $placeholder = '', $acceptdelayedhtml = 0)
	{
		...
		if (empty(getDolGlobalString("MAIN_USE_JQUERY_MULTISELECT")) && !defined('REQUIRE_JQUERY_MULTISELECT')) {
			return '';
		}
	...
	public static function multiselectarray($htmlname, $array, $selected = array(), $key_in_label = 0, $value_as_key = 0, $morecss = '', $translate = 0, $width = 0, $moreattrib = '', $elemtype = '', $placeholder = '', $addjscombo = -1)
	{
		...
		// Add code for jquery to use multiselect
		if (!empty($conf->use_javascript_ajax) && !empty(getDolGlobalString("MAIN_USE_JQUERY_MULTISELECT")) || defined('REQUIRE_JQUERY_MULTISELECT')) {
			$out .= "\n".'<!-- JS CODE TO ENABLE select for id '.$htmlname.', addjscombo='.$addjscombo.' -->';
			$out .= "\n".'<script>'."\n";
			if ($addjscombo == 1) {
				$tmpplugin = empty(getDolGlobalString("MAIN_USE_JQUERY_MULTISELECT")) ?constant('REQUIRE_JQUERY_MULTISELECT') : getDolGlobalString("MAIN_USE_JQUERY_MULTISELECT");
	...
	public function showLinkToObjectBlock($object, $restrictlinksto = array(), $excludelinksto = array())
	{
		...
		if (is_object($object->thirdparty) && !empty($object->thirdparty->id) && $object->thirdparty->id > 0) {
			...
			if (($object->thirdparty->parent > 0) && !empty(getDolGlobalString("THIRDPARTY_INCLUDE_PARENT_IN_LINKTO"))) {
				$listofidcompanytoscan .= ','.$object->thirdparty->parent;
			}
			if (($object->fk_project > 0) && !empty(getDolGlobalString("THIRDPARTY_INCLUDE_PROJECT_THIRDPARY_IN_LINKTO"))) {
		...
		foreach ($possiblelinks as $key => $possiblelink) {
			...
			if (!empty($possiblelink['perms']) && (empty($restrictlinksto) || in_array($key, $restrictlinksto)) && (empty($excludelinksto) || !in_array($key, $excludelinksto))) {
				...
				if (!empty(getDolGlobalString("MAIN_LINK_BY_REF_IN_LINKTO"))) {
				...
				if ($num > 0 || !empty(getDolGlobalString("MAIN_LINK_BY_REF_IN_LINKTO"))) {
	...
	public static function showphoto($modulepart, $object, $width = 100, $height = 0, $caneditfield = 0, $cssclass = 'photowithmargin', $imagesize = '', $addlinktofullsize = 1, $cache = 0, $forcecapture = '', $noexternsourceoverwrite = 0)
	{
		...
		if ($modulepart == 'societe') {
			...
			if (!empty(getDolGlobalString("MAIN_OLD_IMAGE_LINKS"))) {
		...
		if ($resql) {
			...
			if ($num) {
				...
				while ($i < $num) {
					...
					$out .= $obj->name;
					if (!empty($conf->multicompany->enabled) && empty(getDolGlobalString("MULTICOMPANY_TRANSVERSE_MODE")) && $conf->entity == 1) {
						$out .= " (".$obj->label.")";
					}
	...
	public function selectInvoice($socid = -1, $selected = '', $htmlname = 'invoiceid', $maxlength = 24, $option_only = 0, $show_empty = '1', $discard_closed = 0, $forcefocus = 0, $disabled = 0, $morecss = 'maxwidth500', $projectsListId = '', $showproject = 'all', $usertofilter = null)
	{
		...
		if (!empty(getDolGlobalString("PROJECT_HIDE_UNSELECTABLES"))) {
