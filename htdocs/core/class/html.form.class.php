...
class Form
{
	...
	/**
		 ...
		 * @param string 	$htmlname 		Name of select zone
		 * @param string 	$filtre 		To filter list. This parameter must not come from input of users
		 * @param int 		$showempty 		'' to not show empty line. Translation key to show an empty line. '1' show empty line with no text.
		 * @param string 	$moreattrib 	To add more attribute on select
		 * @param string 	$morecss 		More CSS
		 * @param int 		$nooutput 		1=Return string, do not send to output
		 * @return int|string              	If noouput=0: Return integer <0 if error, Num of bank account found if OK (0, 1, 2, ...), If nooutput=1: Return a HTML select string.
		 */
	public function selectTasks($selected = '', $htmlname = 'taskid', $filtre = '', $showempty = 0, $moreattrib = '', $morecss = '', $nooutput = 0)
	{
		// phpcs:enable
		global $langs, $conf;
		...
		$sql .= " FROM " . $this->db->prefix() . "projet_task";
		...
		dol_syslog(get_class($this) . "::select_tasks", LOG_DEBUG);
		$result = $this->db->query($sql);
		if ($result) {
			$num = $this->db->num_rows($result);
			$i = 0;
			if ($num) {
				...
				while ($i < $num) {
					...
					$out .= trim($obj->ref) . ", " . trim($obj->label);
					$out .= '</option>';
					$i++;
				}
				$out .= "</select>";
