...
class Form
{
	...
	public function showLinkToObjectBlock($object, $restrictlinksto = array(), $excludelinksto = array())
	{
		...
		if ($object->table_element == 'commande_fournisseur') {
			...
			$possiblelinks['order']['sql'] = "SELECT s.rowid as socid, s.nom as name, s.client, t.rowid, t.ref, t.rowid, '0' as total_ht FROM ".$this->db->prefix()."societe as s INNER JOIN ".$this->db->prefix().'commande as t ON t.fk_soc = s.rowid  WHERE t.entity IN ('.getEntity('commande').')';
		} elseif ($object->table_element == 'mrp_mo' || $object->table_element == 'commande') {
			$possiblelinks['order_supplier']['sql'] = "SELECT s.rowid as socid, s.nom as name, s.client, t.rowid, t.ref, t.ref_supplier, t.total_ht FROM ".$this->db->prefix()."societe as s, ".$this->db->prefix().'commande_fournisseur as t WHERE t.fk_soc = s.rowid AND t.entity IN ('.getEntity('supplier_order').')';
		}
