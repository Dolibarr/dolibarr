...
class Note extends CommonObject
{
	...
	public function fetchAll(&$notes, $objecttype, $objectid, $sortfield=null, $sortorder=null)
	{
		...
		$sql = "SELECT rowid, entity, datec, title, text, objecttype, fk_user_author FROM " . MAIN_DB_PREFIX . "note";
		...
		if ($resql)
		{
			...
			if ($num > 0)
			{
				while ($obj = $this->db->fetch_object($resql))
				{
					...
					$note->datec = $obj->datec;
					$note->title = $obj->title;
					$note->text = $obj->text;
					$note->fk_user_author = $obj->fk_user_author;
	...
	public function fetch($rowid=null)
	{
		...
		$sql = "SELECT rowid, entity, datec, title, text, objecttype, objectid, fk_user_author FROM " . MAIN_DB_PREFIX . "note";
		...
		if ($resql)
		{
			if($this->db->num_rows($resql) > 0)
			{
				...
				$this->datec = $obj->db->jdate($obj->datec);  
	 ...
	 *
	 *	@param	int		$rowid	Id of note to delete
