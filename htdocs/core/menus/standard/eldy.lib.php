...
function get_left_menu_products($mainmenu, &$newmenu, $usemenuhider = 1, $leftmenu = 'none', $type_user = 0)
{
	...
	if ($mainmenu == 'products') {
		...
		if (isModEnabled('service')) {
			...
			if (isModEnabled('Stock')) {
#				$newmenu->add("/product/reassort.php?type=1", $langs->trans("MenuStocks"), 1, $user->hasRight('service',  'read') && $user->hasRight('stock',  'lire')) && $conf->global->STOCK_SUPPORTS_SERVICES;
