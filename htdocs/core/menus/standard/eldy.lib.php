	...
	if ($mainmenu)
	{
		...
		if ($mainmenu == 'companies')
		{
			...
			if (! empty($conf->societe->enabled) && empty($conf->global->SOCIETE_DISABLE_PROSPECTS))
			{
				...
				*/
				$newmenu->add("/societe/card.php?leftmenu=prospects&amp;action=create&amp;client=2", $langs->trans("MenuNewProspect"), 2, $user->rights->societe->creer);
			...
			if (! empty($conf->societe->enabled) && empty($conf->global->SOCIETE_DISABLE_CUSTOMERS))
			{
				...
				$newmenu->add("/societe/card.php?leftmenu=customers&amp;action=create&amp;client=1", $langs->trans("MenuNewCustomer"), 2, $user->rights->societe->creer);
