	...
	if ($mainmenu)	// If this is empty, loading hard coded menu and loading personalised menu will fail
	{
		...
		if ($mainmenu == 'products')
		{
			...
			if (!empty($conf->product->enabled))
			{
				...
				{
					$newmenu->add("/product/reassort.php?leftmenu=product", $langs->trans("MenuStocks"), 1, $user->rights->produit->lire && $user->rights->stock->lire);
				}
				if (!empty($conf->productbatch->enabled))
				{
					$langs->load("stocks");
					$newmenu->add("/product/reassortlot.php?leftmenu=product", $langs->trans("StocksByLotSerial"), 1, $user->rights->produit->lire && $user->rights->stock->lire);
					$newmenu->add("/product/stock/productlot_list.php?leftmenu=product", $langs->trans("LotSerial"), 1, $user->rights->produit->lire && $user->rights->stock->lire);
				}
				if (!empty($conf->variants->enabled))
				{
					$newmenu->add("/variants/list.php?leftmenu=product", $langs->trans("VariantAttributes"), 1, $user->rights->produit->lire);
				}
				...
				{
					$newmenu->add("/product/stats/card.php?id=all&leftmenu=stat&type=0", $langs->trans("Statistics"), 1, $user->rights->produit->lire && $user->rights->propale->lire);
				}
