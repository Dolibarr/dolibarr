...
class DoliDBPgsql
{
    ...
    //! Database label
    public static $label='PostgreSQL';      // Label of manager
    //! Charset
    ...
    public static $versionmin=array(8,4,0);	// Version min database

    //! Resultset of last request
    private $_results;

    public $connected;               // 1 si connecte, 0 sinon
    public $database_selected;       // 1 si base selectionne, 0 sinon
    public $database_name;			//! Nom base selectionnee
    public $database_user;	   		//! Nom user base
    //! >=1 if a transaction is opened, 0 otherwise
    public $transaction_opened;
    public $lastquery;
    public $lastqueryerror;		// Ajout d'une variable en cas d'erreur

    public $unescapeslashquot=0;              // By default we do not force the unescape of \'. This is used only to process sql with mysql escaped data.
    public $standard_conforming_strings=1;    // Database has option standard_conforming_strings to on
     ...
     */
    public static function convertSQLFromMysql($line,$type='auto',$unescapeslashquot=0)
    {
