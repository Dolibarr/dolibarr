		...
		if ($action == 'edit' && ($canedituser || $caneditfield || $caneditpassword || ($user->id == $object->id))) {
			...
			if ($user->id == $object->id || !$user->admin) {
				...
				if ($object->socid > 0 && !($object->contact_id > 0)) {	// external user but no link to a contact
					...
					print img_picto('', 'contact').$form->selectcontacts(0, 0, 'contactid', 1, '', '', 1, '', false, 1, 0, array(), '', '', false, 0, true);
					if ($object->ldap_sid) {
						print ' ('.$langs->trans("DomainUser").')';
					}
					...
					print img_picto('', 'contact').$form->selectcontacts(0, $object->contact_id, 'contactid', 1, '', '', 1, '', false, 1, 0, array(), '', '', false, 0, true);
					if ($object->ldap_sid) {
						print ' ('.$langs->trans("DomainUser").')';
					}
