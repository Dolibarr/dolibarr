...
class Fichinter extends CommonObject
{
	 ...
	 *	@return		int					Return integer <0 if KO, >0 if OK
	 */
	public function setSign($user, $notrigger = 0)
	{
		global $conf;

		$error = 0;

		if ($this->statut == self::STATUS_SIGNED) {
			return 0;
		} else {
			$this->db->begin();
	...
	public function LibStatut($status, $mode = 0)
	{
		...
		if (empty($this->labelStatus) || empty($this->labelStatusShort)) {
			...
			$this->labelStatus[self::STATUS_SIGNED] = $langs->transnoentitiesnoconv('InterStatusSigned');
			...
			$this->labelStatusShort[self::STATUS_SIGNED] = $langs->transnoentitiesnoconv('InterStatusSigned');
		}
