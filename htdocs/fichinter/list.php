...
if(!empty($conf->global->FICHINTER_LIST_SHOW_INVOICES)) {
    $sql.= " LEFT JOIN (
    SELECT f.rowid fid,fac.facnumber facref,fac.rowid facid
    FROM llx_fichinter as f
    JOIN llx_element_element as ee ON ee.fk_source = f.rowid
    JOIN llx_facture as fac ON ee.fk_target = fac.rowid
    WHERE ee.sourcetype='fichinter' AND ee.targettype='facture'
) as inter ON inter.fid = f.rowid";
}
