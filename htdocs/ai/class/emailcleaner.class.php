...
class EmailCleaner
{
	...
	{
		$actionCode = dol_strtolower(trim((string) $action));
	...
	public function processEmailCollectorMessage($parameters, &$object)
	{
		...
		// Email sender metadata; the SQL plugin treats variables named 'from' as SQL clauses.
		...
		if ($msgid === '') {
			$msgid = sha1($subject."\n".$from."\n".dol_substr($rawBody, 0, 4000, '8bit'));
		}
		...
		if (dol_strlen($safeKey) > 180) $safeKey = dol_substr($safeKey, 0, 180);
		...
		} elseif (!getDolGlobalInt('AI_LOG_REQUESTS')) {
			$payload['ai_request_log_status'] = 'not_persisted_logging_disabled';
		} else {
	...
	private function insertAiRequestLogRow($entity, $actioncommId, $collectorId, $msgid, $rawBody, $cleanedText, $segments, $confidence, $engine, $model, $promptCode, $promptVersion, $contextProfileCode, $contextProfileVersion, $emailContext, $emailUnderstanding, $handoffPayload, $payload)
	{
		...
		if (!getDolGlobalInt('AI_LOG_REQUESTS')) return 0;
		...
		// Plain log text, escaped by ai_log_request() when building the INSERT.
		// @phan-suppress-next-line SqlInjection
		$queryText = 'EmailCleaner message '.$msgid;

		// Use the shared logger so the module logging policy also applies to collected emails.
		$context = array(
			'fk_actioncomm' => (int) $actioncommId,
	...
	public static function buildNoiseSummaryFromSegments($segments)
	{
		...
		foreach ($segments as $seg) {
			...
			$type = dol_strtolower(trim((string) ($seg['type'] ?? 'unknown')));
			...
			if ($text !== '' && count($out['samples'][$type]) < 2) {
				$out['samples'][$type][] = (dol_strlen($text) > 180 ? dol_substr($text, 0, 180) : $text);
			}
		}
		return $out;
	}
	...
	public static function collectAttachmentRows($attachments, $savedAttachments, $savedDir = '')
	{
		...
		if (is_array($savedAttachments)) {
			foreach ($savedAttachments as $sa) {
				...
				if ($n === '') continue;
				$savedByName[dol_strtolower($n)] = $sa;
			}
		}
		...
		if (is_array($attachments)) {
			foreach ($attachments as $att) {
				...
				$ext = dol_strtolower(pathinfo($name, PATHINFO_EXTENSION));
				$out[] = array(
					'name' => $name,
	...
	{
		$ext = dol_strtolower(pathinfo((string) $filename, PATHINFO_EXTENSION));
	...
	public function runEmailCleaner($subject, $from, $rawBody)
	{
		...
		if (dol_strlen($bodyForPrompt) > $maxInput) {
			$bodyForPrompt = dol_substr($bodyForPrompt, 0, $maxInput);
		}
		if ($redactionEnabled) {
		...
		if ($cleanBody !== '') {
			...
			if (is_array($lines)) {
				foreach ($lines as $line) {
					...
					if (dol_strlen($line) > 160) $line = dol_substr($line, 0, 160);
					$keyPoints[] = $line;
	...
	private static function sanitizeStringList($items, $maxItems, $maxLen)
	{
		...
		foreach ($items as $item) {
			...
			if ($item === '' || dol_strtolower($item) === 'null') continue;
			$item = preg_replace('/\s+/', ' ', $item);
			$item = trim((string) $item);
			if ($item === '') continue;
			if (dol_strlen($item) > $maxLen) {
				$item = dol_substr($item, 0, $maxLen);
				$item = rtrim($item, " \t\r\n,;:");
			}
			$key = dol_strtolower($item);
	...
	public static function extractHeaderFieldValue($header, $fieldName)
	{
		...
		$val = (string) ($m[1][0] ?? '');
		$offset = (int) ($m[1][1] ?? 0);
		$after = dol_substr($header, $offset + dol_strlen($val, '8bit'), null, '8bit');
		if ($after !== '') {
			$lines = preg_split("/\n/", $after);
			if (is_array($lines)) {
				foreach ($lines as $ln) {
	...
	public static function normalizeMessageId($messageId)
	{
		...
		return dol_strtolower($messageId);
	}
	...
	public static function buildMessageScopesBasic($body, $subject = '', $from = '', $date = '', $messageId = '', $inReplyTo = '', $references = array())
	{
		...
		if (dol_strlen($quotedContext) > 2400) $quotedContext = dol_substr($quotedContext, 0, 2400);
	...
	public static function extractQuotedMessageBlocksBasic($body, $maxBlocks = 16)
	{
		...
		$collecting = false;
		/**
		 * @param array<int,array<string,mixed>> $blocks Parsed quoted blocks
		 * @param array<int,string> $current Current block lines
		 * @param bool $collecting Whether a quoted block has started
		 * @return void
		 */
		$flush = function (array &$blocks, array &$current, bool $collecting) use ($maxBlocks) {
			...
			$blocks[] = array(
				...
				'text' => (dol_strlen($text) > 1200 ? dol_substr($text, 0, 1200) : $text),
			);
		};
		...
		foreach ($lines as $line) {
			...
			if ($isSeparator || $isWroteLine || ($isHeaderStart && !$collecting)) {
				$flush($blocks, $current, $collecting);
				$collecting = true;
	...
	private static function extractLeadingQuotedHeaderBasic($raw)
	{
		...
		$header = implode("\n", $headerLines);
		/**
		 * @param array<int,string> $names Accepted header names
		 * @return string
		 */
		$get = function (array $names) use ($header) {
	...
	public static function extractReplyOnlyTextBasic($body)
	{
		...
		foreach ($separators as $sep) {
			...
			if ($pos !== false && $pos > 0) {
				return trim(dol_substr($body, 0, $pos, '8bit'));
			}
		}
	...
	public static function extractQuotedThreadSnippetBasic($body, $maxLen = 1200)
	{
		...
		if ($startPos === false) {
			...
			$txt = trim(implode("\n", $quoteLines));
			return (dol_strlen($txt) > $maxLen ? dol_substr($txt, 0, $maxLen) : $txt);
		}

		$txt = trim(dol_substr($body, (int) $startPos, null, '8bit'));
		if (dol_strlen($txt) > $maxLen) $txt = dol_substr($txt, 0, $maxLen);
		return $txt;
	}
	...
	private static function fallbackCleanBody($rawBody)
	{
		...
		$clean = trim(implode("\n", $out));
		if ($clean === '') {
			$clean = trim(dol_substr($text, 0, 2000));
		}
	...
	private static function decodeJsonFromAiString($raw)
	{
		...
		$payload = dol_substr($raw, $first, $last - $first + 1, '8bit');
