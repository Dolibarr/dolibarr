...
class ToolEmailCleaner extends McpTool
{
	...
	private const OPERATION_CODE = 'email_cleaner';
	...
	public function getDefinitions(): array
	{
		...
		return [
			[
				...
				'description' => 'List EmailCollector AI cleaner runs attached to agenda events (technical metadata only). No business interpretation.',
				'inputSchema' => [
					'type' => 'object',
					'properties' => [
			...
			[
				...
				'description' => 'Get one EmailCollector AI cleaner run from agenda event AI metadata.',
				'inputSchema' => [
					'type' => 'object',
					'properties' => [
						'actioncomm_ai_id' => ['type' => 'integer', 'description' => 'AI metadata rowid.'],
						'cleaning_id' => ['type' => 'integer', 'description' => 'Backward-compatible alias for actioncomm_ai_id.'],
			...
			[
				...
				'description' => 'Get handoff payload generated by email cleaner from agenda event AI metadata. Evidence only, no decision.',
				'inputSchema' => [
					'type' => 'object',
					'properties' => [
						'actioncomm_ai_id' => ['type' => 'integer', 'description' => 'AI metadata rowid.'],
						'handoff_id' => ['type' => 'integer', 'description' => 'Backward-compatible alias for actioncomm_ai_id.'],
						'cleaning_id' => ['type' => 'integer', 'description' => 'Backward-compatible alias for actioncomm_ai_id.'],
					],
					'anyOf' => [
						['required' => ['actioncomm_ai_id']],
	...
	{
		if (!$this->isTableAvailable(self::TABLE_ACTIONCOMM_AI)) {
			return ['error' => 'Agenda event AI metadata table is not available.'];
		}
		...
		$sql = "SELECT rowid, fk_actioncomm, provider, model, prompt_code, prompt_version, confidence, status, privacy_profile_code, privacy_profile_version, input_metadata_json, date_creation";
		$sql .= " FROM ".MAIN_DB_PREFIX.self::TABLE_ACTIONCOMM_AI;
		$sql .= " WHERE entity = ".((int) $entity);
		$sql .= " AND operation_code = '".$this->db->escape(self::OPERATION_CODE)."'";

		if ($actioncommId > 0) {
			$sql .= " AND fk_actioncomm = ".((int) $actioncommId);
		}
		if ($collectorId > 0) {
			$sql .= " AND input_metadata_json LIKE '%\"collector_id\":".((int) $collectorId)."%'";
		}
		if ($messageId !== '') {
			$sql .= " AND input_metadata_json LIKE '%".$this->db->escape($messageId)."%'";
		}
		if ($contextProfileCode !== '') {
			$sql .= " AND privacy_profile_code = '".$this->db->escape($contextProfileCode)."'";
		}
		...
		while ($obj = $this->db->fetch_object($resql)) {
			$inputMetadata = $this->decodeJsonOrRaw((string) $obj->input_metadata_json);
			$items[] = array(
				'actioncomm_ai_id' => (int) $obj->rowid,
				...
				'cleaning_model' => (!empty($obj->model) ? (string) $obj->model : null),
				...
				'context_profile_code' => (!empty($obj->privacy_profile_code) ? (string) $obj->privacy_profile_code : null),
				'context_profile_version' => (!empty($obj->privacy_profile_version) ? (string) $obj->privacy_profile_version : null),
	...
	private function getEmailCleanerRun(array $args): array
	{
		...
		$entity = (int) (!empty($this->conf->entity) ? $this->conf->entity : 1);
		$actioncommAiId = (int) ($args['actioncomm_ai_id'] ?? ($args['cleaning_id'] ?? 0));
		$messageId = trim((string) ($args['message_id'] ?? ''));
		$collectorId = (int) ($args['collector_id'] ?? 0);

		$sql = "SELECT rowid, entity, fk_actioncomm, provider, model, prompt_code, prompt_version, input_hash, output_hash, security_hash, confidence, status, privacy_profile_code, privacy_profile_version, input_metadata_json, output_json, date_creation";
		$sql .= " FROM ".MAIN_DB_PREFIX.self::TABLE_ACTIONCOMM_AI;
		$sql .= " WHERE entity = ".((int) $entity);
		$sql .= " AND operation_code = '".$this->db->escape(self::OPERATION_CODE)."'";

		if ($actioncommAiId > 0) {
			$sql .= " AND rowid = ".((int) $actioncommAiId);
		} elseif ($messageId !== '') {
			$sql .= " AND input_metadata_json LIKE '%".$this->db->escape($messageId)."%'";
			if ($collectorId > 0) {
				$sql .= " AND input_metadata_json LIKE '%\"collector_id\":".((int) $collectorId)."%'";
			...
			return ['error' => "Missing selector: use 'actioncomm_ai_id', 'cleaning_id' or 'message_id'."];
		}
		...
		$inputMetadata = $this->decodeJsonOrRaw((string) $obj->input_metadata_json);
		$outputJson = $this->decodeJsonOrRaw((string) $obj->output_json);
		...
		return array(
			...
			'status' => (!empty($obj->status) ? (string) $obj->status : null),
			'input_metadata_json' => $inputMetadata,
	...
	private function getEmailThreadContext(array $args): array
	{
		...
		return array(
			'actioncomm_ai_id' => (int) $run['actioncomm_ai_id'],
	...
	{
		$actioncommAiId = (int) ($args['actioncomm_ai_id'] ?? ($args['handoff_id'] ?? ($args['cleaning_id'] ?? 0)));
		if ($actioncommAiId <= 0) {
			return ['error' => "Missing selector: use 'actioncomm_ai_id', 'handoff_id' or 'cleaning_id'."];
		}

		$run = $this->getEmailCleanerRun(array('actioncomm_ai_id' => $actioncommAiId));
		if (!empty($run['error'])) {
			return $run;
		}
		...
		return array(
			'actioncomm_ai_id' => (int) $run['actioncomm_ai_id'],
			'handoff_id' => (int) $run['actioncomm_ai_id'],
			'cleaning_id' => (int) $run['cleaning_id'],
			'actioncomm_id' => (int) $run['actioncomm_id'],
