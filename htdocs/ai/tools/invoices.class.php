...
class ToolInvoices extends McpTool
{
    ...
    private function searchInvoices($args)
    {
        ...
        if ($resql) {
            while ($r = $db->fetch_object($resql)) {
                ...
                if ($status !== 'draft' && $r->fk_statut == 0) continue;

                $ref = ($r->fk_statut == 0 ? "(PROV" . $r->rowid . ")" : $r->ref);

                // Calculate Status Label
                $statusLabel = "Unknown";
                if ($r->fk_statut == 0) $statusLabel = "Draft";
                elseif ($r->fk_statut == 1) $statusLabel = "Unpaid";
                elseif ($r->fk_statut == 2) $statusLabel = "Paid";
                elseif ($r->fk_statut == 3) $statusLabel = "Abandoned";
    ...
    private function validateInvoice($args)
    {
        ...
        if ($invoice->statut != 0) return ["error" => "Invoice is already validated."];
    ...
    private function payInvoice($args)
    {
        ...
        if (!$bank) return ["error" => "No active Bank account found to receive payment."];
    ...
    private function findCustomer($identifier)
    {
        ...
        if (preg_match('/^(?:socid|id)[:\s]+(\d+)$/i', $identifier, $m)) $identifier = $m[1];
        elseif (preg_match('/^(?:code|ref)[:\s]+(.+)$/i', $identifier, $m)) $identifier = $m[1];

        if (is_numeric($identifier)) {
            if ($customer->fetch((int)$identifier) > 0) return $customer;
        }
    ...
    private function findInvoice($identifier)
    {
        ...
        if (preg_match('/^\(?prov[-_]?(\d+)\)?$/i', $identifier, $matches)) {
            if ($invoice->fetch((int)$matches[1]) > 0) return $invoice;
        }
        if (is_numeric($identifier)) {
            if ($invoice->fetch((int)$identifier) > 0) return $invoice;
        }
        if ($invoice->fetch('', $identifier) > 0) return $invoice;
