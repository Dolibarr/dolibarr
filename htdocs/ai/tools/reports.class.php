...
class ToolReports extends McpTool
{
    ...
    /**
     * Resolves Thirdparty ID from ID or Name.
     */
        ...
        if ($resql) {
            while ($r = $db->fetch_object($resql)) {
                ...
                if ($r->source_type === 'Invoice') {
                    ...
                    if ($r->fk_statut == 2) $statusTxt = $langs->transnoentitiesnoconv("BillStatusPaid");
                    elseif ($r->fk_statut == 1) $statusTxt = $langs->transnoentitiesnoconv("BillStatusNotPaid");
                } elseif ($r->source_type === 'Order') {
                    $urlPath = "/commande/card.php?id=" . $r->rowid;
                    if ($r->fk_statut == 1) $statusTxt = $langs->transnoentitiesnoconv("StatusOrderValidated");
                    elseif ($r->fk_statut == 2) $statusTxt = $langs->transnoentitiesnoconv("StatusOrderOnProcess");
                    elseif ($r->fk_statut == 3) $statusTxt = $langs->transnoentitiesnoconv("StatusOrderDelivered");
                } elseif ($r->source_type === 'Proposal') {
                    $urlPath = "/comm/propal/card.php?id=" . $r->rowid;
                    if ($r->fk_statut == 1) $statusTxt = $langs->transnoentitiesnoconv("PropalStatusValidated");
                    elseif ($r->fk_statut == 2) $statusTxt = $langs->transnoentitiesnoconv("PropalStatusSigned");
                    elseif ($r->fk_statut == 3) $statusTxt = $langs->transnoentitiesnoconv("PropalStatusNotSigned");
                    elseif ($r->fk_statut == 4) $statusTxt = $langs->transnoentitiesnoconv("PropalStatusBilled");
                }
