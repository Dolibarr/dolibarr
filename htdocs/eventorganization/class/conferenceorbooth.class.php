...
class ConferenceOrBooth extends ActionComm
{
	...
	public function fetch($id, $ref = null, $ref_ext = '', $email_msgid = '', $loadresources = 1)
	{
		...
		if (!$this->isConferenceType()) {
			...
			$linkSubscription = $dolibarr_main_url_root.'/public/eventorganization/attendee_new.php?id='.urlencode((string) $id).'&type=conf';
			$secureKey = dol_hash(getDolGlobalString('EVENTORGANIZATION_SECUREKEY').'conferenceorbooth'.((int) $id), 'md5');
			$linkSubscription .= '&securekey='.urlencode($secureKey);
			...
			$this->pubregister = $linkSubscription;
		}
