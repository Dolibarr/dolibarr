...
/**
 *    \file       htdocs/eventorganization/attendee/card.php
...
if (empty($reshook)) {
	...
	$backurlforlist = DOL_URL_ROOT.'/eventorganization/attendee/list.php?withproject=1&fk_project='.((int) $fk_project);
	//} else {
	//	$backurlforlist = DOL_URL_ROOT.'/eventorganization/attendee/list.php';
	//}
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = DOL_URL_ROOT.'/eventorganization/attendee/card.php?fk_project='.((int) $fk_project).'&id='.($id > 0 ? $id : '__ID__').'&withproject=1';
			}
		}
	}
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/eventorganization/attendee/list.php', 1).'?restore_lastsearch_values=1'.$moreparam.'">'.$langs->trans("BackToList").'</a>';
