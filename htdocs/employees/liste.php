...
/**
 *      \file       htdocs/employees/liste.php
 *      \ingroup    employee
 *		  \brief      Page to list all employees of society
 */
...
llxHeader('',$langs->trans("Employee"),'EN:Module_Foundations|FR:Module_Adh&eacute;rents|ES:M&oacute;dulo_Miembros');

$now=dol_now();

$sql = "SELECT d.rowid, d.login, d.lastname, d.firstname,";
$sql.= " d.email, d.fk_employee_type as type_id, d.sex, d.statut,";
$sql.= " t.label as type";
$sql.= " FROM ".MAIN_DB_PREFIX."employee as d";
if (! empty($search_categ) || ! empty($catid)) $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX."categorie_employee as cm ON d.rowid = cm.fk_employee"; // We need this table joined to the select in order to filter by categ
$sql.= ", ".MAIN_DB_PREFIX."employee_type as t";
...
if ($sall)
{
        ...
        foreach ($scrit as $crit) {
            ...
            $sql.=" d.firstname LIKE '%".$sall."%' OR d.lastname LIKE '%".$sall."%'";
...
if ($resql)
{
	...
	{
		if ($statut == '-1,1') { $titre=$langs->trans("EmployeesListQualified"); }
		if ($statut == '-1')   { $titre=$langs->trans("EmployeesListToValid"); }
		if ($statut == '1' && ! $filter)    		{ $titre=$langs->trans("EmployeesListValid"); }
		if ($statut == '1' && $filter=='uptodate')  { $titre=$langs->trans("EmployeesListUpToDate"); }
		if ($statut == '1' && $filter=='outofdate')	{ $titre=$langs->trans("EmployeesListNotUpToDate"); }
		if ($statut == '0')    { $titre=$langs->trans("EmployeesListResiliated"); }
	}
	...
	if ($type > 0)
	{
		...
		$result=$employeetype->fetch($_REQUEST["type"]);
	...
	$listetype=$employeetypestatic->liste_array();
	...
	while ($i < $num && $i < $conf->liste_limit)
	{
		...
		$employeestatic->id=$objp->rowid;
		$employeestatic->ref=$objp->rowid;
		$employeestatic->lastname=$objp->lastname;
		$employeestatic->firstname=$objp->firstname;
		...
		// Ref
		print "<td>";
		print $employeestatic->getNomUrl(1);
		print "</td>\n";

		// Lastname
		print "<td><a href=\"fiche.php?rowid=$objp->rowid\">";
		print ((! empty($objp->lastname) || ! empty($objp->firstname)) ? dol_trunc($employeestatic->getFullName($langs)) : '');
		...
		print $employeetypestatic->getNomUrl(1,32);
		print '</td>';
		...
		// Statut
		print '<td class="nowrap">';
		print $employeestatic->LibStatut($objp->statut,$objp->cotisation,$datefin,2);
		print "</td>";
		...
		print '&nbsp;';
		if ($user->rights->employee->supprimer && $objp->statut == -1)
		{
