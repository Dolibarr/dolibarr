...
if (empty($reshook)) {
	...
	if ($action == 'builddocrib' && $permissiontoread) {
		...
		$moreparams = array(
			...
			'force_dir_output' => $conf->societe->multidir_output[$object->entity ?? 1].'/'.dol_sanitizeFileName((string) $object->id)
		);
	...
	$id = $socid;
	$upload_dir = $conf->societe->multidir_output[$object->entity ?? 1];
