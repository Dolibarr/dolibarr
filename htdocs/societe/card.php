...
if (empty($reshook)) {
	...
	if ((!GETPOST('getcustomercode') && !GETPOST('getsuppliercode')) && ($action == 'add' || $action == 'update') && $permissiontoadd) {
		...
		if (!$error) {
			...
			if ($action == 'update' && $permissiontoadd) {
				...
				// Logo/Photo save
				$dir     = $conf->societe->multidir_output[$object->entity ?? 1]."/".$object->id."/logos";
				$file_OK = is_uploaded_file($_FILES['photo']['tmp_name']);
	...
	$id = $socid;
	$upload_dir = !empty($conf->societe->multidir_output[$object->entity ?? 1]) ? $conf->societe->multidir_output[$object->entity ?? 1] : $conf->societe->dir_output;
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($canvasdisplayaction)) {
	...
	if ($action == 'create') {
		...
		$diroutput = $conf->societe->multidir_output[$object->entity ?? 1];
		$trackid = 'thi'.$object->id;
