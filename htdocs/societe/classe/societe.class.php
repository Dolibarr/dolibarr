...
class Societe extends CommonObject
{
	...
	{
        global $conf;
        $sql = "SELECT * FROM ".MAIN_DB_PREFIX."socpeople as sp WHERE sp.fk_soc =".$this->id;
        $allsp = $this->db->query($sql);
        $return = array();
		$count = 0;
		$sql = "SELECT DISTINCT 
			ctp.element, 
			p.ref as pref, 
			p.fk_statut as pstatut,
			p.rowid as prowid, 
			p.dateo as pdate, 
			p.title as pname, 
			CONCAT(sp.firstname,\" \",sp.lastname) as cname, 
			sp.rowid as crowid, 
			ctp.libelle as ctype 
			FROM ".MAIN_DB_PREFIX."socpeople as sp 
			INNER JOIN ".MAIN_DB_PREFIX."element_contact as ec 
			ON ec.fk_socpeople = sp.rowid 
			INNER JOIN ".MAIN_DB_PREFIX."c_type_contact as ctp 
			ON ctp.source = \"external\" AND ctp.element = \"project\" 
			AND ctp.rowid = ec.fk_c_type_contact 
			INNER JOIN ".MAIN_DB_PREFIX."projet as p 
			ON p.rowid = ec.element_id 
			WHERE";
		$now = date('Y-m-d',strtotime("-1year",time()));
		//prepare sql condtions
		while($row = $allsp->fetch_object()) 
		{
			$subs = substr($sql, -5, 5);
			if($subs == 'WHERE') 
			{
				$sql .= " (sp.rowid = ".$row->rowid." AND p.dateo > \"".$now."\")";

			} else 
			{
				$sql .= " OR (sp.rowid = ".$row->rowid." AND p.dateo > \"".$now."\")";
			}
		}
		$sql.= " ORDER BY prowid DESC";
			$last_proj = $this->db->query($sql);
            
			while($row = $last_proj->fetch_object())
			{
				$pline = new stdClass();
                $pline->ref = $row->pref;
				$pline->prowid = $row->prowid;
				$pline->pstatut = $row->pstatut;
                $pline->pdate = $row->pdate;
                $pline->title = $row->pname;
                $pline->crowid = $row->crowid;
                $pline->cname = $row->cname;
				$pline->ctype = $row->ctype;
				$return[$count] = $pline;  
				$count++;           
			}	
		return $return;
