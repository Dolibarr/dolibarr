...
if ($result) {
	while ($objp = $db->fetch_object($result)) {
		...
		if (isModEnabled('fournisseur') && $user->hasRight('fournisseur', 'lire') && empty($conf->global->SOCIETE_DISABLE_SUPPLIERS) && empty($conf->global->SOCIETE_DISABLE_SUPPLIERS_STATS) && $objp->fournisseur) {
...
if (!empty($conf->use_javascript_ajax) && ((round($third['prospect']) ? 1 : 0) + (round($third['customer']) ? 1 : 0) + (round($third['supplier']) ? 1 : 0) + (round($third['other']) ? 1 : 0) >= 2)) {
	...
	if (isModEnabled('fournisseur') && $user->hasRight('fournisseur', 'lire') && empty($conf->global->SOCIETE_DISABLE_SUPPLIERS) && empty($conf->global->SOCIETE_DISABLE_SUPPLIERS_STATS)) {
