...
if (empty($reshook)) {
	$backurlforlist = dolBuildUrl('/contrat/list.php');
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dolBuildUrl('/contrat/card.php', ['id'=> ((!empty($id) && $id > 0) ? $id : '__ID__')]);
			}
		}
	}
...
if ($action == 'create') {
	...
	if ($socid > 0) {
		...
		print ' <a href="'.dolBuildUrl('/societe/card.php', ['action' => 'create', 'customer' => 3, 'backtopage' => dolBuildUrl($_SERVER["PHP_SELF"], ['action' => 'create'])]).'"><span class="fa fa-plus-circle valignmiddle paddingleft" title="'.$langs->trans("AddThirdParty").'"></span></a>';
		print '</td>';
	}
	print '</tr>'."\n";
	...
	if (isModEnabled('project')) {
		...
		print ' &nbsp; <a href="' . dolBuildUrl('/projet/card.php', ['socid' => $soc->id, 'action' => 'create', 'status' => 1, 'backtopage' => dolBuildUrl($_SERVER["PHP_SELF"], ['action' => 'create', 'socid' => $soc->id])]) . '"><span class="fa fa-plus-circle valignmiddle" title="'.$langs->trans("AddProject").'"></span></a>';
		print "</td></tr>";
	}
	...
	if ($object->id > 0) {
		...
		if ($action != 'presend') {
			...
			$filedir = $conf->contrat->multidir_output[$object->entity ?? $conf->entity]."/".dol_sanitizeFileName($object->ref);
		...
		$diroutput = $conf->contrat->multidir_output[$object->entity ?? $conf->entity];
		$trackid = 'con'.$object->id;
