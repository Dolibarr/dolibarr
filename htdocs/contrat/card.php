	...
	if ($object->id > 0)
	{
		...
		while ($cursorline <= $nbofservices)
		{
			...
			if ($user->rights->contrat->activer && $action == 'activateline' && $object->lines[$cursorline - 1]->id == GETPOST('ligne'))
			{
				...
				print '<tr class="oddeven">';
				print '<td class="nohover">'.$langs->trans("Comment").'</td><td colspan="3" class="nohover" colspan="'.($conf->margin->enabled ? 4 : 3).'"><input type="text" class="minwidth300" name="comment" value="'.htmlentities($_POST["comment"],ENT_QUOTES, 'utf-8').'"></td>';
