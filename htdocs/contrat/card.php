	...
	if ($action == 'add' && $user->hasRight('contrat', 'creer')) {
		...
		if (!$cancel) {
			...
			if ($result < 0) {
				...
				$old_filedir = $conf->contrat->multidir_output[$object->entity ?? 1].'/'.dol_sanitizeFileName($old_ref);
				$new_filedir = $conf->contrat->multidir_output[$object->entity ?? 1].'/'.dol_sanitizeFileName($object->ref);
...
if ($action == 'create') {
	...
	if ($object->id > 0) {
		...
		if ($action != 'presend') {
			...
			$filedir = $conf->contrat->multidir_output[$object->entity ?? 1]."/".dol_sanitizeFileName($object->ref);
		...
		$diroutput = $conf->contrat->multidir_output[$object->entity ?? 1];
		$trackid = 'con'.$object->id;
