...
if (empty($reshook)) {
	...
	if ($action == 'confirm_active' && $confirm == 'yes' && $permissiontoactivate) {
		...
		$result = $object->setSignedStatus($user, CommonSignedObject::$SIGNED_STATUSES['STATUS_NO_SIGNATURE'], 0, 'CONTRACT_MODIFY');
		if ($result >= 0) {
			header('Location: ' . $_SERVER["PHP_SELF"] . '?id=' . $object->id);
			exit;
		} else {
		...
		if ($user->socid == 0 && $action != 'presend' && $action != 'editline') {
			...
			if (empty($reshook)) {
				...
				if ($object->status > Contrat::STATUS_DRAFT) {
					if ($object->signed_status != CommonSignedObject::$SIGNED_STATUSES['STATUS_SIGNED_ALL']) {
