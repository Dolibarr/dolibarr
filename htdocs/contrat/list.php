...
$arrayfields = array(
	...
	'c.note_public' => array('label' => 'NotePublic', 'checked' => '0', 'position' => 520, 'enabled' => (!getDolGlobalInt('MAIN_LIST_HIDE_PUBLIC_NOTES'))),
	'c.note_private' => array('label' => 'NotePrivate', 'checked' => '0', 'position' => 521, 'enabled' => (!getDolGlobalInt('MAIN_LIST_HIDE_PRIVATE_NOTES'))),
...
if ($search_note_public) {
	$sql .= " AND c.note_public LIKE '%".$db->escape($db->escapeforlike($search_note_public))."%'";
}
if ($search_note_private) {
	$sql .= " AND c.note_private LIKE '%".$db->escape($db->escapeforlike($search_note_private))."%'";
}
if ($search_email) {
...
if (!empty($arrayfields['c.note_public']['checked'])) {
	print_liste_field_titre($arrayfields['c.note_public']['label'], $_SERVER["PHP_SELF"], "c.note_public", "", $param, '', $sortfield, $sortorder, 'center nowrap ');
	$totalarray['nbfield']++;
}
if (!empty($arrayfields['c.note_private']['checked'])) {
	print_liste_field_titre($arrayfields['c.note_private']['label'], $_SERVER["PHP_SELF"], "c.note_private", "", $param, '', $sortfield, $sortorder, 'center nowrap ');
	$totalarray['nbfield']++;
}
if (!empty($arrayfields['status']['checked'])) {
