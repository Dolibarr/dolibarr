	...
	public function doCollectOneCollector($mode = 0)
	{
		...
		if (!$error && !empty($arrayofemail) && count($arrayofemail) > 0 && $connection !== false) {
			...
			foreach ($richarrayofemail as $tmpval) {
				// Guard the processing of each email: on PHP 8, mysqli throws on a query error
				// instead of returning false, so a single "poison" email (duplicate key, invalid
				// date, ...) used to abort the whole collector run before datelastresult and the
				// processing lock were updated. See the matching catch at the end of the loop.
				try {
					$iforemailloop++;
					...
					// One email failed hard (see the try at the top of the loop). Roll back just
					// this email, log it and continue, so the rest of the batch still drains and
					// the collector's processing lock is released. Do not bump $error: the other
					// emails are fine; the poison email is left in the mailbox and re-skipped.
					try {
						$this->db->rollback();
					} catch (Throwable $e2) {
						// transaction may already be closed; ignore
					}
					dol_syslog("EmailCollector::doCollectOneCollector email #".$iforemailloop." skipped due to exception: ".$e->getMessage(), LOG_ERR);
					continue;
				}
			}
