				...
				foreach ($this->actions as $operation) {
					...
					if ($operation['type'] == 'loadthirdparty' || $operation['type'] == 'loadandcreatethirdparty') {
						...
						if (empty($operation['actionparam'])) {
							...
							// Overwrite values with values extracted from source email
							$errorforthisaction = $this->overwritePropertiesOfObject($contact_static, $operation['actionparam'], $messagetext, $subject, $header);
							if ($errorforthisaction) {
								$errorforactions++;
							} else {
								...
								if ($result < 0) {
									...
									// Overwrite values with values extracted from source email
									$errorforthisaction = $this->overwritePropertiesOfObject($contactstatic, $operation['actionparam'], $messagetext, $subject, $header);
