...
/**
 *  \file       htdocs/mymodule/class/myobjectstats.class.php
 *  \ingroup    mymodule
 *  \brief      File of class to manage myobject statistics
 */
 ...
 */
class MyObjectStats extends Stats
{
	/**
	 ...
	 */
	public function __construct($db, $socid, $mode, $userid = 0, $categid = 0)
	{
		global $hookmanager;
		$this->db = $db;
	...
	public function getNbByYear()
	{
		...
		$sql = "SELECT date_format(c.date_creation, '%Y') as dm, COUNT(*) as nb, SUM(c.".$this->db->sanitize($this->field).")";
		$sql .= " FROM ".$this->db->sanitize($this->from, 0, 1, 1);
		if (empty($user->socid) && !$user->hasRight('societe', 'client', 'voir')) {
			$sql .= "  INNER JOIN ".MAIN_DB_PREFIX."societe_commerciaux as sc ON c.fk_soc = sc.fk_soc AND sc.fk_user = ".((int) $user->id);
		}
	...
	public function getAmountByMonth($year, $format = 0)
	{
		...
		$sql = "SELECT date_format(c.date_creation, '%m') as dm, SUM(c.".$this->db->sanitize($this->field).")";
		$sql .= " FROM ".$this->db->sanitize($this->from, 0, 1, 1);
		if (empty($user->socid) && !$user->hasRight('societe', 'client', 'voir')) {
			$sql .= "  INNER JOIN ".MAIN_DB_PREFIX."societe_commerciaux as sc ON c.fk_soc = sc.fk_soc AND sc.fk_user = ".((int) $user->id);
		}
	...
	public function getAverageByMonth($year)
	{
		...
		$sql = "SELECT date_format(c.date_creation, '%m') as dm, AVG(c.".$this->db->sanitize($this->field).")";
		$sql .= " FROM ".$this->db->sanitize($this->from, 0, 1, 1);
		if (empty($user->socid) && !$user->hasRight('societe', 'client', 'voir')) {
			$sql .= "  INNER JOIN ".MAIN_DB_PREFIX."societe_commerciaux as sc ON c.fk_soc = sc.fk_soc AND sc.fk_user = ".((int) $user->id);
		}
	...
	public function getAllByProduct($year, $limit = 10)
	{
		...
		$sql .= " AND c.date_creation BETWEEN '".$this->db->idate(dol_get_first_day($year, 1, false))."' AND '".$this->db->idate(dol_get_last_day($year, 12, false))."'";
