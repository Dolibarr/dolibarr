...
/**
 * \file    documentsfunctionality/admin/setup.php
 * \ingroup documentsfunctionality
 * \brief   DocumentsFunctionality setup page.
 */
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/documentsfunctionality/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=documentsfunctionality-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DOCUMENTSFUNCTIONALITY_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'DOCUMENTSFUNCTIONALITY_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$page_name = "DocumentsFunctionalitySetup";
...
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "documentsfunctionality@documentsfunctionality");
...
$moduledir = 'documentsfunctionality';
$myTmpObjects = array();
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'DOCUMENTSFUNCTIONALITY_'.strtoupper($myTmpObjectKey).'_ADDON';
