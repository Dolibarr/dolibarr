...
/**
 *  \file       htdocs/core/modules/facture/doc/pdf_ellax.modules.php
 *  \ingroup    facture
 *  \brief      File of class to generate customers invoices from ellax model
 */
...
/**
 *	Class to manage PDF invoice template ellax
 */
class pdf_elaax extends ModelePDFFactures
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "ELAAX";
		$this->description = $langs->trans('PDFEllaxDescription');
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				//QR Code oben rechts in der Ecke
				if ($qrcodestring) {
					...
					$pdf->write2DBarcode($qrcodestring, 'QRCODE,M', 180, 3, 17, 17, $styleQr, 'N');
				...
				if ($notetoshow) {
					...
					$pdf->SetFont('', '', $default_font_size - 1);
					$pdf->writeHTMLCell(190, 4, $this->posxdesc - 1, $tab_top - 12, dol_htmlentitiesbr($notetoshow), 0, "L");
					// Description
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if (!empty($conf->global->MAIN_PDF_DASH_BETWEEN_LINES) && $i < ($nblines - 1)) {
						$pdf->setPage($pageposafter);
						$pdf->SetLineStyle(array('width' => 0.2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(45, 44, 49)));
						//$pdf->SetDrawColor(190,190,200);
						$pdf->line($this->marge_gauche, $nexY, $this->page_largeur - $this->marge_droite, $nexY);
						$pdf->SetLineStyle(array('dash'=>'0.1'));
					}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		if (!empty($conf->global->DOC_SHOW_FIRST_SALES_REP)) {
			...
			if (count($arrayidcontact) > 0) {
				...
				$pdf->SetXY($posx, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell(100, 3, $langs->trans("SalesRepresentative") . " : " . $usertmp->getFullName($langs), '', 'L');
			}
		}
		...
		if ($showaddress) {
			...
			if (!empty($conf->global->MAIN_INVERT_SENDER_RECIPIENT)) {
				$posx = $this->page_largeur - $this->marge_droite - 60;
			}
			...
			if ($object->type == 0 && $objectidnext) {
				...
				$pdf->SetXY($posx + 2, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell(80, 3, $outputlangs->transnoentities("ReplacementByInvoice") . ' : ' . $outputlangs->convToOutputCharset($objectreplacing->ref),0, $ltrdirection);
			}
			if ($object->type == 1) {
				...
				$pdf->SetXY($posx + 2, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell(80, 3, $outputlangs->transnoentities("ReplacementInvoice") . ' : ' . $outputlangs->convToOutputCharset($objectreplaced->ref),0, $ltrdirection);
			}
			...
			$pdf->SetXY($posx + 2, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell(80, 3, $outputlangs->transnoentities("CorrectionInvoice").' : '.$outputlangs->convToOutputCharset($objectreplaced->ref),0, $ltrdirection);
			}
			...
			$pdf->MultiCell(80, 3, $title." : ".dol_print_date($object->date, "day", false, $outputlangs, true),0, $ltrdirection);

			if (!empty($conf->global->INVOICE_POINTOFTAX_DATE)) {
				$posy += 4;
				$pdf->SetXY($posx + 2, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell(80, 3, $outputlangs->transnoentities("DatePointOfTax")." : ".dol_print_date($object->date_pointoftax, "day", false, $outputlangs),0, $ltrdirection);
			}
			...
			if ($object->type != 2) {
				...
				$pdf->MultiCell(80, 3, $title." : ".dol_print_date($object->date_lim_reglement, "day", false, $outputlangs, true),0, $ltrdirection);
			}
			...
			if ($object->ref_client) {
				...
				$pdf->SetXY($posx + 2, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell(80, 3, $outputlangs->transnoentities("RefCustomer") . " : " . dol_trunc($outputlangs->convToOutputCharset($object->ref_client), 65),0, $ltrdirection);
			}
			...
			if (empty($conf->global->MAIN_PDF_NO_RECIPENT_FRAME)) {
				$pdf->SetTextColor(0, 0, 0);
				$pdf->SetFont('', '', $default_font_size - 3);
				$pdf->SetXY($posx + 3, $posy - 7);
				$pdf->MultiCell($widthrecbox, 5, $outputlangs->convToOutputCharset($this->emetteur->name) . " Am Baunsberg 1 / 34270 Schauenburg / info@elaax.net", 0, $ltrdirection);
				//$pdf->Rect($posx, $posy, $widthrecbox, $hautcadre);
			}
			...
			// überschrift (Rechnung: xy)
			$pdf->SetFont('', 'B', $default_font_size + 2);
			$nummer = "Rechnungsnummer:";
			$nummer .= ' ' . $outputlangs->convToOutputCharset($object->ref);
			if ($object->statut == $object::STATUS_DRAFT) {
				$pdf->SetTextColor(128, 0, 0);
				$nummer .= ' - ' . $outputlangs->transnoentities("NotValidated");
			}
			$pdf->SetXY($posx, 100);
			$pdf->MultiCell($w, 3, $nummer, '', 'L');
			$pdf->SetTextColor(0, 0, 60);
			$pdf->SetFont('', 'B', $default_font_size);
	...
	public function defineColumnField($object, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$this->cols['totalincltax'] = array(
			'rank' => $rank,
			'width' => 23, // in mm
			'status' => empty($conf->global->PDF_PROPAL_SHOW_PRICE_INCL_TAX) ?  true : false,
