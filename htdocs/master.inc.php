 ...
 * Copyright (C) 2005 	   Andre Cianfarani     <andre.cianfarani@acdeveloppement.net> 
 *
...
// La fonction clearstatcache ne doit pas etre appel�de mani�e globale car ralenti.
// Elle doit etre appel� uniquement par les pages qui ont besoin d'un cache fichier vid�
...
if ($result)
{
    ...
    while ($i < $numr)
    {
        ...
        $key=$objp->name;
        $value=$objp->value; // Pas de stripslashes (ne s'applique pas sur lecture base mais apr� POST quand get_magic_quotes_gpc()==1)
        define ("$key", $value);
...
$conf->produit->MultiPricesEnabled=defined("PRODUIT_MULTIPRICES")?PRODUIT_MULTIPRICES:0;
// Module service
 ...
 * Objet Societe qui contient carac de l'institution g�� par Dolibarr.
 */
