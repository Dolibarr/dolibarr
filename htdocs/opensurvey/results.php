...
if (GETPOST("boutonp") || GETPOST("boutonp.x") || GETPOST("boutonp_x")) {		// boutonp for chrome, boutonp.x for firefox
	if (GETPOST('nom')) {
		...
		if ($num_rows > 0) {
			...
			$sql .= " VALUES ('".$db->escape($nom)."', '".$db->escape($numsondage)."', '".$db->escape($nouveauchoix)."', '".$db->idate(time())."')";
			$resql = $db->query($sql);
			if (!$resql) {
				dol_print_error($db);
			}
...
if ($action == 'edit') {
	...
	if ($object->date_fin && $object->date_fin < time() && $object->status == Opensurveysondage::STATUS_VALIDATED) {
