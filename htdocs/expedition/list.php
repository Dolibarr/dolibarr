...
while ($i < $imaxinloop) {
	...
	if ($mode == 'kanban') {
		...
		if (!empty($arrayfields['e.ref']['checked'])) {
			...
			$filedir = ($conf->expedition->multidir_output[$object->entity ?? 1] ? $conf->expedition->multidir_output[$object->entity ?? 1] : $conf->expedition->dir_output).'/sending/'.get_exdir(0, 0, 0, 1, $object, '');
