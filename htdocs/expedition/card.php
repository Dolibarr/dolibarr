		...
		if ($object->fetch($origin_id)) {	// This include the fetch_lines
			...
			while ($indiceAsked < $numAsked) {
				...
				if (empty($reshook)) {
					...
					if (count($warehousePicking) == 1 || $conf->global->STOCK_ALLOW_NEGATIVE_TRANSFER || !($line->fk_product > 0) || !isModEnabled('stock')) {     // If warehouse was already selected or if product is not a predefined, we go into this part with no multiwarehouse selection
