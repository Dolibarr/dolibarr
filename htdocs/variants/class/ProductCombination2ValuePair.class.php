...
class ProductCombination2ValuePair
{
	...
	{
		$sql = "INSERT INTO " . $this->db->prefix() . "product_attribute_combination2val
		...
		if ($query) {
			$this->id = $this->db->last_insert_id($this->db->prefix() . 'product_attribute_combination2val');
	...
	public function fetchByFkCombination($fk_combination)
	{
        ...
        FROM " . $this->db->prefix() . "product_attribute c LEFT JOIN " . $this->db->prefix() . "product_attribute_combination2val c2v ON c.rowid = c2v.fk_prod_attr
	...
	{
		$sql = "DELETE FROM " . $this->db->prefix() . "product_attribute_combination2val WHERE fk_prod_combination = " . (int) $fk_combination;
