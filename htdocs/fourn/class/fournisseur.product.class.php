...
class ProductFournisseur extends Product
{
     ...
     *    @return	int								<0 if KO, >=0 if OK
     */
    function update_buyprice($qty, $buyprice, $user, $price_base_type, $fourn, $availability, $ref_fourn, $tva_tx, $charges=0, $remise_percent=0, $remise=0, $newnpr=0, $delai_livraison_jours=0)
    {
        global $conf;
		...
		if (empty($delai_livraison_jours)) $delai_livraison_jours=0;
        ...
        if ($this->product_fourn_price_id)
        {
			...
			$sql.= " charges = ".$charges.",";
			$sql.= " delai_livraison_jours = ".$delai_livraison_jours;
				...
				if ($resql)
		  		{
		            ...
		            $sql.= "datec, fk_product, fk_soc, ref_fourn, fk_user, price, quantity, remise_percent, remise, unitprice, tva_tx, charges, unitcharges, fk_availability, info_bits, entity, delai_livraison_jours)";
		            ...
		            $sql.= " ".$newnpr.",";
		            $sql.= $conf->entity.",";
		            $sql.= $this->delai_livraison_jours;
		            $sql.=")";
    ...
    function fetch_product_fournisseur_price($rowid, $ignore_expression = 0)
    {
        ...
        $sql.= " pfp.fk_soc, pfp.ref_fourn, pfp.fk_product, pfp.charges, pfp.unitcharges, pfp.fk_supplier_price_expression, pfp.delai_livraison_jours"; // , pfp.recuperableonly as fourn_tva_npr";  FIXME this field not exist in llx_product_fournisseur_price
        ...
        if ($resql)
        {
            ...
            if ($obj)
            {
				...
				$this->delai_livraison_jours	= $obj->delai_livraison_jours;
    ...
    function list_product_fournisseur_price($prodid, $sortfield='', $sortorder='')
    {
        ...
        $sql.= " pfp.price, pfp.quantity, pfp.unitprice, pfp.remise_percent, pfp.remise, pfp.tva_tx, pfp.fk_availability, pfp.charges, pfp.unitcharges, pfp.info_bits, pfp.delai_livraison_jours";
        $sql.= " FROM ".MAIN_DB_PREFIX."product_fournisseur_price as pfp";
        ...
        if ($resql)
        {
            ...
            while ($record = $this->db->fetch_array($resql))
            {
				...
				$prodfourn->delai_livraison_jours	= $record["delai_livraison_jours"];
                $prodfourn->id						= $prodid;
    ...
    function find_min_price_product_fournisseur($prodid, $qty=0)
    {
        ...
        $sql.= " pfp.remise, pfp.remise_percent, pfp.fk_supplier_price_expression, pfp.delai_livraison_jours";
            ...
            else
            {
                ...
                { 
                    ...
                    if ($fourn_unitprice < $min || $min == -1)
                    {
						...
						$this->delai_livraison_jours	= $record["delai_livraison_jours"];
