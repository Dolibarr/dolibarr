...
class SupplierInvoices extends DolibarrApi
{
    ...
    function addPayment($id, $datepaye, $paiementid, $closepaidinvoices, $accountid, $num_paiement='', $comment='', $chqemetteur='', $chqbank='') {
        ...
        if( ! DolibarrApi::_checkAccessToResource('facturefourn',$id)) {
                throw new RestException(401, 'Access not allowed for login '.DolibarrApiAccess::$user->login);
        }
        ...
        {
            $this->db->rollback();
            dol_syslog('Payment error : '.$paiement->error);
        ...
        if (! empty($conf->banque->enabled)) {
            ...
            $bank_line_id=$paiement->addPaymentToBank(DolibarrApiAccess::$user,'payment_supplier','(SupplierInvoicePayment)',$accountid,$chqemetteur,$chqbank);

            if ($bank_line_id < 0)
            {
                $this->db->rollback();
                dol_syslog('Add payment to bank error : '.$paiement->error);
