...
$originid  = (GETPOSTINT('originid') ?: GETPOSTINT('origin_id')); // For backward compatibility
$rank      = (GETPOSTINT('rank') > 0) ?: -1;

// PDF
$hidedetails = (GETPOSTINT('hidedetails') ?: (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_HIDE_DETAILS) ? 1 : 0));
$hidedesc = (GETPOSTINT('hidedesc') ?: (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_HIDE_DESC) ? 1 : 0));
$hideref = (GETPOSTINT('hideref') ?: (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_HIDE_REF) ? 1 : 0));
...
if (empty($reshook)) {
	...
	if ($action == 'set_incoterms' && $usercancreate) {
		$result = $object->setIncoterms(GETPOSTINT('incoterm_id'), GETPOST('location_incoterms', 'alpha'));
		if ($result < 0) {
			setEventMessages($object->error, $object->errors, 'errors');
		}
	}
	...
	if ($action == 'confirm_commande' && $confirm == 'yes' && $usercanorder) {
		...
		$result = $object->commande($user, GETPOST("datecommande"), GETPOSTINT("methode"), GETPOST('comment', 'alphanohtml'));
		if ($result > 0) {
			if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE)) {
				$outputlangs = $langs;
				$newlang = '';
...
if ($action == 'create') {
	...
	$form->select_types_paiements((GETPOSTISSET('mode_reglement_id') && GETPOSTINT('mode_reglement_id') != 0) ? GETPOSTINT('mode_reglement_id') : $mode_reglement_id, 'mode_reglement_id');
	print '</td></tr>';
