...
class mod_commande_fournisseur_orchidee extends ModeleNumRefSuppliersOrders
{
	...
	/**   \brief      Constructeur
   */
  function mod_commande_fournisseur_orchidee()
  {
    $this->nom = "Orchidee";
  }
...
function info()
    {
      ...
      $texte = $langs->trans('OrchideeNumRefModelDesc1')."<br>\n";
      ...
      $texte.= '<td align="right"><input type="text" class="flat" size="30" name="matrice" value="'.$conf->global->COMMANDE_FOURNISSEUR_NUM_MATRICE.'"></td>';
      ...
      $texte.= '<td align="right"><input type="text" class="flat" size="30" name="prefixcommande" value="'.$conf->global->COMMANDE_FOURNISSEUR_NUM_PREFIX.'"></td>';
      $texte.= '<td align="left"><input type="submit" class="button" value="'.$langs->trans("Modify").'" name="Button"></td>';
      ...
      $texte.= '<td align="right"><input type="text" class="flat" size="30" name="offset" value="'.$conf->global->COMMANDE_FOURNISSEUR_NUM_DELTA.'"></td>';
      $texte.= '<td align="left"><input type="submit" class="button" value="'.$langs->trans("Modify").'" name="Button"></td>';
      ...
      $texte.= '<td align="right">';
      $texte.= $form->selectyesno('numrestart',$conf->global->COMMANDE_FOURNISSEUR_NUM_RESTART_BEGIN_YEAR,1);
      $texte.= '</td><td align="left"><input type="submit" class="button" value="'.$langs->trans("Modify").'" name="Button"></td>';
    ...
    function getExample()
    {
    	...
    	$this->prefixorder     = $conf->global->COMMANDE_FOURNISSEUR_NUM_PREFIX;
      $this->ordernummatrice = $conf->global->COMMANDE_FOURNISSEUR_NUM_MATRICE;
        ...
        if ($this->ordernummatrice != '')
        {
        	...
        	$matricePrefix   = "PREF|COM"; // PREF : prefix libre (ex: CF pour commande fournisseur), COM : prefix du client
      ...
      $num = sprintf($arg,$conf->global->COMMANDE_FOURNISSEUR_NUM_DELTA?$conf->global->COMMANDE_FOURNISSEUR_NUM_DELTA:1);
	...
	*/
    function getNextValue($objsoc)
    {
        ...
        if ($conf->global->COMMANDE_FOURNISSEUR_NUM_RESTART_BEGIN_YEAR) $sql.= " WHERE ref like '${searchLast}%'";
        $resql=$db->query($sql);
        if ($resql)
        {
        ...
        if ($conf->global->COMMANDE_FOURNISSEUR_NUM_DELTA != '')
        {
        ...
        else if ($conf->global->COMMANDE_FOURNISSEUR_NUM_DELTA != '' && !eregi('^'.$searchLastWithPreviousYear.'',$lastyy))
        {
        	// on applique le delta une seule fois
        	$max=$conf->global->COMMANDE_FOURNISSEUR_NUM_DELTA?$conf->global->COMMANDE_FOURNISSEUR_NUM_DELTA-1:0;
        }
        else
        {
        	$max=0;
