...
if ($search_billed != '' && $search_billed >= 0) {
	$sql .= " AND cf.billed = ".$db->escape($search_billed);
}
if ($search_product_category > 0) {
...
if ($search_total_vat != '') {
	$sql .= natural_search('cf.tva', $search_total_vat, 1);
}
if ($search_total_ttc != '') {
...
if ($search_multicurrency_montant_vat != '') {
	$sql .= natural_search('cf.multicurrency_total_tva', $search_multicurrency_montant_vat, 1);
}
...
if ($resql) {
	...
	if (!empty($arrayfields['cf.date_delivery']['checked'])) {
		print_liste_field_titre($arrayfields['cf.date_delivery']['label'], $_SERVER["PHP_SELF"], 'cf.date_livraison', '', $param, '', $sortfield, $sortorder, 'center ');
	}
	if (!empty($arrayfields['cf.total_ht']['checked'])) {
		print_liste_field_titre($arrayfields['cf.total_ht']['label'], $_SERVER["PHP_SELF"], "cf.total_ht", "", $param, '', $sortfield, $sortorder, 'right ');
	}
	if (!empty($arrayfields['cf.total_vat']['checked'])) {
		print_liste_field_titre($arrayfields['cf.total_vat']['label'], $_SERVER["PHP_SELF"], "cf.tva", "", $param, '', $sortfield, $sortorder, 'right ');
	}
	if (!empty($arrayfields['cf.total_ttc']['checked'])) {
	...
	if (!empty($arrayfields['cf.multicurrency_total_vat']['checked'])) {
		print_liste_field_titre($arrayfields['cf.multicurrency_total_vat']['label'], $_SERVER['PHP_SELF'], 'cf.multicurrency_total_tva', '', $param, 'class="right"', $sortfield, $sortorder);
	}
	if (!empty($arrayfields['cf.multicurrency_total_ttc']['checked'])) {
	...
	while ($i < min($num, $limit)) {
		...
		if (!empty($arrayfields['cf.total_vat']['checked'])) {
			...
			if (!$i) {
				$totalarray['pos'][$totalarray['nbfield']] = 'cf.total_vat';
			}
