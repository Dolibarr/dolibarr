...
/**
 *   	\file       preopportunity_list.php
 *		\ingroup    preopportunity
 *		\brief      List page for preopportunity
 */
...
require_once __DIR__.'/class/preopportunity.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('preopportunity', 'preopportunity', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("preopportunity")) {
	accessforbidden('Module preopportunity not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'PreOpportunity';
	$objectlabel = 'PreOpportunity';
	$uploaddir = $conf->preopportunity->dir_output;
...
$title = $langs->trans("PreOpportunitys");
//$help_url = "EN:Module_PreOpportunity|FR:Module_PreOpportunity_FR|ES:Módulo_PreOpportunity";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/preopportunity/preopportunity_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-preopportunity page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/preopportunity/preopportunity_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendPreOpportunityRef";
$modelmail = "preopportunity";
$objecttmp = new PreOpportunity($db);
