	...
	if (in_array($val['type'], array('int', 'integer'))) {
		...
		print '$(document).ready(function () {
					$("#select'.$key.'").change(function() {
						document.formname.action.value="edit";
						document.formname.submit();
					});
		        });';
		print '</script>'."\n";
		$value = GETPOST($key, 'int');
		print $form->select_country((GETPOSTISSET($key) ? GETPOST($key) : $object->$key), $key, '0', 0, 'minwidth300 maxwidth500 widthcentpercentminusx', '', 'Select Country');
