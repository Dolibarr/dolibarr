...
foreach ($object->fields as $key => $val) {
	...
	if (empty($val['alwayseditable'])) {
		...
		if ($key == 'lang') {
			...
			} elseif(preg_match('/^country/', $val['type'])) {
                print $langs->transnoentitiesnoconv(getCountry($object->$key, 1));
            } elseif(preg_match('/^state/', $val['type'])) {
                print getState($object->$key, 0);
            } else {
