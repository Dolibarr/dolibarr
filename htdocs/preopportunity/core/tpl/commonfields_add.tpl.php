	...
	if (in_array($val['type'], array('int', 'integer'))) {
		...
		print '$(document).ready(function () {
					$("#select'.$key.'").change(function() {
						document.formname.action.value="create";
		...
		print $form->select_country((GETPOSTISSET($key) ? GETPOST($key) : $object->$key), $key, '', 0, 'minwidth300 maxwidth500 widthcentpercentminusx', '', 'Select Country');
	} elseif (preg_match('/^state/', $val['type'])) {
		$value = GETPOST($key, 'int');
		$countryref = $val['countryref'];
		print $object->select_state($object->$key, (GETPOSTISSET($countryref) ? GETPOST($countryref) : $object->$countryref), $key);
