...
/**
 *  \file       preopportunity_agenda.php
 *  \ingroup    preopportunity
 *  \brief      Tab of events on PreOpportunity
 */
...
$langs->loadLangs(array("preopportunity@preopportunity", "other"));
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->preopportunity->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('preopportunity', 'read');
	$permissiontoadd = $user->hasRight('preopportunity', 'write');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
}
...
if ($object->id > 0) {
	$title = $langs->trans("PreOpportunity")." - ".$langs->trans('Agenda');
	//$title = $object->ref." - ".$langs->trans("Agenda");
	$help_url = 'EN:Module_Agenda_En|DE:Modul_Terminplanung';

	llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-preopportunity page-card_agenda');
	...
	print dol_get_fiche_head($head, 'agenda', $langs->trans("PreOpportunity"), -1, $object->picto);
