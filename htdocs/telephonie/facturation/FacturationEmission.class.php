...
class FacturationEmission {
  ...
  function FacturationEmission($db)
  {
    $this->db = $db;
	...
	foreach ($contrats as $contrat)
	  {
		...
		if (sizeof($numlignes) > 0)
		  {
		    $this->facture_contrat($this->db, $user, $contrat, $numlignes, $datetime, $factures_prev, $factures_a_mailer); 
		  }
		
		if (sizeof($numlignes) > 0)
		  {
		    $this->_prelevements($this->db, $user, $factures_prev); 
		  }
		
		if (sizeof($numlignes) > 0)
		  {
		    //$this->_emails($this->db, $user, $contrat, $factures_a_mailer);
		  }
