		...
		if (empty($reshook)) {
			...
			if ($type != 1) {	// Nature, Weight and volume only applies to products and not to services
				if (!getDolGlobalString('PRODUCT_DISABLE_NATURE')) {
					...
					print $formproduct->selectProductNature('finished', (GETPOST('finished') ? GETPOST('finished') : (string) $object->finished));
					print '</td></tr>';
				}
			}
			...
			if (empty($reshook)) {
				...
				if (!$object->isService()) {
					if (!getDolGlobalString('PRODUCT_DISABLE_WEIGHT')) {
						...
						print $formproduct->selectMeasuringUnits("weight_units", "weight", GETPOSTISSET('weight_units') ? GETPOST('weight_units') : intval($object->weight_units), 0, 2);
						print '</td></tr>';
					}
					...
					if (!getDolGlobalString('PRODUCT_DISABLE_SIZE')) {
						...
						print $formproduct->selectMeasuringUnits("size_units", "size", GETPOSTISSET('size_units') ? GETPOST('size_units') : intval($object->length_units), 0, 2);
						print '</td></tr>';
					}
					if (!getDolGlobalString('PRODUCT_DISABLE_SURFACE')) {
						// Brut Surface
						...
						print $formproduct->selectMeasuringUnits("surface_units", "surface", GETPOSTISSET('surface_units') ? GETPOST('surface_units') : intval($object->surface_units), 0, 2);
						print '</td></tr>';
					}
					if (!getDolGlobalString('PRODUCT_DISABLE_VOLUME')) {
						// Brut Volume
						...
						print $formproduct->selectMeasuringUnits("volume_units", "volume", GETPOSTISSET('volume_units') ? GETPOST('volume_units') : intval($object->volume_units), 0, 2);
						print '</td></tr>';
					}
					...
					if (getDolGlobalString('PRODUCT_ADD_NET_MEASURE')) {
						...
						print $formproduct->selectMeasuringUnits("net_measure_units", "", GETPOSTISSET('net_measure_units') ? GETPOST('net_measure_units') : intval($object->net_measure_units), 0, 0);
						print '</td></tr>';
					}
				}
				// Units
