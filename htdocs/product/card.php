	...
	if ($action == 'create' && $usercancreate) {
		...
		if (isModEnabled('productbatch')) {
			...
			if ($status_batch !== '0') {
				...
				if ((!empty($conf->global->PRODUCTBATCH_LOT_USE_PRODUCT_MASKS) && getDolGlobalString('PRODUCTBATCH_LOT_ADDON') == 'mod_lot_advanced')
					|| (!empty($conf->global->PRODUCTBATCH_SN_USE_PRODUCT_MASKS) && getDolGlobalString('PRODUCTBATCH_SN_ADDON') == 'mod_sn_advanced')) {
					...
					if ($conf->global->PRODUCTBATCH_SN_USE_PRODUCT_MASKS && getDolGlobalString('PRODUCTBATCH_SN_ADDON') == 'mod_sn_advanced') {
						print '
										if (this.value == 2) {
											$("#field_mask, #mask_option").toggleClass("hideobject");
