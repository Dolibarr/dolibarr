	...
	if ($mode == 'kanban') {
		...
		if (getDolGlobalString('MAIN_CHECKBOX_LEFT_COLUMN')) {
			...
			if ($massactionbutton || $massaction) {   // If we are in select mode (massactionbutton defined) or if we have already selected and sent an action ($massaction) defined
				...
				if ($permissiontoadd){
					if (!getDolGlobalString('PRODUCT_EDIT_IN_MENU_NOT_IN_POPUP')) {
						$htmlname = 'product'.$obj->rowid;
						$urltoopen = '/product/card.php?id='.$obj->rowid.'&action=edit';
						// Add html code to add the edit button and go back
						$jsonclose = 'location.reload();';
						print dolButtonToOpenUrlInDialogPopup($htmlname, $langs->transnoentitiesnoconv("Product"), img_picto('', 'edit', 'class="pictofixedwidth"'), $urltoopen, '', '', '', $jsonclose);
					}else{
