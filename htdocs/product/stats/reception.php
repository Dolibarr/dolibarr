...
if ($id > 0 || !empty($ref)) {
	...
	if ($result > 0) {
		...
		if ($user->hasRight('reception', 'lire')) {
			...
			$sql .= " INNER JOIN ".MAIN_DB_PREFIX."commande_fournisseur_dispatch as cfd ON cfd.fk_reception = r.rowid";
			$sql .= " INNER JOIN ".MAIN_DB_PREFIX."commande_fournisseurdet as cfdet ON cfdet.rowid = cfd.fk_commandefourndet";
			if (empty($user->rights->societe->client->voir) && !$socid) {
