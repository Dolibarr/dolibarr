...
if ($action == 'del_default')
{
	...
	if ($ret > 0)
	{
        if ($conf->global->PRODUCT_ADDON_PDF_ODT == "$value") dolibarr_del_const($db, 'PRODUCT_ADDON_PDF_ODT',$conf->entity);
	}
	$res = true;
}
...
if ($action == 'setdoc')
{
	if (dolibarr_set_const($db, "PRODUCT_ADDON_PDF_ODT",$value,'chaine',0,'',$conf->entity))
	{
