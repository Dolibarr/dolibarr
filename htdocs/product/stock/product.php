...
if ($action == "correct_stock" && ! $cancel)
{
	...
	if (! empty($conf->productdluo->enabled))
	{
		...
		if ($product->hasdluo() && (! GETPOST("dluo")) && (! GETPOST("dlc")) && (! GETPOST("lot_number"))) {
			setEventMessage($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("atleast1dlcfield")), 'errors');
			$error++;
			$action='correction';
		}
	}
	...
	if (! $error)
	{
		...
		if (is_numeric(GETPOST("nbpiece")) && $id)
		{
			...
			if ($product->hasdluo()) {
				$d_dlc=dol_mktime(12, 0 , 0, $_POST['dlcmonth'], $_POST['dlcday'], $_POST['dlcyear']);
				$d_dluo=dol_mktime(12, 0 , 0, $_POST['dluomonth'], $_POST['dluoday'], $_POST['dluoyear']);
				$result=$product->correct_stock_dlc(
					$user,
					GETPOST("id_entrepot"),
					GETPOST("nbpiece"),
					GETPOST("mouvement"),
...
if ($id > 0 || $ref)
{
	...
	if ($result > 0)
	{
		...
		if ($conf->productdluo->enabled) {
			print '<tr><td>'.$langs->trans("Status").' ('.$langs->trans("DLUO").')</td><td>';
	...
	if ($action == "correction")
	{
		...
		if ((! empty($conf->productdluo->enabled)) && $product->hasdluo()) {
			print '<tr>';
			print '<td width="15%">'.$langs->trans("DLC").'</td><td width="15%">';
			$form->select_date('','dlc','','',1,"");
			print '</td>';
			print '<td width="15%">'.$langs->trans("lDLUO").'</td><td width="15%">';
			$form->select_date('','dluo','','',1,"");
			print '</td>';
			print '<td width="15%">'.$langs->trans("lot_number").'</td><td width="15%">';
			print '<input type="text" name="lot_number" size="40" value="'.GETPOST("lot_number").'">';
			print '</td></tr>';
		}
		print '</table>';
...
if (empty($action) && $product->id)
{
    ...
    if (($user->rights->stock->mouvement->creer) && !$product->hasdluo())
	{
...
print '</tr>';
if ( (! empty($conf->productdluo->enabled)) && $product->hasdluo()) {
	print '<tr class="liste_titre"><td width="10%"></td><td width="10%">'.$langs->trans("DLC").'</td>';
	print '<td align="right" width="10%">'.$langs->trans("lDLUO").'</td>';
	print '<td align="right" width="10%">'.$langs->trans("lot_number").'</td>';
...
if ($resql)
{
	...
	while ($i < $num)
	{
		...
		//DLC/DLUO Detail
		if ((! empty($conf->productdluo->enabled)) && $product->hasdluo()) {
			$details=ProductDluo::findAll($db,$obj->product_stock_id);
			if ($details<0) dol_print_error($db);
			foreach ($details as $pdluo) {
				print "\n".'<tr><td width="10%"></td><td width="10%" align="right">'. dol_print_date($pdluo->dlc,'day') .'</td>';
				print '<td align="right" width="10%">'. dol_print_date($pdluo->dluo,'day') .'</td>';
				print '<td align="right" width="10%">'.$pdluo->lot.'</td>';
