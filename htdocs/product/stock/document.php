...
$upload_dir = $conf->stock->multidir_output[empty($object->entity) ? 1 : $object->entity].'/'.dol_sanitizeFileName($object->ref);
...
$hookmanager->initHooks(array('stockdocument', 'globalcard'));
...
if ($id > 0 || !empty($ref)) {
	if ($object->fetch($id, $ref) > 0) {
		...
		$upload_dir = $conf->stock->multidir_output[$object->entity ?? $conf->entity].'/'.dol_sanitizeFileName($object->ref);

		$head = stock_prepare_head($object);
		print dol_get_fiche_head($head, 'documents', $langs->trans('Warehouse'), -1, $object->picto);
		...
		// Warehouse content
		$linkback = '<a href="'.DOL_URL_ROOT.'/product/stock/list.php?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
