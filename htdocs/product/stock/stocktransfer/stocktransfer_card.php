	...
	if ($permissiontoadd) {
		...
		if ($action == 'confirm_destock' && $confirm == 'yes' && $object->status == $object::STATUS_VALIDATED) {
			...
			if (empty($error)) {
				...
				$object->date_reelle_depart = time();
				$object->update($user);
		...
		if ($action == 'confirm_addstock' && $confirm == 'yes' && $object->status == $object::STATUS_TRANSFERED) {
			...
			if (empty($error)) {
				...
				$object->date_reelle_arrivee = time();
				$object->update($user);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action == 'clone') {
		...
		$formquestion = array(	'text' => '',
			...
			1 => array('type' => 'text', 'name' => 'inventorycode', 'label' => $langs->trans("InventoryCode"), 'value' => dol_print_date(time(), '%y%m%d%H%M%S'), 'size' => 25)
		);
