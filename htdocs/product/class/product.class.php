...
class Product extends CommonObject
{
    ...
    //! Ref
    public $ref;
    public $libelle;            // TODO deprecated
    public $label;
    public $description;
    ...
    public $price_min;         // Minimum price net
    public $price_min_ttc;     // Minimum price with tax
    ...
    public $customcode;       // Customs code
    public $country_id;       // Country origin id
    public $country_code;     // Country origin code (US, FR, ...)
    ...
    //! barcode
    public $barcode;               // value
    public $barcode_type;          // id
    public $barcode_type_code;     // code (loaded by fetch_barcode)
    public $barcode_type_label;    // label (loaded by fetch_barcode)
    public $barcode_type_coder;    // coder (loaded by fetch_barcode)
     ...
     *	@return		int					<0 if KO, >0 if OK
     */
    public function _log_price($user,$level=0)
    {
        $now=dol_now();
     ...
     */
    public function get_buyprice($prodfournprice,$qty,$product_id=0,$fourn_ref=0)
    {
        $result = 0;
     ...
     * 	@return		int						<0 if KO, >0 if OK
     */
    public function updatePrice($newprice, $newpricebase, $user, $newvat='',$newminprice='', $level=0, $newnpr=0, $newpsq=0)
    {
        global $conf,$langs;
     ...
     *  @return   array       			Tableau des stats
     */
    public function load_stats_commande($socid=0,$filtrestatut='')
    {
        global $conf,$user;
     ...
     *  @return   array       				Tableau des stats
     */
    public function load_stats_commande_fournisseur($socid=0,$filtrestatut='')
    {
        global $conf,$user;
     ...
     *  @return   array       			Tableau des stats
     */
    public function load_stats_sending($socid=0,$filtrestatut='')
    {
        global $conf,$user;
     ...
     */
    public function _get_stats($sql,$mode)
    {
        $resql = $this->db->query($sql);
        if ($resql) {
     ...
     * 	@return   	array       		<0 if KO, result[month]=array(valuex,valuey) where month is 0 to 11
     */
    public function get_nb_vente($socid,$mode)
    {
        global $conf;
        global $user;
     ...
     * 	@return   	array       		<0 if KO, result[month]=array(valuex,valuey) where month is 0 to 11
     */
    public function get_nb_achat($socid,$mode)
    {
        global $conf;
        global $user;
     ...
     * 	@return   	array       		<0 if KO, result[month]=array(valuex,valuey) where month is 0 to 11
     */
    public function get_nb_propal($socid,$mode)
    {
        global $conf;
        global $user;
     ...
     * 	@return   	array       		<0 if KO, result[month]=array(valuex,valuey) where month is 0 to 11
     */
    public function get_nb_order($socid,$mode)
    {
        global $conf, $user;
     ...
     *  @return 	void
     */
    public function fetch_prod_arbo($prod, $compl_path="", $multiply=1, $level=1)
    {
        global $conf,$langs;
     ...
     *  @return void
     */
    public function fetch_prods($prod)
    {
        $this->res;
     ...
     *  @return     array       		Prod
     */
    public function getChildsArbo($id)
    {
     ...
     * 	@return     int     				<0 if KO, >0 if OK
     */
    public function correct_stock($user, $id_entrepot, $nbpiece, $movement, $label='', $price=0)
    {
        if ($id_entrepot) {
            $this->db->begin();
     ...
     *  @return	void
     */
    public function add_photo_web($sdir, $file)
    {
     ...
     */
    public function show_photos($sdir,$size=0,$nbmax=0,$nbbyrow=5,$showfilename=0,$showaction=0,$maxHeight=120,$maxWidth=160)
    {
        global $conf,$user,$langs;
