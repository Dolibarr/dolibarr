		...
		foreach($ents as $key => $val)
		{
			...
			if ($nbToBuild > 0)
			{
				...
				if ($nbToBuild > $nbmaxfabricable)
				{
					...
					$error++;
					$action = 'build';
					$mesg='<div class="error">'.$entrepot->label." ".$entrepot->lieu." (".$entrepot->cp.") :".$langs->trans("ErrorNotEnoughtComponentToBuild").'</div>';
				}
				else
				{
		...
		if ($action == 'build')
		{
			...
			if (count($ents) > 0)
			{
				...
				foreach($ents as $key => $val)
				{
					...
					$fabricable=$object->getNbProductBuildable($key, $id);
					print "<tr><td>";
					$entrepot->fetch($key);
					print $entrepot->label." ".$entrepot->lieu." (".$entrepot->cp.")</td>";
					print '<td align=right ><input style="text-align:right;" type="text" name="nbToBuild'.$key.'" size=5 value="'.$fabricable.'">';
					print '</td></tr>';
				}
			}
			print '<tr>';
...
if ($action == '' && $bproduit)
{
	...
	if ($user->rights->produit->creer || $user->rights->service->creer)
	{	// we build only if we manage store 
		if ($conf->stock->enabled)
