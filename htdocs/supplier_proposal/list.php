...
if ($search_montant_vat != '') {
	$sql .= natural_search("sp.tva", $search_montant_vat, 1);
}
if ($search_montant_ttc != '') {
	$sql .= natural_search("sp.total", $search_montant_ttc, 1);
}
...
if ($resql) {
	...
	if (!empty($arrayfields['sp.total_vat']['checked'])) {
		print_liste_field_titre($arrayfields['sp.total_vat']['label'], $_SERVER["PHP_SELF"], 'sp.total_vat', '', $param, '', $sortfield, $sortorder, 'right ');
	}
	if (!empty($arrayfields['sp.total_ttc']['checked'])) {
	...
	while ($i < min($num, $limit)) {
		...
		if (!empty($arrayfields['sp.total_vat']['checked'])) {
			...
			if (!$i) {
				$totalarray['pos'][$totalarray['nbfield']] = 'sp.total_vat';
			}
