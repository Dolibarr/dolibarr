...
if ($id > 0 || !empty($ref)) {
	...
	if ($object->fetch($id, $ref)) {
		...
		if ($societe->fetch($object->socid)) {
			...
			$morehtmlref .= $form->editfieldval("RefSupplier", 'ref_supplier', $object->ref_supplier, $object, $user->rights->fournisseur->commande->creer, 'string'.(isset($conf->global->THIRDPARTY_REF_INPUT_SIZE) ? ':' . getDolGlobalString('THIRDPARTY_REF_INPUT_SIZE') : ''), '', null, null, '', 1);
			$morehtmlref .= '<br>';
			// Thirdparty
