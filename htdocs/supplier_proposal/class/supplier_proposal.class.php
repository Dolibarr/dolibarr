...
class SupplierProposal extends CommonObject
{
	...
	public function getTooltipContentArray($params)
	{
		...
		if (!empty($this->total_tva)) {
			$datas['amount_vat'] = '<br><b>'.$langs->trans('VAT').':</b> '.price($this->total_tva, 0, $langs, 0, -1, -1, getDolCurrency());
		}
		if (!empty($this->total_ttc)) {
			$datas['amount_ttc'] = '<br><b>'.$langs->trans('AmountTTC').':</b> '.price($this->total_ttc, 0, $langs, 0, -1, -1, getDolCurrency());
		}
	...
	public function getNomUrl($withpicto = 0, $option = '', $get_params = '', $notooltip = 0, $save_lastsearch_value = -1, $addlinktonotes = 0)
	{
		...
		if ($addlinktonotes) {
			...
			if ($txttoshow) {
				...
				$result .= '<a href="'.DOL_URL_ROOT.'/core/note.php?element=supplier_proposal&id='.$this->id.'" class="classfortooltip" title="'.dol_escape_htmltag($notetoshow).'">';
