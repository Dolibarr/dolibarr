...
if ($user->rights->adherent->cotisation->creer && $action == 'subscription' && ! $_POST["cancel"])
{
	...
	if (! $error && $action=='subscription')
	{
		...
		// Create subscription
		$crowid=$object->subscription($datecotisation, $subscription, $accountid, $operation, $label, $num_chq, $emetteur_nom, $emetteur_banque, $datesubend);
		if ($crowid <= 0)
		{
			$error++;
			$errmsg=$object->error;
		...
		if (! $error)
		{
			...
			if ($option == 'bankdirect' && $accountid)
			{
				...
				if ($insertid > 0)
				{
					...
					if ($inserturlid > 0)
					{
						// Met a jour la table subscription
						$sql ="UPDATE ".MAIN_DB_PREFIX."subscription SET fk_bank=".$insertid;
						$sql.=" WHERE rowid=".$crowid;

						dol_syslog("card_subscriptions::subscription", LOG_DEBUG);
						$resql = $db->query($sql);
						if (! $resql)
						{
							$error++;
			...
			if (($option == 'bankviainvoice' && $accountid) || $option == 'invoiceonly')
			{
				...
				if (! $error)
				{
					...
					$result=$invoice->addline($label,0,1,$vattouse,0,0,$idprodsubscription,0,$datecotisation,$datesubend,0,0,'','TTC',$subscription,1);
					if ($result <= 0)
					{
						$errmsg=$invoice->error;
						$error++;
