<?php
/* Copyright (C) 2026 direct copy from htdocs/societe/ajax/company.php
...
/**
 *       \file       htdocs/adherents/ajax/member.php
 *       \brief      File to return Ajax response on member list request. Used by the combo list of member.
...
// Security check
restrictedArea($user, 'member', $object->id, '&member');
...
if (!empty($action) && $action == 'fetch' && !empty($id) && $user->hasRight('member', 'lire')) {
	require_once DOL_DOCUMENT_ROOT.'/adherents/class/adherent.class.php';
	...
	if (dol_strlen($filter) < getDolGlobalInt('MEMBER_USE_SEARCH_TO_SELECT')) {
		httponly_accessforbidden('Call the adherents/ajax/company file with a too short filter', 400);
	}

	$arrayresult = $form->select_member_list('0', (string) $htmlname, $filter, 1, $showtype, 0, array(), $searchkey, $outjson, $limit, 'minwidth100', '', false, $excludeids, $showcode);
