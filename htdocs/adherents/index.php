...
if ($conf->use_javascript_ajax)
{
    ...
    {
        $datalabels[]=array($i,$adhtype->getObjectUrl(0,dol_size(16)));
...
if ($resql)
{
	...
	if ($num)
	{
		...
		while ($i < $num)
		{
			...
			print '<td>'.$staticmember->getObjectUrl(1,32).'</td>';
			print '<td>'.$statictype->getObjectUrl(1,32).'</td>';
...
if ($resql)
{
	...
	if ($num)
	{
		...
		while ($i < $num)
		{
			...
			$staticmember->ref=$staticmember->getFullName($langs);
			print '<td>'.$subscriptionstatic->getObjectUrl(1).'</td>';
			print '<td>'.$staticmember->getObjectUrl(1,32,'subscription').'</td>';
...
foreach ($AdherentType as $key => $adhtype)
{
	$var=!$var;
	print "<tr ".$bc[$var].">";
	print '<td>'.$adhtype->getObjectUrl(1, dol_size(32)).'</td>';
