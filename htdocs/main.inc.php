	...
	if (empty($disablehead))
	{
		...
		if(is_array($cur_modulepart) && count($cur_modulepart)>0)
		{
			foreach($conf->modules as $module)
			{
				...
				{
					//Check if the override folder exists in "/custom/modulename"
					$module_folder_style = dol_buildpath('', 0).DIRECTORY_SEPARATOR.(empty($conf->global->CUSTOM_MODULE_STYLE_FOLDER)?DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR."override":DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$conf->global->CUSTOM_MODULE_STYLE_FOLDER);
					if (file_exists($module_folder_style))
					{
						//browse the style dir and add the contains css files
						if ($handle = opendir($module_folder_style))
						{
							while (false !== ($entry = readdir($handle))) 
							{
								list($filename, $extension) = explode(".", $entry);
								if ($entry !== '.' && $entry !== '..' && $extension == 'css')
								{
									print '<link rel="stylesheet" type="text/css" href="'.DOL_URL_ROOT."/".(empty($conf->global->CUSTOM_MODULE_STYLE_FOLDER)?"custom/$module/override":"custom/$module/".$conf->global->CUSTOM_MODULE_STYLE_FOLDER)."/".$entry.'">'."\n";
								}
							}
							closedir($handle);
						}
