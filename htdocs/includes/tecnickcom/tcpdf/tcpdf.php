...
class TCPDF {
	 ...
	 */
	protected $default_monospaced_font = 'freemono';	// @CHANGE DOL
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->setPDFVersion();
		$this->tcpdflink = !getDolGlobalInt('MAIN_PDF_DISABLE_TCPDF_LINK');	// @CHANGE DOL
	...
	public function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if ($file[0] === '@') {
			...
			if (!@$this->fileExists($file)) {
				// @CHANGE DOL If we keep this, the image is not visible on pages after the first one.
	 ...
	 * @public
	 */
	public function _out($s) {	// @CHANGE DOL
		if ($this->state == 2) {
			if ($this->inxobj) {
				// we are inside an XObject template
