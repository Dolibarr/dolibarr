	...
	if ($res > 0) {
		...
		*/
		...
		if (!empty($conf->global->TICKET_SHOW_MESSAGES_ON_CARD)) {
			...
			$btnstatus = $object->status < Ticket::STATUS_CLOSED && $action != "presend" && $action != "presend_addmessage" && $action != "add_message"; ;
			$url = dol_buildpath('/comm/action/card.php', 1).'?action=create&datep='.date('YmdHi').'&origin=ticket&originid='.$object->id.'&projectid='.'&socid='.$object->fk_soc.$object->fk_project.'&backtopage='.urlencode($_SERVER["PHP_SELF"].'?track_id='.$object->track_id);
