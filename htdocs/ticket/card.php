	...
	if (GETPOST('save', 'alpha') && $permissiontoadd) {
		...
		if (!$error) {
			...
			if (!$error) {
				...
				if (!empty(getDolGlobalString('TICKET_AUTO_READ_WHEN_CREATED_FROM_BACKEND')) && $user->rights->ticket->write) {
...
if ($action == 'create' || $action == 'presend') {
	...
	$formticket->withnotifytiersatcreate = ($notifyTiers ? 1 : (empty(getDolGlobalString('TICKET_CHECK_NOTIFY_THIRDPARTY_AT_CREATION')) ? 0 : 1));
	...
	if ($res > 0) {
		...
		if (!$user->socid && (!empty(getDolGlobalString('TICKET_LIMIT_VIEW_ASSIGNED_ONLY')) && $object->fk_user_assign != $user->id) && !$user->rights->ticket->manage) {
			accessforbidden('', 0, 1);
		}
		...
		if ($action == 'close') {
			...
			$default = (!empty(getDolGlobalString('TICKET_NOTIFY_AT_CLOSING'))) ? -2 : -3;
			$formquestion = array(
				array(
					'name' => 'contactid',
					'type' => 'select',
		...
		*/
		...
		if (!empty(getDolGlobalString('TICKET_SHOW_MESSAGES_ON_CARD'))) {
			$param = '&id='.$object->id;
