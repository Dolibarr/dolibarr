...
if (!empty($object->id)) {
	...
	$btnstatus = $object->status < Ticket::STATUS_CLOSED && $action != "presend" && $action != "presend_addmessage";
	$url = DOL_URL_ROOT.'/comm/action/card.php?action=create&datep=now&origin=ticket&originid='.$object->id.'&projectid='.$object->fk_project.'&socid='.$object->fk_soc.'&backtopage='.urlencode($_SERVER["PHP_SELF"].'?id='.$object->id);
