...
foreach ($search as $key => $val) {
	if ($key == 'fk_statut' && !empty($search['fk_statut'])) {
		...
		if (count($newarrayofstatus)) {
			$sql .= natural_search('t.'.$key, implode(',', $newarrayofstatus), 2);
		}
		continue;
	} elseif ($key == 'fk_user_assign' || $key == 'fk_user_create' || $key == 'fk_project' || $key == 'fk_contract') {
		if ($search[$key] > 0) {
			$sql .= natural_search('t.'.$key, $search[$key], 2);
		}
		continue;
		...
		if (count($newarrayoftypecodes)) {
			$sql .= natural_search('t.'.$key, implode(',', $newarrayoftypecodes), 3);
		}
		continue;
	}
	...
	if ((is_array($search[$key]) && !empty($search[$key])) || (!is_array($search[$key]) && $search[$key] != '')) {
		$sql .= natural_search('t.'.$key, $search[$key], $mode_search);
	}
}
if ($search_all) {
