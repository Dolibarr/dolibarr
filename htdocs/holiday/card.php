	...
	if ($error && $action != 'edit') {
		...
		if ($action != 'presend' && $action != 'edit') {
			...
			if ($includedocgeneration) {
				...
				$genallowed = ($user->rights->holiday->read && $object->fk_user == $user->id) || $user->hasRight('holiday', 'readall'); // If you can read, you can build the PDF to read content
				$delallowed = ($user->rights->holiday->write && $object->fk_user == $user->id) || $user->hasRight('holiday', 'writeall_advance'); // If you can create/edit, you can remove a file on card
