...
if ($action == 'convertutf8') {
	...
	if ($resql) {
		...
		while ($i < $num) {
			...
			if ($row[0] == $field) {
				$sql = 'ALTER TABLE '.$db->sanitize($table).' MODIFY '.$db->sanitize($row[0]).' '.$db->sanitize($row[1]).' CHARACTER SET utf8';		// We must not sanitize the $row[1]
				$db->query($sql);

				$sql = 'ALTER TABLE '.$db->sanitize($table).' MODIFY '.$db->sanitize($row[0]).' '.$db->sanitize($row[1]).' COLLATE utf8_unicode_ci';	// We must not sanitize the $row[1]
				$db->query($sql);
...
if ($action == 'convertutf8mb4') {
	...
	if ($resql) {
		...
		while ($i < $num) {
			$row = $db->fetch_row($resql);
			if ($row[0] == $field) {
				$sql = 'ALTER TABLE '.$db->sanitize($table).' MODIFY '.$db->sanitize($row[0]).' '.$db->sanitize($row[1]).' CHARACTER SET utf8mb4';		// We must not sanitize the $row[1]
				$db->query($sql);

				$sql = 'ALTER TABLE '.$db->sanitize($table).' MODIFY '.$db->sanitize($row[0]).' '.$db->sanitize($row[1]).' COLLATE utf8mb4_unicode_ci';	// We must not sanitize the $row[1]
				$db->query($sql);
