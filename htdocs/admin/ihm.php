...
if ($action == 'update') {
	...
	if ($mode == 'template') {
		...
		// Spécifique Easya / Oblyon
		if (GETPOST("main_theme", 'aZ09') == "eldy" || GETPOST("main_theme", 'aZ09') == "md") {
			dolibarr_del_const($db,'MAIN_MENU_STANDARD_FORCED', $conf->entity);
			dolibarr_del_const($db,'MAIN_MENUFRONT_STANDARD_FORCED', $conf->entity);
			dolibarr_del_const($db,'MAIN_MENU_SMARTPHONE_FORCED', $conf->entity);
			dolibarr_del_const($db,'MAIN_MENUFRONT_SMARTPHONE_FORCED', $conf->entity);
		}

		if (GETPOST("main_theme", 'aZ09') == "oblyon") {
			dolibarr_set_const($db,'MAIN_MENU_INVERT', getDolGlobalInt('MAIN_MENU_INVERT_OBLYON_SAVE'), 'chaine', 0, '', $conf->entity);
			dolibarr_del_const($db,'MAIN_MENU_INVERT_OBLYON_SAVE', $conf->entity);

			dolibarr_set_const($db,'MAIN_MENUFRONT_SMARTPHONE_FORCED','oblyon_menu.php', 'chaine', 0, '', $conf->entity);
			dolibarr_set_const($db,'MAIN_MENU_SMARTPHONE_FORCED','oblyon_menu.php', 'chaine', 0, '', $conf->entity);
			dolibarr_set_const($db,'MAIN_MENUFRONT_STANDARD_FORCED','oblyon_menu.php', 'chaine', 0, '', $conf->entity);
			dolibarr_set_const($db,'MAIN_MENU_STANDARD_FORCED','oblyon_menu.php', 'chaine', 0, '', $conf->entity);
		}
