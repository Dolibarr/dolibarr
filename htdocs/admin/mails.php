...
if ($action == 'update' && !$cancel) {
	...
	if (!$error) {
		...
		dolibarr_set_const($db, 'MAIN_EMAIL_SUPPORT_ACK', GETPOST('MAIN_EMAIL_SUPPORT_ACK', 'int'), 'chaine', 0, '', $conf->entity);
...
if ($action == 'edit') {
	...
	//Allow email Ack
	print '<tr class="oddeven"><td>'.$langs->trans("MAIN_EMAIL_SUPPORT_ACK").'</td><td>';
	print $form->selectyesno('MAIN_EMAIL_SUPPORT_ACK', getDolGlobalString('MAIN_EMAIL_SUPPORT_ACK'), 1);
	print '</td></tr>';
	...
	if (empty($conf->global->MAIN_DISABLE_ALL_MAILS)) {
		...
		//Email Ack
		print '<tr class="oddeven"><td>'.$langs->trans("MAIN_EMAIL_SUPPORT_ACK").'</td><td>'.yn(!empty($conf->global->MAIN_EMAIL_SUPPORT_ACK)).'</td></tr>';
