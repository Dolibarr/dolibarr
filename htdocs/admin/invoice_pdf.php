...
/**
 *	    \file       htdocs/admin/invoice_pdf.php
 *		\ingroup    facture
 *		\brief      Setup page for invoice module
 */
...
$type = 'invoice';
$dirforterms = $conf->invoice->dir_output.'/';
if (!empty($conf->invoice->multidir_output[$conf->entity])) {
	$dirforterms = $conf->invoice->multidir_output[$conf->entity].'/';
}
...
if ($action == "update") {
	...
	/*if (GETPOSTISSET('MAIN_GENERATE_INVOICES_WITH_PICTURE')) {
		dolibarr_set_const($db, "MAIN_GENERATE_INVOICES_WITH_PICTURE", GETPOST("MAIN_GENERATE_INVOICES_WITH_PICTURE"), 'chaine', 0, '', $conf->entity);
	}*/
...
if ($action == 'removetermsofsale') {
	$filename = getDolGlobalString('MAIN_INFO_INVOICE_TERMSOFSALE');
	$file = $dirforterms.'/'.$filename;

	if ($filename != '') {
		dol_delete_file($file);
	}
	dolibarr_del_const($db, 'MAIN_INFO_INVOICE_TERMSOFSALE', $conf->entity);
}
...
llxHeader('', $title, '', '', 0, 0, '', '', '', 'mod-admin page-invoice');
...
print dol_get_fiche_head($head, 'pdf', $langs->trans("Invoice"), -1, 'bill');
...
// Concat PDF
print '<tr class="oddeven"><td>';
print $form->textwithpicto($langs->trans("MAIN_PDF_ADD_TERMSOFSALE_INVOICE"), $tooltipconcatpdf);
print '</td><td>';
if ($conf->use_javascript_ajax) {
