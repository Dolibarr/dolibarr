 ...
 */
class pdf_gdaparatos extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "GD aparatos";
		$this->logo = 'plantillas/gdaparatos.jpg';
		$this->description = 'Plantilla '.$this->name.' ('.$conf->mycompany->dir_output.'/logos/'.$this->logo.')';
		...
		$this->emetteur->fetch($conf->global->GDTABLET_SOCIETE);
...
//		}
		...
		if ($showaddress)
		{
			...
			$contact_comercial = new User($this->db);

			//Si hay comercial de seguimiento, lo incluimos
			if ($idcomercial = $object->getIdContact('internal', 'SALESREPFOLL')) {
				$res = $contact_comercial->fetch($idcomercial[0]);
			} else {
				global $db;

				require_once dol_buildpath('/casuna/lib/casuna.lib.php');

				if ($sr = getSalesRepresentativeInfoBySocid($conf, $db, $object->socid)) {
					$res = $contact_comercial->fetch($sr['rowid']);
				} else {
					$res = -1;
				}
			}
			...
			if ($res <= 0) {
				$contact_comercial = null;
			}

			// Sender properties
			$carac_emetteur = casunaPdfBuildAddress($outputlangs,$this->emetteur, $contact_comercial);
			...
			$carac_client= casunaPdfBuildTargetAddress($outputlangs,$this->emetteur,$object->client,($usecontact?$object->contact:null),$usecontact);
