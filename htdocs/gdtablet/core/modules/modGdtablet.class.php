 ...
 */
class modGdtablet extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->description = "Vista personalizada adaptada para tablet";
		...
		*/
		...
		$this->menu = array(
			array(
				...
				'type' => 'top',
				'titre' => 'Tablet',
				'url' => '/gdtablet/tablet/index.php',
				'mainmenu' => 'gdtablet',
				'leftmenu' => 'gdtablet',
				'langs' => '',
				'position' => 100,
				'enabled' => '$conf->gdtablet->enabled',
				'perms' => '1',
				'target' => '_blank',
				'user' => 0
			)
		...
		// $this->export_TypeFields_array[$r]=array('t.date'=>'Date', 't.qte'=>'Numeric', 't.poids'=>'Numeric', 't.fad'=>'Numeric', 't.paq'=>'Numeric', 't.stockage'=>'Numeric', 't.fadparliv'=>'Numeric', 't.livau100'=>'Numeric', 't.forfait'=>'Numeric', 's.nom'=>'Text','s.address'=>'Text','s.zip'=>'Text','s.town'=>'Text','c.code'=>'Text','s.phone'=>'Text','s.siren'=>'Text','s.siret'=>'Text','s.ape'=>'Text','s.idprof4'=>'Text','s.code_compta'=>'Text','s.code_compta_fournisseur'=>'Text','s.tva_intra'=>'Text','f.facnumber'=>"Text",'f.datec'=>"Date",'f.datef'=>"Date",'f.date_lim_reglement'=>"Date",'f.total'=>"Numeric",'f.total_ttc'=>"Numeric",'f.tva'=>"Numeric",'f.paye'=>"Boolean",'f.fk_statut'=>'Status','f.note_private'=>"Text",'f.note_private'=>"Text",'fd.description'=>"Text",'fd.subprice'=>"Numeric",'fd.tva_tx'=>"Numeric",'fd.qty'=>"Numeric",'fd.total_ht'=>"Numeric",'fd.total_tva'=>"Numeric",'fd.total_ttc'=>"Numeric",'fd.date_start'=>"Date",'fd.date_end'=>"Date",'fd.special_code'=>'Numeric','fd.product_type'=>"Numeric",'fd.fk_product'=>'List:product:label','p.ref'=>'Text','p.label'=>'Text','p.accountancy_code_sell'=>'Text');
	...
	public function init($options='')
	{
		...
		//Creación del extrafield de teléfono habitual de contacto
		if ($extrafields->addExtraField('tlfhabitual', 'Teléfono habitual de contacto', 'varchar', 15, '100', 'societe') < 1) {
			$this->error = 'Error al inicializar el módulo';
			dol_syslog($this->error.'. Error al crear el campo tlfhabitual', LOG_ERR);
			return -1;
		}

		//Creación del extrafield de color para categorías de terceros
		if ($extrafields->addExtraField('COLOR', 'Color (para terceros)', 'select', 1, '', 'categories', 0, 0, '', array(
			'options' => GdtabletFrontend::getMapColors()
		), 1, '', 1) < 1) {
			$this->error = 'Error al inicializar el módulo';
			dol_syslog($this->error.'. Error al crear el campo color', LOG_ERR);
			return -1;
		}

		//Creación del extrafield de visita para agenda
		if ($extrafields->addExtraField('VISITA', 'Visita', 'boolean', 20, '', 'actioncomm') < 1) {
			$this->error = 'Error al inicializar el módulo';
			dol_syslog($this->error.'. Error al crear el campo visita', LOG_ERR);
			return -1;
		}

		//Creación del tipo de evento AC_SEGUIMIEN
		$cactioncomm = new CActionComm($db);
		if ($cactioncomm->fetch(GdtabletFrontend::EVENT_TYPE) === 0) {
			$sql = 'SELECT MAX(id)+1 max FROM llx_actioncomm';
			$query = $db->query($sql);
			$result = $db->fetch_object($query);

			$modsql[] = "INSERT INTO llx_c_actioncomm (id, code, libelle, type, color, position, active) VALUES (".(int) $result->max.", 'AC_SEGUIMIEN', 'Seguimiento de cliente', 'system', '', 50, 1)";
		}

		return $this->_init($modsql, $options);
	}
