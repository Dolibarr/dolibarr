	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $usercancreate) {
		...
		if (!$error && (!empty($line_desc) || (!empty($idprod) && $idprod > 0))) {
			...
			*/
			...
			if (!$error) {
				// Insert line
				$result = $object->addline($desc, $pu_ht, (float) $qty, $tva_tx, $localtax1_tx, $localtax2_tx, $idprod, $remise_percent, $info_bits, 0, $price_base_type, $pu_ttc, $date_start, $date_end, $type, min($rank, count($object->lines) + 1), 0, GETPOSTINT('fk_parent_line'), (int) $fournprice, $buyingprice, $label, $array_options, $fk_unit, '', 0, (float) $pu_ht_devise, '', 0, $line_multicurrency_subprice_source);
		...
		if (!empty($importLines) && is_array($importLines) && !empty($fromElement) && ctype_alpha($fromElement) && !empty($fromElementid)) {
			...
			foreach ($importLines as $lineId) {
				...
				if (intval($fromElementid) > 0 && $originLine->fetch($lineId) > 0) {
					...
					$res = $object->addline($desc, $pu_ht, $qty, $txtva, $txlocaltax1, $txlocaltax2, $fk_product, $remise_percent, $info_bits, $fk_remise_except, $price_base_type, $pu_ttc, $date_start, $date_end, $type, $rang, $special_code, $fk_parent_line, $fk_fournprice, $pa_ht, $label, $array_options, $fk_unit, $origin, $origin_id, $pu_ht_devise, '', 0, $originLine->multicurrency_subprice_source);
