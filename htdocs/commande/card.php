...
if (empty($reshook)) {
	...
	$upload_dir = !empty($conf->commande->multidir_output[$object->entity ?? 1]) ? $conf->commande->multidir_output[$object->entity ?? 1] : $conf->commande->dir_output;
...
if ($action == 'create' && $usercancreate) {
	...
	if ($object->id > 0) {
		...
		if ($action != 'presend') {
			...
			$filedir = $conf->commande->multidir_output[$object->entity ?? 1] . '/' . $objref;
