	...
	if ($resql)
	{
		...
		if ($num)
		{
			...
			while ($i < $num)
			{
				...
				print '<td align="right" class="nowrap">'.dol_print_date($obj->date_creation,'day').'</tr>';
				$i++;
				$total += $obj->total_ttc;
			}
			if ($total>0)
			{
				$var=!$var;
				print '<tr class="liste_total"><td>'.$langs->trans("Total").'</td><td colspan="2" align="right">'.price($total).'</td>';
				print "<td/></tr>";
			}
		}
		print "</table><br>";
