	...
	if ($resql)
	{
		...
		if ($num > 0)
		{
			...
			while ($i < $num)
			{
				...
				print '</td>';
				print '<td align="right" nowrap="nowrap">'.price($obj->total_ht).'</td>';
				print '<td align="right" nowrap="nowrap">'.dol_print_date($obj->datep,'day').'</td></tr>';
				$i++;
				$total += $obj->total_ht;
			}
			if ($total>0)
	...
	if ($resql)
	{
		...
		if ($num)
		{
			...
			while ($i < $num)
			{
				...
				print '<td align="right" nowrap="nowrap">'.dol_print_date($obj->date_creation,'day').'</tr>';
				$i++;
				$total += $obj->total_ttc;
			}
			if ($total>0)
