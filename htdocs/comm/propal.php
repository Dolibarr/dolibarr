...
} elseif ($action == 'setdate' && $user->rights->propal->creer) {
...
} elseif ($action == 'setecheance' && $user->rights->propal->creer) {
    $result=$object->set_echeance($user,dol_mktime(12, 0, 0, $_POST['echmonth'], $_POST['echday'], $_POST['echyear']));
    if ($result < 0) dol_print_error($db,$object->error);
} elseif ($action == 'setdate_livraison' && $user->rights->propal->creer) {
...
} elseif ($action == 'setnote_public' && $user->rights->propal->creer) {
    $result=$object->update_note(dol_html_entity_decode(GETPOST('note_public'), ENT_QUOTES),'_public');
    if ($result < 0) dol_print_error($db,$object->error);
} elseif ($action == 'setnote_private' && $user->rights->propal->creer) {
...
} elseif ($action == "setabsolutediscount" && $user->rights->propal->creer) {
    if ($_POST["remise_id"]) {
        if ($object->id > 0) {
...
} elseif ($action == 'updateligne' && $user->rights->propal->creer && GETPOST('cancel') == $langs->trans('Cancel')) {
...
} elseif ($action == 'setremisepercent' && $user->rights->propal->creer) {
    $result = $object->set_remise_percent($user, $_POST['remise_percent']);
} elseif ($action == 'setremiseabsolue' && $user->rights->propal->creer) {
...
*/
    ...
    exit;
} elseif ($action == 'down' && $user->rights->propal->creer) {
