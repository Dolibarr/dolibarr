...
if ($resql) {
	...
	if (!empty($arrayfields['p.total_vat']['checked'])) {
		print_liste_field_titre($arrayfields['p.total_vat']['label'], $_SERVER["PHP_SELF"], 'p.tva', '', $param, 'class="right"', $sortfield, $sortorder);
	}
	if (!empty($arrayfields['p.total_ttc']['checked'])) {
		print_liste_field_titre($arrayfields['p.total_ttc']['label'], $_SERVER["PHP_SELF"], 'p.total', '', $param, 'class="right"', $sortfield, $sortorder);
	}
	if (!empty($arrayfields['p.total_ht_invoiced']['checked'])) {
	...
	if (!empty($arrayfields['p.multicurrency_total_vat']['checked'])) {
		print_liste_field_titre($arrayfields['p.multicurrency_total_vat']['label'], $_SERVER['PHP_SELF'], 'p.multicurrency_total_tva', '', $param, 'class="right"', $sortfield, $sortorder);
	}
	if (!empty($arrayfields['p.multicurrency_total_ttc']['checked'])) {
	...
	while ($i < min($num, $limit)) {
		...
		if (!empty($arrayfields['p.total_vat']['checked'])) {
			...
			if (!$i) {
				$totalarray['pos'][$totalarray['nbfield']] = 'p.total_vat';
			}
