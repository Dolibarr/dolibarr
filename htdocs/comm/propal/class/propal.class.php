...
class Propal extends CommonObject
{
    ...
    public $ref;
    public $ref_client;
    public $statut;					// 0 (draft), 1 (validated), 2 (signed), 3 (not signed), 4 (billed)
    public $datec;						// Date of creation
    public $datev;						// Date of validation
    public $date;						// Date of proposal
    public $datep;						// Same than date
    ...
    public $total_ht;					// Total net of tax
    public $total_tva;					// Total VAT
    public $total_localtax1;			// Total Local Taxes 1
    public $total_localtax2;			// Total Local Taxes 2
    public $total_ttc;					// Total with tax
    public $price;						// deprecated (for compatibility)
    public $tva;						// deprecated (for compatibility)
    public $total;						// deprecated (for compatibility)
    ...
    public $note;						// deprecated (for compatibility)
    public $note_private;
    public $note_public;
    public $fk_delivery_address;		// deprecated (for compatibility)
    public $fk_address;
    public $address_type;
    public $address;
     ...
     *    	@see       	add_product
     */
    public function addline($propalid, $desc, $pu_ht, $qty, $txtva, $txlocaltax1=0, $txlocaltax2=0, $fk_product=0, $remise_percent=0, $price_base_type='HT', $pu_ttc=0, $info_bits=0, $type=0, $rang=-1, $special_code=0, $fk_parent_line=0, $fk_fournprice=0, $pa_ht=0, $label='',$date_start='', $date_end='',$array_option=0)
    {
        global $conf;
     ...
     *  @return     int     		        		0 if OK, <0 if KO
     */
    public function updateline($rowid, $pu, $qty, $remise_percent, $txtva, $txlocaltax1=0, $txlocaltax2=0, $desc='', $price_base_type='HT', $info_bits=0, $special_code=0, $fk_parent_line=0, $skip_update_total=0, $fk_fournprice=0, $pa_ht=0, $label='', $type=0, $date_start='', $date_end='', $array_option=0)
    {
     ...
     *	@return     int         		<0 if ko, >0 if ok
     */
    public function update_extrafields($user)
    {
        // Actions on extra fields (by external module or standard code)
        // FIXME le hook fait double emploi avec le trigger !!
        $hookmanager->initHooks(array('propaldao'));
     ...
     *	@return     int         					<0 if KO, >0 if OK
     */
    public function set_echeance($user, $date_fin_validite)
    {
        if ($user->rights->propale->creer) {
     ...
     *  @return     int           			<0 if KO, >0 if OK
     */
    public function set_availability($user, $id)
    {
        if ($user->rights->propale->creer) {
            $sql = "UPDATE ".MAIN_DB_PREFIX."propal ";
     ...
     *  @return     int           		<0 if KO, >0 if OK
     */
    public function set_demand_reason($user, $id)
    {
        if ($user->rights->propale->creer) {
            $sql = "UPDATE ".MAIN_DB_PREFIX."propal ";
     ...
     *  @return     int						<0 if ko, >0 if ok
     */
    public function set_ref_client($user, $ref_client)
    {
        if ($user->rights->propale->creer) {
     ...
     *	@return     int         		<0 if KO, >0 if OK
     */
    public function reopen($user, $statut, $note)
    {
        global $langs,$conf;
     ...
     *	@return     int     	<0 si ko, >0 si ok
     */
    public function classer_facturee()
    {
     ...
     *  @return int                  	>0 if OK, <0 if KO
     */
    public function availability($availability_id)
    {
...
class PropaleLigne  extends CommonObject
{
    ...
    // From llx_product
    public $ref;						// Reference produit
    public $libelle;       // Label produit
