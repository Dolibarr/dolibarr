 ...
 *	\brief      Return the uid of agenda events deleted since a given time.
 *
 ...
 *	without reloading the whole set. Events the requesting user was not allowed to
 *	read (not the owner, not assigned, no "see all" permission) are left out, the
 *	same way agenda/myactions vs agenda/allactions already filters live events.
 *
 *	Input  (GET or POST):
 *	  - since   : unix timestamp, lower bound of the deletion date (optional,
 *	              defaults to now minus the retention delay)
 *	Output (application/json):
 *	  {"now": <server unix time>, "since": <effective since>, "deleted": [uid, ...]}
 *	Poll again later passing the returned "now" as the next "since". "deleted" holds
 *	the event uid, not its internal id, so this endpoint cannot be used to enumerate ids.
 */
...
// Knowing that an event was deleted must not be visible to someone who could not read it.
// Once the row is gone we can no longer re-derive its thirdparty, so external users are excluded.
if (!empty($user->socid)) {
	httponly_accessforbidden('The deletion log is not available to external users', 403);
}
...
$deleted = $deletionlog->getDeletionsSince($since, $user, $conf->entity);
