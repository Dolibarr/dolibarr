...
// Knowing that an object was deleted must not be visible to someone who could not read it.
...
// This first pass only serves agenda events. Opening it to other element types
// needs a per-element read-permission check (see PR discussion), so anything
// else is rejected for now.
$element = GETPOST('element', 'aZ09') ? GETPOST('element', 'aZ09') : 'action';
if ($element !== 'action') {
	httponly_accessforbidden('Unsupported element type', 403);
}
...
$deleted = $deletionlog->getDeletionsSince($element, $since, $conf->entity);
