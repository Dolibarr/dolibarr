...
/**
 * \file    test/functional/LeaveListFunctionalTest.php
 * \ingroup leavelist
 ...
 *
 * @package Testleavelist
 */
class LeaveListFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modLeaveList')->click();
		} else {
			// Disable the module
			$this->byHref('modLeaveList')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/leavelist/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('leavelist/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/leavelist/admin/about.php');
		$this->authenticate();
		return $this->assertContains('leavelist/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('leavelistwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modLeaveList_LeaveListTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modLeaveList_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
