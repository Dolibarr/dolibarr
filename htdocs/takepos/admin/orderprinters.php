...
//V20: Use selected category in setup
if($conf->global->POS_ID_CATEGORY>0){
	$catid=$conf->global->POS_ID_CATEGORY;
	$maincategories = array_filter($cate_arbo, function ($item) use ($catid) {
	    if ($item['fk_parent']==$catid)      
	    	return true;
	    
	    return false;
	});
}else{
	$maincategories = array_filter($cate_arbo, function ($item) {
...
$cate_arbo=array_values($maincategories);	//V20: Sorting keys
