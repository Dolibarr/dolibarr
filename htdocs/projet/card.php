...
if (empty($reshook)) {
	...
	if ($action == 'remove_file' && $permissiontoadd) {
		if ($object->id > 0) {
			...
			$langs->load("other");
			$upload_dir = $conf->project->multidir_output[$object->entity ?? 1];
...
if ($action == 'create' && $user->hasRight('projet', 'creer')) {
	...
	if ($action != 'presend') {
		...
		$filedir = $conf->project->multidir_output[$object->entity ?? 1]."/".dol_sanitizeFileName($object->ref);
	...
	$diroutput = $conf->project->multidir_output[$object->entity ?? 1];
	$autocopy = 'MAIN_MAIL_AUTOCOPY_PROJECT_TO'; // used to know the automatic BCC to add
	$trackid = 'proj'.$object->id;
