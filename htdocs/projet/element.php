			...
			for ($i = 0; $i < $num; $i++) {
				...
				if ($tablename == 'expensereport_det') {
					...
					if (!empty($conf->$element_doc)) {
						...
						$filedir = $confelementdoc->multidir_output[$element->entity ?? 1].'/'.dol_sanitizeFileName($element->ref);
					} else {
						$filedir = '';
					}
					...
					if ($element_doc === 'order_supplier') {
						...
						$filedir = $conf->fournisseur->commande->multidir_output[$element->entity ?? 1].'/'.dol_sanitizeFileName($element->ref);
						...
						$filedir = $conf->fournisseur->facture->multidir_output[$element->entity ?? 1].'/'.$filename;
					}
