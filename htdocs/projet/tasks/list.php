...
$sql .= " LEFT JOIN llx_element_element AS pee ON (pee.fk_source = propal.rowid AND pee.sourcetype='propal' AND pee.targettype='commande' ) or (pee.fk_target = propal.rowid AND pee.targettype='propal' AND pee.sourcetype='commande' )";
$sql .= " LEFT JOIN (select fk_source as peefs, max(fk_target) as peeft FROM llx_element_element WHERE (sourcetype='commande' and targettype='facture') GROUP BY fk_source ) peef ON pee.rowid = peef.peefs ";
...
$sql .= " LEFT JOIN (select rowid, ref_client, round(total_ht,2) as factot FROM llx_facture) fact ON fact.rowid=peef.peeft OR fact.rowid=peep.peept OR fact.rowid is null OR fact.ref_client LIKE concat('%',ef.refexpertise,'%')";
...
$sql .= " LEFT JOIN (select rowid, ref_supplier, round(total_ht,2) as faftot, fk_statut FROM llx_facture_fourn) factfourn  ON factfourn.rowid=ceef.ceeft OR factfourn.rowid is null OR factfourn.ref_supplier LIKE concat('%',ef.reffranchise,'%')";
$sql .= " LEFT JOIN llx_extralistcomp_commande as extcde ON extcde.rowid = cdefourn.fk_statut ";
//$sql .= " LEFT JOIN llx_extralistcomp_commande as extfac ON extfac.rowid = factfourn.fk_statut ";
// END OF DIRTY
