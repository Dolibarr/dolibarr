...
print dol_get_fiche_head($head, 'inputstatisticsglobal', $langs->trans('TimeSpent'), -1, $picto);
...
print '<div class="floatright right'.($conf->dol_optimize_smallscreen ? ' centpercent' : '').'">'.$formother->selectyear($year ? $year : -1, 'year', 0, 20, 0, 0, 0, '', 'valignmiddle width75', 1).'<button type="submit" name="submitdateselect" value="x" class="bordertransp"><span class="fa fa-search"></span></button></div>';
...
/*********************************
 * Graphic: Hours by User
 */
// Build graphic number of object
// $data = array(array('Lib',val1,val2,val3),...)
$data = $stats_time->getConsumedTimeByUser($year);

$filenamenb = $conf->project->dir_output."/stats/consumedtimebyuseryear-".$year.".png";
$fileurlnb = DOL_URL_ROOT.'/viewimage.php?modulepart=taskstats&amp;file=consumedtimebyuseryear-'.$year.'.png';
...
if (!$mesg) {
	...
	$px1->setShowLegend(2);
	$px1->mode = 'depth';
	$px1->SetTitle($langs->trans("TotalTimeInHourByUser").' ('.$year.')');
	$px1->SetType(array('pie'));
	$px1->draw($filenamenb, $fileurlnb);
}

print '<div class="fichethirdleft"><div class="ficheaddleft">';
...
// Build graphic number of object
// $data = array(array('Lib',val1,val2,val3),...)
$data = $stats_time->getConsumedTimeByMonth($startyear, $endyear);

$filenamenb = $conf->project->dir_output."/stats/consumedtimeprevyear-".$year.".png";
$fileurlnb = DOL_URL_ROOT.'/viewimage.php?modulepart=taskstats&amp;file=consumedtimeprevyear-'.$year.'.png';
...
if (!$mesg) {
	...
	$px1->SetHorizTickIncrement(1);
	$px1->mode = 'depth';
	$px1->SetTitle($langs->trans("TotalTimeInHourByMonth").' ('.$year.')');
...
/*********************************
 * Graphic: Hours by Project
 */
// Build graphic number of object
// $data = array(array('Lib',val1,val2,val3),...)
$data = $stats_time->getConsumedTimeByProject($year);

$filenamenb = $conf->project->dir_output."/stats/consumedtimebyprojectyear-".$year.".png";
$fileurlnb = DOL_URL_ROOT.'/viewimage.php?modulepart=taskstats&amp;file=consumedtimebyprojectyear-'.$year.'.png';
...
if (!$mesg) {
	...
	$px1->SetHorizTickIncrement(1);
	$px1->mode = 'depth';
	$px1->SetTitle($langs->trans("TotalTimeInHourByProject").' ('.$year.')');
	$px1->SetType(array('pie'));
	$px1->draw($filenamenb, $fileurlnb);
}
...
/*********************************
 * Graphic: Hours by Teams
 */
// Build graphic number of object
// $data = array(array('Lib',val1,val2,val3),...)
$data = $stats_time->getConsumedTimeByTeam($year);

$filenamenb = $conf->project->dir_output."/stats/consumedtimebyprojectteam-".$year.".png";
$fileurlnb = DOL_URL_ROOT.'/viewimage.php?modulepart=taskstats&amp;file=consumedtimebyprojectyteam-'.$year.'.png';
...
if (!$mesg) {
	...
	$px1->setShowLegend(2);
	$px1->mode = 'depth';
	$px1->SetTitle($langs->trans("TotalTimeInHourByTeam").' ('.$year.')');
	$px1->SetType(array('pie'));
	$px1->draw($filenamenb, $fileurlnb);
}
