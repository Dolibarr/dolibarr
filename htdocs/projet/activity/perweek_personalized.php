...
if ($action == 'addtime' && $user->rights->projet->lire && GETPOST('emptytasks'))
{
	unset($_SESSION['tasktime_edit_tasks']);
}

if ($action == 'addtime' && $user->rights->projet->lire && GETPOST('addtask'))
{
	$taskid = GETPOST('add_task_id');
	$task = new Task($db);
	$resultat = $task->fetch($taskid);
	...
	{
		$TTask = array();
		if(!empty($_SESSION['tasktime_edit_tasks'])) $TTask = $_SESSION['tasktime_edit_tasks'];
		$TTask[$taskid] = $taskid;
		$_SESSION['tasktime_edit_tasks'] = $TTask;
			...
			foreach($value as $key => $val)          // Loop on each day
			{
				...
				if (! empty($amountoadd))
				{
					...
					if ($newduration > 0)
					{
						...
						if(!empty($conf->global->PROJECT_TIMESHEET_AUTO_ADD_IN_CONTACT)) {
							$project = new Project($db);
							$project->fetch($object->fk_project);
							$object->add_contact($user->id, 181, 'internal');
							$project->add_contact($user->id, 161, 'internal');
						}
...
$TTask = $_SESSION['tasktime_edit_tasks'];
$tasksarray=$taskstatic->getTasksPersonalizedArray($user, $firstdaytoshow, $lastdaytoshow, $TTask, $morewherefilter);    // We want to see all tasks of open project i am allowed to see and that match filter, not only my tasks. Later only mine will be editable later.
...
dol_fiche_head($head, 'inputperweekpersonalized', $langs->trans('TimeSpent'), -1, 'task');

// Show description of content
print '<div class="hideonsmartphone">';

if (empty($conf->global->PROJECT_TIMESHEET_AUTO_ADD_IN_CONTACT) && ! $user->rights->projet->all->creer)
{
	print $langs->trans("MyTasksDesc").'.<br>';
	print $langs->trans("OnlyYourTaskAreVisible").'<br>';
} else {
	print $langs->trans("TasksDesc").'.<br>';
}
print '</div>';
...
$(document).ready(function(){
	$("#add_project_id").change(function(){
		window.location.replace("'.$_SERVER['PHP_SELF'].'?year='.$year.'&month='.$month.'&day='.$day.'&id=" + this.value);
	});
});
</script>
';
print $formproject->select_projects_list('-1',$projectid,'add_project_id',40, 0, 1, 0, 0, 0, 0, '', 1);
if(!empty($projectid)) {
	$formproject->selectTasks('-1','','add_task_id',24, 0, '0', 0, 0, 0, 'maxwidth500', $projectsListId=$projectid, '');
}
print '</div>';
print ' ';
//print '<a href="#" id="add_task" style="vertical-align:middle">'.img_picto('Ajouter la tâche', 'edit_add.png','style="padding:2px; margin:0 5px;vertical-align:middle"').'</a>';
print '<input type="submit" class="button valignmiddle" name="addtask" '.(empty($projectid)?'disabled="disabled" ':'').'value="'.$langs->trans('Add').'">';
print '</div>';
...
if (count($tasksarray) > 0)
{
	...
	// If auto add right is enable, we authorized user to register time in every tasks
	if(!empty($conf->global->PROJECT_TIMESHEET_AUTO_ADD_IN_CONTACT)) {
		// Able all input task time
		$user->rights->projet->all->creer= 1;
	}
	
	$totalforvisibletasks = projectLinesPerWeek($j, $firstdaytoshow, $user, 0, $tasksarray, $level, $projectsrole, $tasksrole, $mine, 0, $isavailable, 0);
...
print '<input type="submit" class="button" name="emptytasks" value="'.dol_escape_htmltag($langs->transnoentities("ClearTasksInTable")).'">';
