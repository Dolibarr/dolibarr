...
/**
 * \file		htdocs/accounting/bookkeeping/card.php
 * \ingroup		Accounting
...
if ($action == 'create')
{
	...
	*/
	...
	{
		$backlink = '<a href="'.DOL_URL_ROOT.'/accounting/bookkeeping/list.php?restore_lastsearch_values=1">' . $langs->trans('BackToList') . '</a>';
		...
		*/
		...
		if ($object->doc_type == 'customer_invoice')
		{
		 $sqlmid = 'SELECT rowid as ref';
			...
			dol_syslog("accounting/bookkeeping/card.php::sqlmid=" . $sqlmid, LOG_DEBUG);
		...
		if ($result < 0) {
			...
			if (count($object->linesmvt) > 0) {
				...
				if ($mode=='_tmp' && $action=='')
				{
					...
					print ' &nbsp; ';
					print '<a class="button" href="' . DOL_URL_ROOT.'/accounting/bookkeeping/list.php">'.$langs->trans("Cancel").'</a>';
