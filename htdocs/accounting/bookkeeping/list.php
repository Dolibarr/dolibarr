...
/**
 * \file		htdocs/accounting/bookkeeping/list.php
 * \ingroup		Accounting
 * \brief 		List operation of book keeping
 */
require '../../main.inc.php';
require_once DOL_DOCUMENT_ROOT . '/accounting/class/accountingexport.class.php';
...
$groupby = ' <a class="nohover marginrightonly" href="'.DOL_URL_ROOT.'/accounting/bookkeeping/listbyaccount.php?'.$param.'">' . $langs->trans("GroupByAccountAccounting") . '</a>';
...
if ($num > 0)
{
	...
	while ($i < min($num, $limit))
	{
		...
		print '<a href="'.DOL_URL_ROOT.'/accounting/bookkeeping/card.php?piece_num=' . $line->piece_num . $param . '&page=' . $page . ($sortfield ? '&sortfield='.$sortfield : '') . ($sortorder ? '&sortorder='.$sortorder : '') . '">' . img_edit() . '</a>&nbsp;';
