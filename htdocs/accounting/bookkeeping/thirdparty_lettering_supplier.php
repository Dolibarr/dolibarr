...
/**
 * \file    	htdocs/accounting/bookkeeping/thirdparty_lettrage_supplier.php
 * \ingroup 	Accounting
...
require_once DOL_DOCUMENT_ROOT . '/accounting/class/lettering.class.php';
...
dol_syslog("/accounting/bookkeeping/thirdparty_lettrage_supplier.php", LOG_DEBUG);
$resql = $db->query($sql);
if (! $resql) {
		dol_print_error($db);
		exit;
...
if ($resql) {
	...
	while ($obj = $db->fetch_object($resql)) {
		...
		if (empty($obj->lettering_code)) {
			print '<td><a href="' . dol_buildpath('/accounting/bookkeeping/card.php', 1) . '?piece_num=' . $obj->piece_num . '">';
			print img_edit();
