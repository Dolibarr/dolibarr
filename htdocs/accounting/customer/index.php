...
/**
 * \file 	htdocs/accounting/customer/index.php
 * \ingroup Accounting
...
if ($action == 'clean' || $action == 'validatehistory')
{
	...
	dol_syslog("htdocs/accounting/customer/index.php fixaccountancycode", LOG_DEBUG);
...
if ($action == 'validatehistory') {
	...
	dol_syslog('htdocs/accounting/customer/index.php');
...
dol_syslog('htdocs/accounting/customer/index.php sql=' . $sql, LOG_DEBUG);
$resql = $db->query($sql);
if ($resql) {
	...
	while ( $row = $db->fetch_row($resql)) {
		...
		if ($row[0] == 'tobind')
		{
			print $langs->trans("UseMenuToSetBindindManualy", DOL_URL_ROOT.'/accounting/customer/list.php?search_year='.$y, $langs->transnoentitiesnoconv("ToBind"));
		}
		else print $row[1];
		print '</td>';
		for($i = 2; $i <= 12; $i ++) {
