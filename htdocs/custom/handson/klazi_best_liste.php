...
/**
 *	\file       htdocs/custom/handson/klazi_best_liste.php
 *	\ingroup    commande
...
$search_paddress = GETPOST('search_paddress', 'alpha');
$search_pplz = GETPOST('search_pplz', 'alpha');
$search_ptown = GETPOST("search_ptown", 'alpha');
$search_institution = GETPOST("search_institution", 'alpha');
...
$hookmanager->initHooks(array('klaziOrders'));
...
$arrayfields = array(
    ...
    'socp.firstname'=>array('label'=>"Vorname", 'checked'=>1, 'position'=>1005, 'position'=>160),
    'socp.lastname'=>array('label'=>"Nachname", 'checked'=>1, 'position'=>1010, 'position'=>165),
    'socp.email'=>array('label'=>'E-Mail', 'checked'=>1, 'position'=>1011, 'position'=>166),
    'socpv.town'=>array('label'=>"Stadt", 'checked'=>1, 'position'=>1015, 'position'=>170),
    'socpv.address'=>array('label'=>"Adresse", 'checked'=>1, 'position'=>1016, 'position'=>171),
    'socpv.zip'=>array('label'=>"PLZ", 'checked'=>1, 'position'=>1017, 'position'=>172),
    'socpex.institution'=>array('label'=>"Institution", 'checked'=>1, 'position'=>1020, 'position'=>175)
);
// Extra fields
...
$res = $db->query('SELECT total_tva FROM '.MAIN_DB_PREFIX.'commande');
$tva = ($res) ? 'total_tva' : 'tva';
...
$sql .= ' c.rowid, c.fk_soc, c.ref, c.total_ht, c.'.$tva.' as total_tva, c.total_ttc, c.ref_client, c.fk_user_author,';
...
$sql .= ' socp.firstname, socp.lastname, socp.email, socpv.address, socpv.zip, socpv.town AS ptown, socpex.institution ';
...
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."element_contact AS elco ON c.rowid = elco.element_id AND elco.fk_c_type_contact=101";
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."socpeople AS socp ON elco.fk_socpeople = socp.rowid";
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."element_contact AS elcov ON c.rowid = elcov.element_id AND elcov.fk_c_type_contact=102";
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."socpeople AS socpv ON elcov.fk_socpeople = socpv.rowid";
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."socpeople_extrafields AS socpex ON socpex.fk_object = socp.rowid";
...
if ($search_firstname)                       $sql .= natural_search('socp.firstname', $search_firstname);
if ($search_lastname)                        $sql .= natural_search('socp.lastname', $search_lastname);
if ($search_email)                           $sql .= natural_search('socp.email', $search_email);
if ($search_ptown)                           $sql .= natural_search('socpv.town', $search_ptown);
if($search_paddress)                         $sql .= natural_search('socpv.address', $search_paddress);
if($search_pplz)                             $sql .= natural_search('socpv.zip', $search_pplz);
if ($search_institution)                     $sql .= " AND socpex.institution IN '".$db->escape($search_institution)."'";
...
if ($resql)
{
    ...
    if($search_ptown != '')         $param .= '&search_ptown='.$search_ptown;
    if($search_paddress != '')      $param .= '&search_paddress='.$search_paddress;
    if($search_pplz != '')          $param .= '&search_pplz='.$search_pplz;
    if($search_institution != '')   $param .= '&search_institution='.$search_institution;
    ...
    if (!empty($arrayfields['socp.firstname']['checked']))
    {
        // First name
        print '<td class="liste_titre" align="center">';
        print '<input class="flat" size="4" type="text" name="search_firstname" value="'.dol_escape_htmltag($search_firstname).'">';
        print '</td>';
    }
    if (!empty($arrayfields['socp.lastname']['checked']))
    {
        // Last name
        print '<td class="liste_titre" align="center">';
        print '<input class="flat" size="4" type="text" name="search_lastname" value="'.dol_escape_htmltag($search_lastname).'">';
        print '</td>';
    }
    if (!empty($arrayfields['socp.email']['checked']))
    {
        // Last name
        print '<td class="liste_titre" align="center">';
        print '<input class="flat" size="4" type="text" name="search_email" value="'.dol_escape_htmltag($search_email).'">';
        print '</td>';
    }
    if (!empty($arrayfields['socpv.address']['checked']))
    {
        // Address
        print '<td class="liste_titre" align="center">';
        print '<input class="flat" size="4" type="text" name="search_paddress" value="'.dol_escape_htmltag($search_paddress).'">';
        print '</td>';
    }
    if (!empty($arrayfields['socpv.zip']['checked']))
    {
        // PLZ
        print '<td class="liste_titre" align="center">';
        print '<input class="flat" size="4" type="text" name="search_pplz" value="'.dol_escape_htmltag($search_pplz).'">';
        print '</td>';
    }
    if (!empty($arrayfields['socpv.town']['checked']))
    {
        // Town
        print '<td class="liste_titre" align="center">';
        print '<input class="flat" size="4" type="text" name="search_ptown" value="'.dol_escape_htmltag($search_ptown).'">';
        print '</td>';
    }
    if (!empty($arrayfields['socpex.institution']['checked']))
    {
        // Institution
        print '<td class="liste_titre" align="center">';
        print '<input class="flat" size="4" type="text" name="search_institution" value="'.dol_escape_htmltag($search_institution).'">';
        print '</td>';
    }
    // Extra fields
    ...
    if (!empty($arrayfields['socp.firstname']['checked']))       	  print_liste_field_titre($arrayfields['socp.firstname']['label'], $_SERVER["PHP_SELF"], 'socp.firstname', '', $param, 'align="center"', $sortfield, $sortorder);
    if (!empty($arrayfields['socp.lastname']['checked']))       	  print_liste_field_titre($arrayfields['socp.lastname']['label'], $_SERVER["PHP_SELF"], 'socp.lastname', '', $param, 'align="center"', $sortfield, $sortorder);
    if (!empty($arrayfields['socp.email']['checked']))       	  print_liste_field_titre($arrayfields['socp.email']['label'], $_SERVER["PHP_SELF"], 'socp.email', '', $param, 'align="center"', $sortfield, $sortorder);
    if (!empty($arrayfields['socpv.address']['checked']))       	  print_liste_field_titre($arrayfields['socpv.address']['label'], $_SERVER["PHP_SELF"], 'socpv.address', '', $param, 'align="center"', $sortfield, $sortorder);
    if (!empty($arrayfields['socpv.zip']['checked']))       	  print_liste_field_titre($arrayfields['socpv.zip']['label'], $_SERVER["PHP_SELF"], 'socpv.zip', '', $param, 'align="center"', $sortfield, $sortorder);
    if (!empty($arrayfields['socpv.town']['checked']))       	  print_liste_field_titre($arrayfields['socpv.town']['label'], $_SERVER["PHP_SELF"], 'socpv.town', '', $param, 'align="center"', $sortfield, $sortorder);
    if (!empty($arrayfields['socpex.institution']['checked']))       	  print_liste_field_titre($arrayfields['socpex.institution']['label'], $_SERVER["PHP_SELF"], 'socpex.institution', '', $param, 'align="center"', $sortfield, $sortorder);
