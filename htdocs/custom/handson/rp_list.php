...
require_once __DIR__.'/class/rp.class.php';
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'rplist'; // To manage different context of search
...
$hookmanager->initHooks(array('rplist')); // Note that conf->hooks_modules contains array
...
$permissiontoread = $user->rights->handson->rp->read;
$permissiontoadd = $user->rights->handson->rp->write;
$permissiontodelete = $user->rights->handson->rp->delete;

// Security check
if (empty($conf->handson->enabled)) {
...
//$help_url="EN:Module_RP|FR:Module_RP_FR|ES:Módulo_RP";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("RPs"));
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/handson/rp_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/handson/rp_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
