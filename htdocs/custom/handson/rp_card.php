 ...
 *   	\file       rp_card.php
 *		\ingroup    handson
 *		\brief      Page to create/edit/view rp
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'rpcard'; // To manage different context of search
...
$hookmanager->initHooks(array('rpcard', 'globalcard')); // Note that conf->hooks_modules contains array
...
$permissiontoadd = $user->rights->handson->rp->write; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
$permissiontodelete = $user->rights->handson->rp->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
$permissionnote = $user->rights->handson->rp->write; // Used by the include of actions_setnotes.inc.php
$permissiondellink = $user->rights->handson->rp->write; // Used by the include of actions_dellink.inc.php
$upload_dir = $conf->handson->multidir_output[isset($object->entity) ? $object->entity : 1].'/rp';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/handson/rp_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/handson/rp_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'HANDSON_RP_MODIFY'; // Name of trigger action code to execute when we modify record
...
//	include DOL_DOCUMENT_ROOT.'/core/actions_addupdatedelete.inc.php';
	...
	if ($action == 'add' && !empty($permissiontoadd)) {
		foreach ($object->fields as $key => $val) {
			...
			if (preg_match('/^contactlist/i', $object->fields[$key]['type'])) {
				$res = '';
				foreach($value as $e) {
					$res .= $e.', ';
				}
...
// Action to update record
	if ($action == 'update' && !empty($permissiontoadd)) {
		foreach ($object->fields as $key => $val) {
			...
			$object->$key = $value;
			if (isset($val['notnull']) && ($val['notnull'] > 0 && $object->$key == '' && is_null($val['default']))) {
				$error++;
				setEventMessages($langs->trans("ErrorFieldRequired", $langs->transnoentitiesnoconv($val['label'])), null, 'errors');
			}
		}
...
// Action clone object
	...
	$triggersendname = 'HANDSON_RP_SENTBYMAIL';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_RP_TO';
	$trackid = 'rp'.$object->id;
...
// Part to create
if ($action == 'create') {
	print load_fiche_titre('RP hinzufügen', '', 'object_'.$object->picto);
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("RP"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteRP'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$genallowed = $user->rights->handson->rp->read; // If you can read, you can build the PDF to read content
			$delallowed = $user->rights->handson->rp->write; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('handson:RP', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('rp'));
		...
		$morehtmlright = '<a href="'.dol_buildpath('/handson/rp_agenda.php', 1).'?id='.$object->id.'">';
