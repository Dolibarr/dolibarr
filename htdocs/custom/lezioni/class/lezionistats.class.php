...
require_once __DIR__.'/lezione.class.php';

/**
 *	Class to manage statistics of members
 */
class LezioniStats // extends Stats
{
	/**
	...
	public function getFormatDateFilters($startDate, $endDate){
		/*
		search_datalezione_dtstart	"01/03/2024" x
		search_datalezione_dtstartday	"1"
		search_datalezione_dtstartmonth	"3"
		search_datalezione_dtstartyear	"2024"
		search_datalezione_dtend	"01/04/2024" x
		search_datalezione_dtendday	"1"
		search_datalezione_dtendmonth	"4"
		search_datalezione_dtendyear	"2024"
		*/
		global $langs;

		$ret = '';
		$ret .= 'search_datalezione_dtstart='.dol_print_date($startDate,$langs->trans("FormatDateShortInput"));
		$ret .= '&search_datalezione_dtend='.dol_print_date($endDate,$langs->trans("FormatDateShortInput"));
		$ret .= '&search_datalezione_dtstartday='.dol_print_date($startDate,'%d');
		$ret .= '&search_datalezione_dtstartmonth='.dol_print_date($startDate,'%m');
		$ret .= '&search_datalezione_dtstartyear='.dol_print_date($startDate,'%Y');
		$ret .= '&search_datalezione_dtendday='.dol_print_date($endDate,'%d');
		$ret .= '&search_datalezione_dtendmonth='.dol_print_date($endDate,'%m');
		$ret .= '&search_datalezione_dtendyear='.dol_print_date($endDate,'%Y');
	...
	{
		$sql = "SELECT date_format(p.datalezione,'%m') as dm, sum(p.".$this->field.")";
		$sql .= " FROM ".$this->from;
		//if (empty($user->rights->societe->client->voir) && !$user->socid) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
