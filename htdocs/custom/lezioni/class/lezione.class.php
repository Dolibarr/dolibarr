...
/**
 * \file        class/lezione.class.php
 * \ingroup     lezioni
 * \brief       This file is a CRUD class file for Lezione (Create/Read/Update/Delete)
 */
 ...
 * Class for Lezione
 */
class Lezione extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'lezione';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'lezioni_lezione';
	...
	/**
	 * @var string String with name of icon for lezione. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'lezione@lezioni' if picto is file 'img/object_lezione.png'.
	 */
	...
	//  */
	// public $table_element_line = 'lezioni_lezioneline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_lezione';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Lezioneline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Lezione', 'fk_element'=>'fk_lezione'));
	...
	//  */
	// protected $childtablesoncascade = array('lezioni_lezionedet');

	// /**
	//  * @var LezioneLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('lezioni', 'lezione', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('lezioni', 'lezione_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'lezione/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'lezione/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'lezione/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'lezione/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->lezioni->dir_output.'/lezione/'.$oldref;
				$dirdest = $conf->lezioni->dir_output.'/lezione/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->lezioni->dir_output.'/lezione/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('lezioni','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('lezioni','lezioni_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'LEZIONI_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'LEZIONI_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'LEZIONI_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowLezione")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Lezione").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/lezioni/lezione_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new LezioneLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_lezione = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('LEZIONI_MYOBJECT_ADDON')) {
			$conf->global->LEZIONI_MYOBJECT_ADDON = 'mod_lezione_standard';
		}

		if (getDolGlobalString('LEZIONI_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('LEZIONI_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('LEZIONI_MYOBJECT_ADDON');
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/lezioni/doc/";
...
/**
 * Class LezioneLine. You can also remove this and generate a CRUD class for lines objects.
 */
class LezioneLine extends CommonObjectLine
{
	// To complete with content of an object LezioneLine
	// We should have a field rowid, fk_lezione and position
