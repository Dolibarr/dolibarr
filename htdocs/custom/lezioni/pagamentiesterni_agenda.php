...
/**
 *  \file       pagamentiesterni_agenda.php
 *  \ingroup    lezioni
 *  \brief      Tab of events on PagamentiEsterni
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->lezioni->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('lezioni', 'pagamentiesterni', 'write');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
}
...
if ($object->id > 0) {
	$title = $langs->trans("PagamentiEsterni")." - ".$langs->trans('Agenda');
	//$title = $object->ref." - ".$langs->trans("Agenda");
	$help_url = 'EN:Module_Agenda_En|DE:Modul_Terminplanung';

	llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-lezioni page-card_agenda');
	...
	print dol_get_fiche_head($head, 'agenda', $langs->trans("PagamentiEsterni"), -1, $object->picto);
	...
	if (isModEnabled('agenda') && ($user->hasRight('agenda', 'myactions', 'read') || $user->hasRight('agenda', 'allactions', 'read'))) {
		...
		//$cachekey = 'count_events_pagamentiesterni_'.$object->id;
