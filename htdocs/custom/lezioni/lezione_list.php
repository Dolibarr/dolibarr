...
/**
 *   	\file       lezione_list.php
 *		\ingroup    lezioni
 *		\brief      List page for lezione
 */
...
$fileName = 'test.csv';
$upload_dir = $conf->lezioni->multidir_output[$conf->entity]."/temp/".$user->id.'/';
$csvDownloadUrl = dol_buildpath('/document.php', 1).'?modulepart=lezioni&file=temp%2F'.$user->id.'%2F'.$fileName;
//create csv file dir if not exists
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('lezioni', 'lezione', 'write');
	$permissiontodelete = $user->hasRight('lezioni', 'lezione', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("lezioni")) {
	accessforbidden('Module lezioni not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Lezione';
	$objectlabel = 'Lezione';
	$uploaddir = $conf->lezioni->dir_output;
...
$title = $langs->trans("Lezioni");
//$help_url = "EN:Module_Lezione|FR:Module_Lezione_FR|ES:Módulo_Lezione";
$help_url = '';
$morejs = array();
$morecss = array();
...
$sql .= ", m.firstname as i_firstname, m.lastname as i_lastname";
...
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."adherent as m ON t.istruttore = m.rowid";
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/lezioni/lezione_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-lezioni page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/lezioni/lezione_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
$newcardbutton .= dolGetButtonTitle('Export to CSV', '', 'fa fa-share-square', $csvDownloadUrl, '', 1);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendLezioneRef";
$modelmail = "lezione";
$objecttmp = new Lezione($db);
...
print '<div class="div-table-responsive" style="max-width: 100%; overflow-x: auto;">'; // You can use div-table-responsive-no-min if you dont need reserved height for your table
	...
	if ($mode == 'kanban' && $exportCsv) {
		...
		foreach ($object->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.'.$key]['checked'])) {
				...
				elseif($key == 'istruttore' || $key == 'allievo')
				{
					$adh = new Adherent($db);
					$adh->fetch($object->$key);
...
$exportLine = '';
//CSV EXPORT
$resql->data_seek(0); //reset result set
while ($i < $num) {
	$obj = $db->fetch_object($resql);
	if (empty($obj)) {
		break; // Should not happen
	...
	$exportLine .= $object->id;

	foreach ($object->fields as $key => $val) {
		if (!empty($arrayfields['t.'.$key]['checked'])) {

			if ($key == 'status') {
				$exportLine .= ','.$object->$key;
			} elseif ($key == 'rowid') {
				//already printed
			}
			elseif($key == 'istruttore' || $key == 'allievo')
			{
				...
				$vType = gettype($object->$key);
				$class = $vType;
				unset($times);
				if($vType == 'integer'){
					$times = strtotime($object->$key);
				}
				if (isset($times) && $times != ''){
					$exportLine .= ','.dol_print_date($object->$key,$langs->trans("FormatDateShortInput"));
				}else{
					$exportLine .= ','.$object->$key;
				}
			}
...
//print $upload_dir;
//print $exportLine;
$handle = fopen($upload_dir.$fileName, "w");
fwrite($handle, $exportLine);
fclose($handle);
