...
/**
 * \file    dienste/admin/setup.php
 * \ingroup dienste
 * \brief   Dienste setup page.
 */
...
// Translations
$langs->loadLangs(array("admin", "dienste@dienste"));
...
$arrayofparameters = array(
	'DIENSTE_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'DIENSTE_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/dienste/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DIENSTE_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'DIENSTE_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$page_name = "DiensteSetup";
llxHeader('', $langs->trans($page_name));

// Subheader
$linkback = '<a href="'.($backtopage ? $backtopage : DOL_URL_ROOT.'/admin/modules.php?restore_lastsearch_values=1').'">'.$langs->trans("BackToModuleList").'</a>';

print load_fiche_titre($langs->trans($page_name), $linkback, 'object_dienste@dienste');

// Configuration header
$head = diensteAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "dienste@dienste");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("DiensteSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'DIENSTE_'.strtoupper($myTmpObjectKey).'_ADDON';
