 ...
 */
class bueroDienste extends ModeleBoxes
{
	...
	public $boxcode = "bueroDienste";
	public $boxlabel = 'bueroDienste';
	...
	public function loadBox($max = 5)
	{
		...
		$sql = "select d.ref, d.user, d.description, u.rowid from " . MAIN_DB_PREFIX . "dienste_buero as d ";
		$sql .= "join " . MAIN_DB_PREFIX . "user as u on d.user=u.rowid ";
		$sql .= "order by d.ref asc";

		$res = $db->query($sql);

		$textHead = $langs->trans("Kommende Bürodienste");
		...
		for ($i = 0; $i < $res->num_rows; $i++) {
			...
			if($tel->ref > time()) {
				$this->info_box_contents[$line][0] = array(
					...
					'text' => $buero->getNomUrl(null, null, null, null, null, dol_print_date($obj->ref, 'day')),
					'asis' => 1,
				);
