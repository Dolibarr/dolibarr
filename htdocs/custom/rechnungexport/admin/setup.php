...
/**
 * \file    rechnungexport/admin/setup.php
 * \ingroup rechnungexport
 * \brief   RechnungExport setup page.
 */
...
// Translations
$langs->loadLangs(array("admin", "rechnungexport@rechnungexport"));
...
$arrayofparameters = array(
	'RECHNUNGEXPORT_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'RECHNUNGEXPORT_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/rechnungexport/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'RECHNUNGEXPORT_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'RECHNUNGEXPORT_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
print dol_get_fiche_head($head, 'settings', '', -1, "rechnungexport@rechnungexport");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("RechnungExportSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'RECHNUNGEXPORT_'.strtoupper($myTmpObjectKey).'_ADDON';
