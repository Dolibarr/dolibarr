...
/**
 * \file    test/phpunit/RechnungExportFunctionalTest.php
...
/**
 * Class RechnungExportFunctionalTest
 *
 ...
 *
 * @package Testrechnungexport
 */
class RechnungExportFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modRechnungExport')->click();
		} else {
			// Disable the module
			$this->byHref('modRechnungExport')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/rechnungexport/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('rechnungexport/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/rechnungexport/admin/about.php');
		$this->authenticate();
		return $this->assertContains('rechnungexport/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('rechnungexportwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modRechnungExport_RechnungExportTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modRechnungExport_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
