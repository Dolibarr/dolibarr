...
/**
 *   	\file       vehiclerequest_card.php
 *		\ingroup    vehiclerequest
 *		\brief      Page to create/edit/view vehiclerequest
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'vehiclerequestcard'; // To manage different context of search
...
$hookmanager->initHooks(array('vehiclerequestcard', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoapprove=$user->rights->fsa->vehicle->supervisorapprove;
	$permissiontoassign=$user->rights->fsa->vehicle->assign;
	...
	$permissiontoassign=1;
	$permissiontodelete = 1;
	$permissionnote = 1;
	$permissiondellink = 1;
}

$upload_dir = $conf->vehiclerequest->multidir_output[isset($object->entity) ? $object->entity : 1].'/vehiclerequest';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/vehiclerequest/vehiclerequest_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/vehiclerequest/vehiclerequest_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'VEHICLEREQUEST_VEHICLEREQUEST_MODIFY'; // Name of trigger action code to execute when we modify record

	// Actions cancel, add, update, update_extras, confirm_validate, confirm_delete, confirm_deleteline, confirm_clone, confirm_close, confirm_setdraft, confirm_reopen
	include DOL_DOCUMENT_ROOT.'/core/actions_addupdatedelete.inc.php';

	//include my custom actions
	include 'vehiclerequest_card_actions.php';
	...
	$triggersendname = 'VEHICLEREQUEST_VEHICLEREQUEST_SENTBYMAIL';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_VEHICLEREQUEST_TO';
	$trackid = 'vehiclerequest'.$object->id;
...
$title = $langs->trans("VehicleRequest");
$help_url = '';
...
if ($action == 'create') {
	...
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("VehicleRequest")), '', 'object_'.$object->picto);
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("VehicleRequest"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("VehicleRequest"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'setapproved') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('ApproveVehicleRequest'), $langs->trans('ConfirmRequestApproval'), 'confirm_setapproved', '', 0, 1);
	}
...
//			print dolGetButtonAction($langs->trans('ToClone'), '', 'default', $_SERVER['PHP_SELF'].'?id='.$object->id.(!empty($object->socid)?'&socid='.$object->socid:'').'&action=clone&token='.newToken(), '', $permissiontoadd);
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('vehiclerequest:VehicleRequest', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('vehiclerequest'));
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-list-alt imgforviewmode', dol_buildpath('/vehiclerequest/vehiclerequest_agenda.php', 1).'?id='.$object->id);
