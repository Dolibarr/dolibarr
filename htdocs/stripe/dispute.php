...
if (!$rowid)
{
    ...
    $title=$langs->trans("StripeDisputeList");
	...
	if ($stripeacc)
	{
		$list=\Stripe\Dispute::all(array("limit" => $limit), array("stripe_account" => $stripeacc));
	}
	else
	{
		$list=\Stripe\Dispute::all(array("limit" => $limit));
	}
...
print $stripe->getPaymentIntent($object);

$payment_intent = \Stripe\PaymentIntent::retrieve("pi_1DZGTnK034Aqz8l5urP3R1s0");;
$payment_intent->customer= "cus_CwaI3vaovKiuJv";
$payment_intent->save();

//print $pay = \Stripe\PaymentIntent::create([
//  "amount" => 1099,
//  "currency" => "eur",
//  "allowed_source_types" => ["card"],
//]);
print "<br><br><br>";
$intents = \Stripe\PaymentIntent::all(["limit" => 3]);
