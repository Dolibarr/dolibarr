...
**Tech Stack:** PHP 8.2+ (repo dev container runs PHP 8.5), Dolibarr module framework, `google/apiclient` ^2.9 + `google/apiclient-services` (Drive service) already vendored via the sibling `prune` module, jQuery + `jqueryFileTree` (the plugin file is already shipped with Dolibarr core, but it is NOT loaded globally — each page that uses it must push it into its own `$morejs` array, the way `htdocs/ecm/index.php:334` does; a page that forgets this gets a silent `fileTree is not a function` failure in the browser. Corrected here after the final whole-branch review caught this exact bug: Task 3's page skeleton needs its own `$morejs` entry for `public/includes/jquery/plugins/jqueryFileTree/jqueryFileTree.js`, in addition to the module's own JS file).
