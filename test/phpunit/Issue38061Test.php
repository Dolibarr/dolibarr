...
class Issue38061Test extends CommonClassTest
{
	...
	public function testVATReportIncludesPartiallyPaidInvoiceWithSplitCreditNote()
	{
		...
		// For branch 22.0, manually create two discounts instead of using splitAmount
		$amount_ttc_1 = 10.00;
		$amount_ttc_2 = 1196.00 - 10.00;  // 1186.00

		// Calculate amounts for part 1 (10 TTC)
		// Since the original is 1196 TTC = 1000 HT + 196 VAT (19.6%)
		// For 10 TTC: HT = 10 / 1.196 = 8.361..., VAT = 10 - 8.361... = 1.638...
		// For simplicity, we'll use approximate values
		$ht_1 = round(10.00 / 1.196, 2);
		$vatt_1 = round(10.00 - $ht_1, 2);

		// Calculate amounts for part 2 (1186 TTC)
		$ht_2 = round(1186.00 / 1.196, 2);
		$vatt_2 = round(1186.00 - $ht_2, 2);
		...
		// Create the two split discounts manually
		$discount_part1 = new DiscountAbsolute($db);
		$discount_part1->socid = $socid;
		$discount_part1->fk_facture_source = $creditnoteid;
		$discount_part1->amount_ht = $ht_1;
		$discount_part1->amount_tva = $vatt_1;
		$discount_part1->amount_ttc = $amount_ttc_1;
		$discount_part1->tva_tx = 19.6;
		$discount_part1->description = '(CREDIT_NOTE) - Part 1';
		$discount_part1->discount_type = 0;
		$discount_part1->datec = dol_now();
		$discount_part1->fk_user = $user->id;

		$discount_part2 = new DiscountAbsolute($db);
		$discount_part2->socid = $socid;
		$discount_part2->fk_facture_source = $creditnoteid;
		$discount_part2->amount_ht = $ht_2;
		$discount_part2->amount_tva = $vatt_2;
		$discount_part2->amount_ttc = $amount_ttc_2;
		$discount_part2->tva_tx = 19.6;
		$discount_part2->description = '(CREDIT_NOTE) - Part 2';
		$discount_part2->discount_type = 0;
		$discount_part2->datec = dol_now();
		$discount_part2->fk_user = $user->id;
