...
class DeletionLogTest extends CommonClassTest
{
	...
	public function testAddCreatesRow()
	{
		...
		$id = $o->add('unittest_dl_add', 4242, $user, $this->savconf->entity);
	...
	public function testAddRejectsBadInput()
	{
		...
		$o = new DeletionLog($db);
		$this->assertLessThan(0, $o->add('', 5, $user, 1), 'empty element type must be rejected');
		$this->assertLessThan(0, $o->add('unittest_dl_bad', 0, $user, 1), 'non positive id must be rejected');
	}
	...
	public function testGetDeletionsSinceReturnsRecentIdsOnly()
	{
		...
		$o = new DeletionLog($db);
		$o->add($element, 101, $user, $entity);
		$o->add($element, 102, $user, $entity);
		// A different element type must be ignored.
		$o->add('unittest_dl_other', 103, $user, $entity);

		$ids = $o->getDeletionsSince($element, $now - 24 * 3600, $entity);
	...
	public function testTriggerRecordsAgendaEventDeletion()
	{
		...
		$ids = $deletionlog->getDeletionsSince('action', $now - 3600, $this->savconf->entity);
