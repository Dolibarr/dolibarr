...
class FilesLibRenameTest extends CommonClassTest
{
	...
	public function testRenameNonEmptyDirToEmptyDir()
	{
		...
		$this->assertTrue(dol_rename($this->sourceDir, $this->destinationDir), "Failed renaming '{$this->sourceDir}' to '{$this->destinationDir}'");
		...
		$this->assertFileExists($this->destinationDir . '/file2.txt', "file2.txt must exist in destubation");
	...
	public function testRenameEmptyDirToNonEmptyDir()
	{
		...
		$this->assertFalse(dol_rename($src, $dst), "Rename empty to non empty directory must fail");
	...
	public function testRenameEmptyDirToExistingEmptyDir()
	{
		...
		$this->assertTrue(dol_rename($src, $dst), "Renaming the empty directory to an empty one did not succeed");
	...
	public function testRenameNonEmptyDirToNonEmptyDir()
	{
		...
		// Try to rename
		$this->assertFalse(dol_rename($src, $dst), "Rename non empty directory to non empty directory must fail");
	...
	public function testRenameDirectoryDepthOfTwo()
	{
		...
		$this->assertTrue(dol_rename($srcDir, $dstDir), "Rename did not succeed");
