...
/**
 *      \file       test/phpunit/OnNotSuccessfulTestTraitNever.php
 *		\ingroup    test
 *      \brief      Provides CommonClassTest::onNotSuccessfulTest() with a ": never"
 *                  return type (PHPUnit >= 12, which declares this method as ": never").
 *                  This file is only loaded on PHP 8.2+ (where the "never" type exists)
 *                  and defines the SAME trait name as OnNotSuccessfulTestTrait.php, so
 *                  only one of the two files is ever loaded (see CommonClassTest.class.php).
 *      \remarks    This file uses the "never" return type, so it is NOT parseable on
 *                  PHP < 8.2; it must never be included on those versions.
 */

/**
 * Trait injected into CommonClassTest to keep the onNotSuccessfulTest override
 * compatible with PHPUnit versions that declare it with a ": never" return type.
 *
