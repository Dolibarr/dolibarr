...
if ($resql)
{
    ...
    if ($num)
    {
        while ($i < $num)
        {
            ...
            {
            	$message .= $langs->trans("Contract")." ".$obj->ref.": ".$langs->trans("Service")." ".$obj->label." (".price($obj->total_ttc).") ".$obj->nom."\n\n";
...
function envoi_mail($mode,$oldemail,$message,$total,$userlang,$oldsalerepresentative,$duration_value)
{
    ...
    else
    {
    	...
    	$allmessage.= $newlangs->transnoentities("ListOfServicesToExpireNote").($usehtml?"<br>\n":"\n").($usehtml?"<br>\n":"\n");;
    }
