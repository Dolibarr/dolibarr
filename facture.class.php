    ...
    public function createFromShipment($object, User $user)
	{
		...
		if ($order == 'commande'){
			require_once DOL_DOCUMENT_ROOT.'/commande/class/commande.class.php';
				...
				dol_syslog("recup order ".$idcom, LOG_DEBUG);
				$com->fetch($idcom);
				//$com->fetch_optionals($idcom);
				//if (empty($com->lines) && method_exists($com, 'fetch_lines'))
				//$com->fetch_lines();
				//$com->fetch_thirdparty();
				dol_syslog("order from ship fetch ok", LOG_DEBUG);
		}
		dol_syslog("no order", LOG_DEBUG);
		$num = count($object->lines);
		for ($i = 0; $i < $num; $i++)
		{
			$line = new FactureLigne($this->db);
			$line->libelle = $object->lines[$i]->libelle; // deprecated //ok
			$line->desc				= $object->lines[$i]->desc;		//ok
			$line->subprice			= $object->lines[$i]->subprice;
			$line->total_ht			= $object->lines[$i]->total_ht;		//ok
			$line->total_tva		= $object->lines[$i]->total_tva;	//ok
			$line->total_localtax1	= $object->lines[$i]->total_localtax1;	//ok
			$line->total_localtax2	= $object->lines[$i]->total_localtax2;	//ok
			$line->total_ttc		= $object->lines[$i]->total_ttc;		//ok
			$line->tva_tx = $object->lines[$i]->tva_tx;	//ok
			$line->qty = $object->lines[$i]->qty;  //ok
			$line->remise_percent = $object->lines[$i]->remise_percent;//ok
			$line->fk_product = $object->lines[$i]->fk_product;//ok
			$line->rang = $object->lines[$i]->rang;//ok
			$line->fk_unit = $object->lines[$i]->fk_unit;//ok

			// Multicurrency
			$line->fk_multicurrency = $object->lines[$i]->fk_multicurrency;// KO NOOO
			$line->multicurrency_code = $object->lines[$i]->multicurrency_code;// KO NOOO
			$line->multicurrency_subprice = $object->lines[$i]->multicurrency_subprice;// KO NOOO
			$line->multicurrency_total_ht = $object->lines[$i]->multicurrency_total_ht;// KO NOOO
			$line->multicurrency_total_tva = $object->lines[$i]->multicurrency_total_tva;// KO NOOO
			$line->multicurrency_total_ttc = $object->lines[$i]->multicurrency_total_ttc;// KO NOOO
			...
			$object->lines[$i]->fetch_optionals();
			foreach ($object->lines[$i]->array_options as $options_key => $value)
				$line->array_options[$options_key] = $value;//ok suivi

			$this->lines[$i] = $line;
		}

		$this->socid                = $object->socid;  //ok
		$this->fk_project           = $object->fk_project;  //ok
		$this->fk_account = $object->fk_account; //ok
		$this->cond_reglement_id    = $object->cond_reglement_id; //ok
		$this->mode_reglement_id    = $object->mode_reglement_id; //ok
		if ($order == 'commande'){
			$this->mode_reglement_id    = $com->mode_reglement_id; //ok
			$this->availability_id      = $com->availability_id;				//KO NOOOOOOO
			$this->demand_reason_id     = $com->demand_reason_id;			//KO NOOOOOOO
			$this->module_source = $com->module_source;		//KO NOOOOO
			$this->pos_source = $com->pos_source;			//KO NOOOO
		}
		$this->date_livraison       = $object->date_livraison;			// date_delivery
		$this->fk_delivery_address  = $object->fk_delivery_address;		// deprecated ok
		$this->contact_id           = $object->contactid; //ok
		$this->ref_client           = $object->ref_customer;				// ref_customer !

		if (empty($conf->global->MAIN_DISABLE_PROPAGATE_NOTES_FROM_ORIGIN))
		{
		    $this->note_private = $object->note_private;	//ok
            $this->note_public = $object->note_public;		//ok
		}

		$this->origin = $object->element;		//ok			
		$this->origin_id = $object->id;			//ok
